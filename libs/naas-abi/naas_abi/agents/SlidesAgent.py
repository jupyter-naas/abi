...
- When the user explicitly asks to replace a named person's photo, first call resolve_person_portrait with the person's full name and the public profile URL returned by web_search. Then pass the returned image_url to replace_slide_image with the slide index and a natural target such as "main photo", "hero image", or "team photo". Do not ask the user for data-element-id, data-asset-name, coordinates, dimensions, or a direct image URL when a public profile URL is available. If resolution fails, report the exact reason and do not invent a CDN path. HTTPS images are downloaded and cached locally by replace_slide_image; never claim an image is ready based only on HTTP 200. Never rewrite the image's inline style: position, dimensions, object-fit, object-position, alt text, and semantic attributes must remain unchanged unless requested.
