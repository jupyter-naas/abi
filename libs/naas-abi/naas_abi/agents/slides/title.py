"""Name a deck after the topic the user asked about.

``create_slides_project`` takes a title from the model, which often arrives as
the raw request ("fais des slides sur les materiaux de construction") or not at
all. The deck then shows up as "Untitled presentation" in the sidebar tree, as
...
reads like a deck request, meaning a deck noun followed by a topic connector
("sur", "about", "on", "concernant"). "Change the cover title to X" is an edit
instruction, not a name, so it derives nothing and the deck keeps its title.
...
_DECK_NOUN_RE = re.compile(
    r"\b(?:pr[ée]sentations?|diaporamas?|slide\s*decks?|slideshows?|slides?"
    r"|decks?|pitchs?|expos[ée]s?)\b",
    re.IGNORECASE,
)

# A connector is required: it is what separates "make a deck" from the topic.
...
def _fold(text: str) -> str:
    """Lowercase and drop accents so 'Nouvelle présentation' matches."""
    import unicodedata
...
def is_placeholder_deck_title(title: str) -> bool:
...
def derive_deck_title(brief: str) -> str:
    """Topic of a deck request, or "" when the text is not a deck request."""
    text = _normalize(brief)
    if not text or is_placeholder_deck_title(text):
        return ""
...
def resolve_deck_title(title: str, brief: str = "") -> str:
    """Display title for a deck, given the model's title and the turn's brief.
    ...
    if candidate and not is_placeholder_deck_title(candidate):
        derived = derive_deck_title(candidate)
        if derived:
            return derived
