"""Name a workbook after the topic the user asked about.

``create_sheets_project`` takes a title from the model, which often arrives as
the raw request ("fais des sheets sur les materiaux de construction") or not at
all. The workbook then shows up as "Untitled workbook" in the sidebar tree, as
...
reads like a workbook request, meaning a workbook noun followed by a topic connector
("sur", "about", "on", "concernant"). "Change the cover title to X" is an edit
instruction, not a name, so it derives nothing and the workbook keeps its title.
...
# A connector is required: it is what separates "make a workbook" from the topic.
...
def is_placeholder_workbook_title(title: str) -> bool:
...
def derive_workbook_title(brief: str) -> str:
    """Topic of a workbook request, or "" when the text is not a workbook request."""
    text = _normalize(brief)
    if not text or is_placeholder_workbook_title(text):
        return ""
...
def resolve_workbook_title(title: str, brief: str = "") -> str:
    """Display title for a workbook, given the model's title and the turn's brief.
    ...
    if candidate and not is_placeholder_workbook_title(candidate):
        derived = derive_workbook_title(candidate)
        if derived:
            return derived
        if not _looks_like_a_request(candidate):
            return _tidy_topic(candidate) or candidate
    return derive_workbook_title(brief)
