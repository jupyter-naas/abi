"""SheetsAgent tools for Nexus Slides projects.
...
Decks live at ``sheets/<slug>/workbook.html`` on branch ``sheets/<slug>``. When the
user has a deck open in Nexus, ``sheets_active_slug`` is set so tools default
...
def _load_seed_workbook_html() -> str | None:
...
def _looks_like_missing_deck(exc: BaseException, paths: dict[str, str]) -> bool:
    text = str(exc)
    deck = paths.get("workbook_path") or ""
...
def _ensure_sheets_write_paths(slug: str) -> dict[str, str]:
...
def _workbook_path(slug: str, workspace_id: str | None = None) -> str:
    ws = workspace_id or _workspace_id()
    if ws:
        return f"slides/{_workspace_segment(ws)}/{slug}/workbook.html"
    return f"slides/{slug}/workbook.html"
...
def _resolve_paths(slug: str) -> dict[str, str]:
    ...
    if ws:
        ...
        if ns_branch in names:
            return {
                "branch": ns_branch,
                "workbook_path": _workbook_path(slug, ws),
...
def _open_deck_note(slug: str) -> dict[str, Any]:
    title = sheets_active_title.get()
    mode = sheets_active_mode.get()
    note: dict[str, Any] = {
        "open_deck": {
            "slug": slug,
            "path": _workbook_path(slug),
            "branch": _branch(slug),
        }
    }
    if title:
...
def _load_deck_via_forgejo(slug: str) -> str | dict[str, Any]:
    ...
    try:
        file = sc.get_file(
            repo_id=_repo_id(), path=paths["workbook_path"], ref=paths["branch"]
        )
    except SourceControlError as exc:
        if _looks_like_missing_deck(exc, paths):
            seed = _load_seed_workbook_html()
            if seed:
                return seed
...
def _commit_deck_forgejo(
    ...
    paths = _ensure_sheets_write_paths(slug)
    if paths.get("error"):
        return {"error": paths["error"], "source": "forgejo"}
    sc = _get_source_control()
    try:
...
def _load_workbook_text(slug: str) -> tuple[str | dict[str, Any], str]:
...
def slides_tools() -> list[BaseTool]:
    ...
    def create_slides_project(title: str) -> dict[str, Any]:
        ...
        try:
            ...
            seed = _load_seed_workbook_html()
            if not seed:
                return {"error": "Sheets template is missing; cannot seed a deck."}
            commit = sc.upsert_file(
                repo_id=repo_id,
                path=paths["workbook_path"],
    ...
    def list_slides_projects() -> dict[str, Any]:
        ...
        if not agent_user_id.get():
            return {"error": "No authenticated user on this agent session."}
        open_slug = sheets_active_slug.get()
        try:
            sc = _get_source_control()
            repo_id = _ensure_coding_repo()
            ws = _workspace_id()
                ...
                projects.append(
                    {
                        ...
                        "branch": branch.name,
                        "workbook_path": (
                            _workbook_path(slug, ws)
                            if ns_prefix and name.startswith(ns_prefix)
                            else _legacy_workbook_path(slug)
                        ),
    ...
    def list_slides_sections(slug: str = "") -> dict[str, Any]:
        ...
        if isinstance(resolved, dict):
            return resolved
        try:
            html, source = _load_workbook_text(resolved)
    ...
    def read_slides_section(
        ...
        try:
            ...
            if isinstance(resolved_idx, dict):
                return resolved_idx
            blocked = reject_sheets_section_read(resolved_idx)
            if blocked:
                return blocked
            note_sheets_section_read(resolved_idx)
    ...
    def write_slides_section(
        ...
        index: int | None = None,
        section_id: str | None = None,
        message: str = "refactor(sheets): rewrite section via Abi",
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = _persist_workbook(
                resolved,
                new_html,
                message or "refactor(sheets): rewrite section via Abi",
                default_type="refactor",
            )
            if "error" not in result and written:
                result["section_index"] = written[0]
                note_sheets_write(f"slide {written[0] + 1}")
            result.update(_open_deck_note(resolved))
            return result
        except Exception as exc:  # noqa: BLE001
            return _tool_error(exc)
    ...
    def write_slides_sections(
        sections: str,
        slug: str = "",
        message: str = "refactor(sheets): rewrite sections via Abi",
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = _persist_workbook(
                resolved,
                new_html,
                message or "refactor(sheets): rewrite sections via Abi",
                default_type="refactor",
            )
            if "error" not in result and written:
                labels = [f"slide {idx + 1}" for idx in written]
                note_sheets_write(", ".join(labels))
    ...
    def replace_in_slides_deck(
        ...
        section_index: int | None = None,
        section_id: str | None = None,
        message: str = "fix(sheets): replace text via Abi",
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = _persist_workbook(
                resolved,
                updated,
                message or "fix(sheets): replace text via Abi",
                default_type="fix",
            )
            if "error" not in result:
                label = (
    ...
    def write_slides_deck(
        html: str,
        slug: str = "",
        message: str = "refactor(sheets): rewrite deck via Abi",
    ) -> dict[str, Any]:
        ...
        try:
            original = ""
            try:
                loaded, _source = _load_workbook_text(resolved)
                if isinstance(loaded, str):
                    original = loaded
            except Exception:  # noqa: BLE001
                original = ""
            ...
            result = _persist_workbook(
                resolved,
                content,
                message or "refactor(sheets): rewrite deck via Abi",
                default_type="refactor",
            )
            if "error" not in result:
                note_sheets_write("full deck")
            result.update(_open_deck_note(resolved))
            return result
        except Exception as exc:  # noqa: BLE001
            return _tool_error(exc)
    ...
    def insert_slide(
        ...
        title: str = "",
        slug: str = "",
        message: str = "feat(sheets): insert slide via Abi",
    ) -> dict[str, Any]:
        ...
        return _run_slide_mutation(
            ...
            message or "feat(sheets): insert slide via Abi",
            "insert slide",
            default_type="feat",
        )
    ...
    def delete_slide(
        index: int,
        slug: str = "",
        message: str = "refactor(sheets): delete slide via Abi",
    ) -> dict[str, Any]:
        ...
        return _run_slide_mutation(
            ...
            message or "refactor(sheets): delete slide via Abi",
    ...
    def duplicate_slide(
        index: int,
        slug: str = "",
        message: str = "feat(sheets): duplicate slide via Abi",
    ) -> dict[str, Any]:
        ...
        return _run_slide_mutation(
            ...
            message or "feat(sheets): duplicate slide via Abi",
    ...
    def reorder_slides(
        ...
        order: str = "",
        slug: str = "",
        message: str = "style(sheets): reorder slides via Abi",
    ) -> dict[str, Any]:
        ...
        return _run_slide_mutation(
            slug,
            _mutate,
            message or "style(sheets): reorder slides via Abi",
            "reorder slides",
            default_type="style",
        )
