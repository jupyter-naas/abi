...
def sheets_tools() -> list[BaseTool]:
    ...
    @tool
    def update_sheets_cells(sheet_name: str, cells_json: str, slug: str = "") -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(exc)}
        result = _persist_model(resolved, updated, html, f"feat(sheets): edit {sheet_name} cells")
        if "error" not in result:
            note_sheets_write("workbook")
    ...
    def evaluate_sheets_formulas(slug: str = "") -> dict[str, Any]:
        ...
        return {
            "ok": len(errors) == 0 and all(check["status"] != "BREAK" for check in checks),
            "slug": resolved,
            "source": source,
            "errors": errors,
            "checks": checks,
