...
def resolve_paths(slug: str) -> dict[str, str]:
    ...
    if legacy in names:
        ...
        try:
            meta = sc.get_file(
                repo_id=rid, path=_legacy_project_path(slug), ref=legacy
            )
...
def unique_slug(base: str, taken: set[str]) -> str:
    ...
    for suffix in range(2, 1000):
        ...
        if branch_name(candidate) not in taken and _legacy_branch(candidate) not in taken:
            return candidate
...
def _commit_workbook_forgejo(
    ...
    paths = ensure_sheets_write_paths(slug)
    if paths.get("error"):
        return {"error": paths["error"], "source": "forgejo"}
    sc = get_source_control()
    try:
...
def load_workbook_text(slug: str) -> tuple[str | dict[str, Any], str]:
    if _sidecar_available():
        loaded = _load_workbook_via_sidecar(slug)
        if isinstance(loaded, str):
            return loaded, "sidecar"
        forgejo = _load_workbook_via_forgejo(slug)
...
def persist_workbook(
    ...
    if _sidecar_available():
        sidecar_result = _write_workbook_via_sidecar(slug, html)
        if sidecar_result.get("ok"):
            sources.append("sidecar")
        else:
            sources.append("sidecar-failed")
    try:
        forgejo = _commit_workbook_forgejo(slug, html, message, default_type=default_type)
        ...
        if sidecar_result and sidecar_result.get("ok"):
            return {
                **sidecar_result,
                "sources": sources,
                "forgejo_error": friendly_sc_error(exc),
                "note": (
                    "Updated Coder workspace (live edit); Forgejo snapshot failed. "
                    "Preview should follow sidecar. Use File → Save later for history."
                ),
            }
        return {"error": friendly_sc_error(exc), "sources": sources}
