from naas_abi.agents.tools.sheets_workbook_storage import (
    branch_name,
    workbook_path,
)


def test_workbook_paths_are_namespaced_under_sheets() -> None:
    assert (
        workbook_path("budget-q1", "ws-abc") == "sheets/ws-abc/budget-q1/workbook.html"
    )
    assert branch_name("budget-q1", "ws-abc") == "sheets/ws-abc/budget-q1"


def test_viewer_agent_cannot_persist_even_with_valid_revision(monkeypatch):
    from naas_abi.agents.feature import runtime
    from naas_abi.agents.tools import sheets_workbook_storage as store
    from naas_abi_core.services.agent.context import agent_user_id, agent_workspace_id

    user = agent_user_id.set("viewer")
    workspace = agent_workspace_id.set("ws-test")
    monkeypatch.setattr(runtime, "check_member", lambda *args: "viewer")

    def unexpected(*args, **kwargs):
        raise AssertionError("must not reach storage")

    monkeypatch.setattr(store, "get_source_control", unexpected)
    try:
        result = store.persist_workbook(
            "demo", "changed", "edit", expected_revision="a" * 64
        )
        assert "writer role" in result["error"]
    finally:
        agent_user_id.reset(user)
        agent_workspace_id.reset(workspace)


def test_agent_full_replacement_rejects_revision_from_an_older_read(monkeypatch):
    from naas_abi.agents.tools import sheets_workbook_storage as store
    from naas_abi.agents.tools.sheets_tools import sheets_tools
    from naas_abi_core.services.agent.context import agent_user_id
    token = agent_user_id.set('writer')
    monkeypatch.setattr(store, 'load_workbook_text', lambda slug: ('current html', 'forgejo'))
    def unexpected(*args, **kwargs):
        raise AssertionError('must not persist stale workbook')
    monkeypatch.setattr(store, 'persist_workbook', unexpected)
    try:
        tool = next(t for t in sheets_tools() if t.name == 'write_sheets_workbook')
        result = tool.invoke({'slug':'demo', 'expected_revision':store.content_revision('old html'),
                              'workbook_json':'{"title":"Stale", "sheets":[{"name":"S", "rows":[[1]]}]}'})
        assert 'changed' in result['error']
    finally:
        agent_user_id.reset(token)
