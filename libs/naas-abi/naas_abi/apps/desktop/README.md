...
Use the **workspace switcher** (status bar left: folder name + chevron) to open or switch IDE workspaces. Each workspace is a folder on disk (e.g. `~/abi`). In browser dev, **Open Folder…** accepts a typed path; in the pywebview app, **Browse…** opens the native folder picker.
