...
p(
    folder="provider_integration",
    filename="provider_discovery_process",
    ...
    who={
        "ProviderDiscoveryServer": "FastAPI server executing provider enumeration",
        "UserRequester": "Authenticated user requesting available providers",
    },
    where={
        "ProvidersEndpoint": "API endpoint GET /api/providers/available",
    ...
    when={
        "DiscoveryDuration": "Temporal region of 50-200ms for secret queries and registry lookup"
    },
    how_know={
        "APIKeySecret": "Encrypted API key stored in secrets table (Fernet AES-128-CBC)",
    ...
    how_is={
        "ProviderAvailability": "Boolean quality indicating if a provider has a configured API key",
        "ModelCount": "Integer quality counting available models per provider",
    },
    why_roles={
        "ProviderEnumeratorRole": "Role realized when system scans secrets for provider API keys"
    },
    why_disps={
        "AlwaysIncludeOllamaDisposition": "Disposition to always include Ollama provider regardless of API key presence"
    },
    steps=[
        "Query user workspace memberships from workspace_members table",
...
p(
    folder="provider_integration",
    filename="openai_compatible_streaming_process",
    ...
    who={
        "OpenAICompatibleProvider": "External LLM API implementing OpenAI chat completions interface",
        "ChatBackendProxy": "FastAPI server proxying streaming requests to provider",
    },
    where={
        "ChatCompletionsEndpoint": "Provider endpoint {base_url}/chat/completions",
        "StreamEndpoint": "NEXUS endpoint POST /api/chat/stream",
    },
    when={
        "FirstTokenLatency": "200-2000ms from request to first token depending on provider and model",
        "StreamingWindow": "1-30 seconds total streaming duration",
    },
    how_know={
        "ProviderMessages": "Array of {role, content} messages formatted for provider API",
    ...
    how_is={
        "StreamingThroughput": "10-100 tokens/second depending on model and load",
        "ResponseCompleteness": "Quality measuring whether full response was received before timeout",
    },
    why_roles={
        "StreamProxyRole": "Role realized when backend proxies SSE from provider to frontend"
    },
    why_disps={
        "OpenRouterRefererDisposition": "Disposition to add HTTP-Referer header when provider is OpenRouter",
...
p(
    folder="provider_integration",
    filename="ollama_inference_process",
    ...
    who={
        "OllamaServer": "Local Ollama server running at http://localhost:11434",
        "LocalGPU": "Local GPU hardware executing model inference",
    },
    where={
        "OllamaChatEndpoint": "Ollama API endpoint POST /api/chat at localhost:11434",
        "OllamaTagsEndpoint": "Ollama API endpoint GET /api/tags for model listing",
    },
    when={
        "OllamaInferenceDuration": "1-60 seconds depending on model size (2B-70B parameters) and prompt length"
    },
    how_know={
        "OllamaMessage": "Message with role, content, and optional images array (base64)",
    ...
    how_is={
        "ModelAvailability": "Quality indicating if model is pulled and ready locally",
        "VisionSupport": "Boolean quality indicating multimodal image input support",
    },
    why_roles={
        "LocalInferenceRole": "Role realized when Ollama executes model inference on local hardware"
    },
    why_disps={
        "MultimodalDisposition": "Disposition to accept base64 images when model supports vision (llava, qwen3-vl, gemma3)",
...
p(
    folder="provider_integration",
    filename="cloudflare_workers_ai_process",
    ...
    who={
        "CloudflareEdgeNetwork": "Cloudflare's global edge infrastructure executing AI inference",
        "WorkersAIRuntime": "Cloudflare Workers AI model runtime",
    },
    where={
        "CloudflareAIEndpoint": "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/run/@cf/{model}",
        "CloudflareModelsEndpoint": "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/models/search",
    },
    when={
        "EdgeInferenceDuration": "500-5000ms depending on model and edge location proximity"
    },
    how_know={
        "CloudflareRequest": "JSON body with messages array and optional stream:true flag",
    ...
    how_is={
        "EdgeLatency": "Quality measuring response time based on edge node proximity",
        "ModelPathFormat": "Quality: model path format @cf/meta/llama-3.1-8b-instruct",
    },
    why_roles={
        "EdgeInferenceRole": "Role realized when Cloudflare executes inference at the edge"
    },
    why_disps={
        "SSEStreamingDisposition": "Disposition to stream via SSE when stream:true is set"
    },
    steps=[
        "Extract account_id from provider config or secrets",
...
p(
    folder="provider_integration",
    filename="abi_server_streaming_process",
    ...
    who={
        "ABIServer": "External Agentic Brain Infrastructure server (e.g., Forvis Mazars)",
        "ABIAgent": "Named agent on ABI server discovered via OpenAPI spec",
    },
    where={
        "ABIStreamEndpoint": "ABI endpoint POST /agents/{agent_name}/stream-completion?token={token}",
        "ABIOpenAPIEndpoint": "ABI endpoint GET /openapi.json for agent discovery",
    },
    when={
        "ABIStreamDuration": "1-30 seconds depending on agent complexity and query type",
        "ABIFirstTokenLatency": "200-2000ms for first ai_message event",
    },
    how_know={
        "ABIRequest": "JSON body: {prompt: latest_user_message, thread_id: message_hash}",
    ...
    how_is={
        "DuplicateFilteringQuality": "Quality of filtering: only yield from ai_message events, skip message events",
        "MultiLineDataQuality": "Quality: handle multiple data: lines per single event (W3C SSE standard)",
    },
    why_roles={
        "ABIProxyRole": "Role realized when NEXUS proxies requests to external ABI server"
    },
    why_disps={
        "DuplicateFilteringDisposition": "Disposition to only yield ai_message events to prevent duplicate content",
        "MultiLineDataDisposition": "Disposition to not reset current_event until next event: line arrives",
    },
    steps=[
        "Extract latest user message as prompt from messages array",
...
p(
    folder="provider_integration",
    filename="abi_agent_discovery_process",
    ...
    who={
        "ABIOpenAPIServer": "External ABI server serving OpenAPI/Swagger specification",
        "DiscoveryClient": "httpx async client fetching and parsing OpenAPI JSON",
    },
    where={
        "OpenAPIEndpoint": "ABI endpoint GET /openapi.json or GET /redoc",
    ...
    how_know={
        "OpenAPISpec": "JSON document containing paths, operations, and descriptions",
    ...
    how_is={
        "AgentCount": "Integer quality: number of agents discovered from OpenAPI spec",
        "SpecCompleteness": "Quality measuring if spec contains descriptions for agents",
    },
    why_roles={
        "AgentDiscovererRole": "Role realized when system parses OpenAPI to find agents"
    },
    why_disps={
        "OpenAPIParsingDisposition": "Disposition to extract agent names from path patterns"
    },
    steps=[
        "Fetch OpenAPI spec from {endpoint}/openapi.json using httpx async client",
        "Parse JSON response",
...
p(
    folder="agent_management",
    filename="agent_creation_process",
    ...
    who={
        "AgentCreator": "Authenticated user with workspace access creating the agent",
        "AgentConfigStore": "PostgreSQL agent_configs table storing agent definitions",
    },
    where={
        "AgentsEndpoint": "API endpoint POST /api/agents/",
    ...
    how_know={
        "AgentCreateRequest": "Pydantic model: name, description, system_prompt, model (default gpt-4), temperature (0.7), max_tokens (4096), workspace_id",
    ...
    how_is={
        "AgentNameUniqueness": "Quality: agent names should be unique within workspace (not enforced)",
        "DefaultTemperature": "Quality: default temperature of 0.7 for balanced creativity",
    },
    why_roles={
        "AgentDesignerRole": "Role realized when user defines agent personality and capabilities"
    },
    why_disps={
        "ModelRegistryLookupDisposition": "Disposition to infer provider from model_id via model registry"
    },
    steps=[
        "Validate workspace access via require_workspace_access(user_id, workspace_id)",
...
p(
    folder="agent_management",
    filename="agent_sync_from_registry_process",
    ...
    who={
        "ModelRegistry": "In-memory MODEL_REGISTRY containing 60+ model definitions across 9 providers",
        "WorkspaceAgentStore": "PostgreSQL agent_configs table for the target workspace",
    },
    where={
        "AgentSyncEndpoint": "API endpoint POST /api/agents/sync?workspace_id={id}",
        "AgentConfigsTable": "PostgreSQL table agent_configs",
    },
    when={"SyncDuration": "100-300ms for registry scan + batch insert of 60+ agents"},
    how_know={
        "ModelRegistryEntry": "ModelInfo: id, name, provider, context_window, capabilities, released date",
    ...
    how_is={
        "SyncCompleteness": "Quality: all registry models should have corresponding agents",
        "DuplicateSkipping": "Quality: existing agents (by model_id) are skipped, not duplicated",
    },
    why_roles={
        "AgentProvisionerRole": "Role realized when system auto-creates agents from model catalog"
    },
    why_disps={
        "BatchInsertDisposition": "Disposition to batch-insert multiple agents in single transaction"
    },
    steps=[
...
p(
    folder="agent_management",
    filename="agent_provider_resolution_process",
    ...
    who={
        "ProviderResolver": "Backend logic determining optimal provider for chat request",
    ...
    where={
        "AgentConfigsTable": "PostgreSQL agent_configs table with provider and model_id columns",
    ...
    when={
        "ResolutionDuration": "50-600ms: 50ms for DB queries, up to 500ms for Ollama health check"
    },
    how_know={
        "ProviderConfigRequest": "Pydantic model: type, model, endpoint, api_key, enabled",
    ...
    how_is={
        "ResolutionPriority": "Quality: explicit provider > agent config > secrets > Ollama fallback",
        "VisionModelSelection": "Quality: if has_images=true, prefer vision-capable models",
    },
    why_roles={
        "ProviderResolverRole": "Role realized when system determines optimal provider for request"
    },
    why_disps={
        "FallbackDisposition": "Disposition to fall back to Ollama when no cloud provider configured",
        "VisionAwareDisposition": "Disposition to select vision-capable model when images are attached",
    },
    steps=[
        "If provider explicitly provided and enabled: return it directly",
...
p(
    ...
    filename="graph_node_creation_process",
    ...
    who={
        "GraphAuthor": "Authenticated user creating the node",
        "GraphDatabase": "PostgreSQL database storing graph_nodes table",
    },
    where={
        "GraphNodesEndpoint": "API endpoint POST /api/graph/nodes",
    ...
    how_know={
        "GraphNodeCreate": "Pydantic model: workspace_id, type (1-100 chars), label (1-500 chars), properties (JSON dict)",
    ...
    how_is={
        "NodeTypeCardinality": "Quality: unlimited node types per workspace",
        "PropertyFlexibility": "Quality: properties stored as free-form JSON",
    },
    why_roles={
        "GraphBuilderRole": "Role realized when user adds entities to the knowledge graph"
    },
    why_disps={
        "JSONSerializationDisposition": "Disposition to serialize properties dict to JSON string for storage"
    },
    steps=[
        "Validate workspace access",
...
p(
    folder="knowledge_graph",
    filename="graph_edge_creation_process",
    ...
    how_know={
        "GraphEdgeCreate": "Pydantic model: workspace_id, source_id, target_id, type (1-100 chars), properties (JSON)",
        "EdgeRecord": "Database row: id (edge-{uuid}), workspace_id, source_id, target_id, type, properties",
    },
    how_is={
        "ReferentialIntegrity": "Quality: both source and target nodes must exist (404 if not)",
        "CascadeDeleteBehavior": "Quality: deleting a node cascades to delete all connected edges",
    },
    why_roles={
        "RelationshipCreatorRole": "Role realized when user defines connections between entities"
    },
    why_disps={
        "NodeExistenceValidationDisposition": "Disposition to verify source and target node existence before edge creation"
    },
    steps=[
        "Validate workspace access",
...
p(
    ...
    who={
        "QueryExecutor": "Backend logic executing graph searches",
        "GraphDatabase": "PostgreSQL database with graph_nodes and graph_edges tables",
    },
    where={
        "GraphQueryEndpoint": "API endpoint POST /api/graph/query?workspace_id={id}",
    ...
    how_know={
        "GraphQuery": "Pydantic model: query (1-10000 chars), language (natural/sparql), limit (1-5000, default 100)",
    ...
    how_is={
        "ResultLimit": "Quality: max 5000 nodes per query result",
    ...
    why_roles={
        "GraphSearcherRole": "Role realized when system searches knowledge graph for matching entities"
    },
    why_disps={
        "TextSearchDisposition": "Disposition to use ILIKE text matching (not semantic search)"
    },
    steps=[
        "Validate workspace access",
    ...
    data_props={
        ...
        "resultEdgeCount": "integer",
    },
)

p(
    folder="knowledge_graph",
    filename="graph_statistics_process",
    ...
    who={
        "StatisticsEngine": "SQL aggregate query executor",
        "GraphDatabase": "PostgreSQL with graph_nodes and graph_edges tables",
    },
    where={
        "StatisticsEndpoint": "API endpoint GET /api/graph/statistics/{workspace_id}"
    },
    when={"StatisticsDuration": "50-200ms for 4 aggregate queries"},
    how_know={
        "StatisticsResult": "Dict: total_nodes, total_edges, nodes_by_type, edges_by_type, avg_degree",
        "TypeDistribution": "Dict mapping type string to count integer",
        "AverageDegree": "Float: 2 * total_edges / total_nodes (0 if no nodes)",
    },
    how_is={
        "DegreeCalculation": "Quality: average degree formula = 2 * edges / nodes",
        "EmptyGraphHandling": "Quality: returns 0 for all metrics when graph is empty",
    },
    why_roles={
        "MetricsCollectorRole": "Role realized when system aggregates graph statistics"
    },
    why_disps={
        "SQLAggregateDisposition": "Disposition to use SQL COUNT and GROUP BY for efficient statistics"
    },
    steps=[
        "Validate workspace access",
...
p(
    folder="knowledge_graph",
    filename="graph_workspace_load_process",
    ...
    who={
        "GraphLoader": "Backend logic fetching full workspace graph",
        "GraphDatabase": "PostgreSQL storing nodes and edges",
    },
    where={
        "WorkspaceGraphEndpoint": "API endpoint GET /api/graph/workspaces/{workspace_id}",
    ...
    how_know={
        "GraphData": "Response: nodes list + edges list",
        "NodeTypeFilter": "Optional query param to filter by node type",
        "LoadLimit": "Default 1000, max 5000 nodes",
    },
    how_is={
        "FilterSupport": "Quality: optional node_type parameter for type-based filtering",
        "SizeLimit": "Quality: configurable limit prevents loading excessively large graphs",
    },
    why_roles={
        "GraphDataProviderRole": "Role realized when system provides full graph for visualization"
    },
    why_disps={
        "LazyEdgeLoadingDisposition": "Disposition to only load edges connecting returned nodes"
    },
    steps=[
        "Validate workspace access",
...
p(
    folder="ontology_management",
    filename="ontology_entity_creation_process",
    ...
    who={
        "OntologyDesigner": "User defining custom ontology entities",
    ...
    how_know={
        "EntityCreate": "Pydantic model: name (1-200 chars), description (max 2000), base_class (max 500, IRI)",
        "OntologyItem": "Response: id (entity-{timestamp}), name, type=entity, description, base_class, created_at",
    },
    how_is={
        "InMemoryPersistence": "Quality: items stored in memory only, lost on server restart",
        "IRIValidation": "Quality: base_class accepts any string (no IRI validation)",
    },
    why_roles={
        "OntologyEngineerRole": "Role realized when user defines domain-specific entities"
    },
    why_disps={
        "TimestampIDDisposition": "Disposition to generate entity ID from timestamp milliseconds"
    },
    steps=[
        "Generate entity ID: entity-{timestamp_ms}",
...
p(
    folder="ontology_management",
    filename="ontology_relationship_creation_process",
    ...
    where={
        "OntologyRelationshipEndpoint": "API endpoint POST /api/ontology/relationship"
    },
    when={"RelationshipCreationDuration": "Less than 1ms (in-memory)"},
    how_know={
        "RelationshipCreate": "Pydantic model: name (1-200 chars), description, base_property IRI",
        "OntologyItem": "Response: id (rel-{timestamp}), name, type=relationship",
    },
    how_is={
        "PropertyDirectionality": "Quality: relationships are unidirectional (no inverse auto-creation)"
    },
    why_roles={
        "PropertyDesignerRole": "Role realized when user defines ontology relationships"
    },
    why_disps={
        "SimpleStorageDisposition": "Disposition to store as simple list item without RDF graph backing"
    },
    steps=[
        "Generate relationship ID: rel-{timestamp_ms}",
...
p(
    folder="ontology_management",
    filename="reference_ontology_import_process",
    ...
    who={
        "OntologyParser": "Regex-based TTL parser extracting classes and properties",
    ...
    how_know={
        "ImportRequest": "Pydantic model: content (1-5000000 chars), filename (validated pattern)",
    ...
    how_is={
        "ParserLimitation": "Quality: basic regex parsing, not full Turtle parser",
        "MaxFileSize": "Quality: 5MB maximum file size for import",
    },
    why_roles={
        "OntologyImporterRole": "Role realized when system parses external ontology specifications"
    },
    why_disps={
        "RegexParsingDisposition": "Disposition to use regex patterns for TTL class/property extraction",
        "FormatDetectionDisposition": "Disposition to detect format from filename extension (.ttl, .owl, .rdf)",
    },
    steps=[
        "Determine format from filename extension (.ttl, .owl, .rdf)",
...
p(
    folder="ontology_management",
    filename="ontology_preview_process",
    ...
    how_know={
        "PreviewResult": "Dict: name, classCount, propertyCount, preview (first 5 classes + 5 properties)"
    },
    how_is={"PreviewLimit": "Quality: shows maximum 5 items of each type as preview"},
    why_roles={
        "PreviewProviderRole": "Role realized when system generates ontology summary for user review"
    },
    why_disps={
        "TruncationDisposition": "Disposition to truncate preview to first 5 items"
    },
    steps=[
        "Parse ontology file (same as import process)",
...
p(
    ...
    who={
        "UploadHandler": "FastAPI multipart parser processing file upload",
        "StorageBackend": "Local filesystem or S3-compatible object storage",
    },
    where={
        "FileUploadEndpoint": "API endpoint POST /api/files/upload",
    ...
    when={
        "UploadDuration": "50-5000ms: 50ms for small local files, up to 5s for large S3 uploads"
    },
    how_know={
        "UploadFile": "FastAPI UploadFile: filename, content_type, file bytes",
    ...
    how_is={
        "MaxFileSize": "Quality: 50MB upload limit enforced before storage",
        "PathSafety": "Quality: path sanitization prevents ../ directory traversal",
    },
    why_roles={
        "FileReceiverRole": "Role realized when system accepts and stores uploaded files"
    },
    why_disps={
        "S3FallbackDisposition": "Disposition to use S3 when STORAGE_TYPE=s3, local otherwise",
...
p(
    ...
    who={
        "DirectoryLister": "Backend logic enumerating directory contents",
    ...
    how_know={
        "FileListResponse": "Response: files list + current path",
    ...
    how_is={
        "SortOrder": "Quality: files sorted alphabetically by name",
        "S3FolderSimulation": "Quality: S3 folders simulated via CommonPrefixes with / delimiter",
    },
    why_roles={
        "DirectoryEnumeratorRole": "Role realized when system lists storage contents"
    },
    why_disps={
        "S3PrefixDisposition": "Disposition to use S3 list_objects_v2 with Delimiter=/ for folder emulation"
    },
    steps=[
        "If local: resolve full path, iterate directory contents, build FileInfo for each",
...
p(
    ...
    who={
        "FileManager": "Backend logic executing file operations",
        "StorageBackend": "Local filesystem or S3 storage",
    },
    where={
        "FileCRUDEndpoints": "POST /api/files/, GET /api/files/{path}, PUT /api/files/{path}, DELETE /api/files/{path}"
    },
    when={"CRUDDuration": "10-2000ms depending on operation and file size"},
    how_know={
        "CreateFileRequest": "Pydantic model: path (max 500), content (optional text)",
        "FileContent": "Response for read: name, path, content (text), size",
        "CreateFolderRequest": "Pydantic model: path for folder creation",
    },
    how_is={
        "PathTraversalProtection": "Quality: all paths sanitized to prevent ../ attacks",
        "ContentTypeDetection": "Quality: content type inferred from file extension",
    },
    why_roles={
        "FileOperatorRole": "Role realized when system performs CRUD on stored files"
    },
    why_disps={
        "DualStorageDisposition": "Disposition to support both local and S3 storage transparently"
    },
    steps=[
        "Create: write content to path (local mkdir -p, S3 put_object)",
...
p(
    folder="search",
    filename="wikipedia_search_process",
    ...
    who={
        "WikipediaAPI": "Wikipedia MediaWiki public API at en.wikipedia.org",
        "SearchClient": "httpx async client executing search request",
    },
    where={
        "WikipediaEndpoint": "https://en.wikipedia.org/w/api.php with action=query&generator=search",
        "WebSearchEndpoint": "NEXUS API endpoint POST /api/search/web",
    },
    when={"SearchDuration": "200-1000ms for Wikipedia API response"},
    how_know={
        "WikipediaParams": "API params: gsrsearch, gsrlimit, prop=extracts|pageimages|info, exsentences=3",
    ...
    how_is={
        "SnippetLength": "Quality: maximum 3 sentences per snippet via exsentences=3",
        "ThumbnailSize": "Quality: thumbnails requested at 200px via pithumbsize=200",
    },
    why_roles={
        "WebSearcherRole": "Role realized when system queries Wikipedia for knowledge"
    },
    why_disps={
        "RelevanceScoringDisposition": "Disposition to assign decreasing relevance: 1.0, 0.95, 0.90..."
    },
    steps=[
        "Build Wikipedia API URL with query parameters",
...
p(
    folder="search",
    filename="duckduckgo_search_process",
    ...
    who={
        "DuckDuckGoAPI": "DuckDuckGo Instant Answer public API",
        "SearchClient": "httpx async client",
    },
    where={
        "DDGEndpoint": "https://api.duckduckgo.com/?q={query}&format=json&no_html=1&skip_disambig=1"
    },
    when={"SearchDuration": "200-800ms for DuckDuckGo API response"},
    how_know={
        "DDGResponse": "JSON with AbstractText, AbstractURL, RelatedTopics array, Image",
        "RelatedTopic": "Topic with Text, FirstURL, Icon, optional nested Topics array",
    },
    how_is={
        "AbstractPriority": "Quality: abstract gets relevance 1.0, topics get 0.9-0.7",
        "NestedTopicSupport": "Quality: handles nested Topics arrays within RelatedTopics",
    },
    why_roles={
        "InstantAnswerRole": "Role realized when DDG provides instant answer with abstract"
    },
    why_disps={
        "HTMLStrippingDisposition": "Disposition to request no_html=1 for clean text results"
    },
    steps=[
        "Build DuckDuckGo API URL with q, format=json, no_html=1",
...
p(
    folder="search",
    filename="search_suggestions_process",
    ...
    who={
        "WikipediaOpenSearch": "Wikipedia OpenSearch API for autocomplete suggestions"
    },
    where={
        "SuggestionsEndpoint": "NEXUS API GET /api/search/suggestions?query={q}&limit={n}",
    ...
    how_know={
        "OpenSearchResponse": "JSON array: [query, [suggestions], [descriptions], [urls]]",
        "SuggestionList": "Array of suggestion strings (index 1 of response)",
    },
    how_is={
        "MaxSuggestions": "Quality: maximum 20 suggestions (configurable 1-20)",
        "MinQueryLength": "Quality: minimum 1 character for query",
    },
    why_roles={
        "AutocompletProviderRole": "Role realized when system provides search suggestions"
    },
    why_disps={
        "WikipediaOnlyDisposition": "Disposition to only use Wikipedia for suggestions (no DDG)"
    },
    steps=[
        "Build Wikipedia OpenSearch URL: action=opensearch, search={query}, limit={n}",
...
p(
    folder="workspace_management",
    filename="workspace_creation_process",
    ...
    who={
        "WorkspaceCreator": "Authenticated user creating the workspace",
        "WorkspaceDatabase": "PostgreSQL with workspaces and workspace_members tables",
    },
    where={
        "WorkspacesEndpoint": "API endpoint POST /api/workspaces",
    ...
    how_know={
        "WorkspaceCreate": "Pydantic model: name (1-200 chars), slug (regex: ^[a-z0-9][a-z0-9-]*[a-z0-9]$)",
    ...
    how_is={
        "SlugUniqueness": "Quality: slug must be unique across all workspaces (400 if duplicate)",
        "OwnerAutoMembership": "Quality: creator automatically added as member with owner role",
    },
    why_roles={
        "WorkspaceOwnerRole": "Role realized when user creates and owns a workspace"
    },
    why_disps={
        "TransactionalDisposition": "Disposition to create workspace + membership in single transaction"
    },
    steps=[
        "Check slug uniqueness in workspaces table",
...
p(
    folder="workspace_management",
    filename="workspace_member_invitation_process",
    ...
    who={
        "InvitingAdmin": "Admin or owner user sending the invitation",
        "InvitedUser": "User being added to the workspace (must exist by email)",
    },
    where={
        "InviteEndpoint": "API endpoint POST /api/workspaces/{id}/members/invite",
    ...
    how_know={
        "WorkspaceMemberInvite": "Pydantic model: email (3-255 chars), role (admin|member|viewer, default: member)",
        "InvitationResult": "Dict: status=invited, member_id",
    },
    how_is={
        "PermissionRequirement": "Quality: only admins and owners can invite members",
    ...
    why_disps={
        "EmailLookupDisposition": "Disposition to find user by email (404 if not registered)"
    },
    steps=[
        "Check current user role is admin or owner (403 if not)",
...
p(
    folder="workspace_management",
    filename="workspace_member_management_process",
    ...
    who={
        "WorkspaceAdmin": "Admin/owner managing workspace membership",
        "TargetMember": "Member whose role is being changed or who is being removed",
    },
    where={
        "MemberEndpoints": "DELETE /api/workspaces/{id}/members/{user_id}, PATCH /api/workspaces/{id}/members/{user_id}"
    },
    when={"ManagementDuration": "30-60ms per operation"},
    how_know={
        "RoleUpdate": "Dict: role (admin|member|viewer)",
        "RemovalResult": "Dict: status=removed",
    },
    how_is={
        "OwnerProtection": "Quality: workspace owner cannot be removed",
        "AdminRequirement": "Quality: only admins/owners can manage members",
    },
    why_roles={
        "MemberManagerRole": "Role realized when admin changes roles or removes members"
    },
    why_disps={
        "OwnerProtectionDisposition": "Disposition to prevent removal of workspace owner"
    },
    steps=[
        "Validate current user is admin or owner",
...
p(
    folder="workspace_management",
    filename="workspace_statistics_process",
    ...
    how_know={
        "WorkspaceStats": "Dict: nodes (int), edges (int), conversations (int), agents (int)"
    },
    how_is={
        "QueryEfficiency": "Quality: 4 independent COUNT queries, could be optimized to 1"
    },
    why_roles={
        "ResourceCounterRole": "Role realized when system counts workspace resources"
    },
    why_disps={
        "SimpleCOUNTDisposition": "Disposition to use simple COUNT(*) per table"
    },
    steps=[
        "Validate workspace access",
...
p(
    folder="secrets_management",
    filename="secret_creation_process",
    ...
    who={
        "SecretCreator": "Admin/owner user storing a secret",
        "FernetEncryptor": "Fernet encryption engine derived from settings.secret_key",
    },
    where={
        "SecretsEndpoint": "API endpoint POST /api/secrets",
    ...
    how_know={
        "SecretCreate": "Pydantic model: workspace_id, key (pattern ^[\\w-. ]+$), value (1-50000 chars), description, category",
    ...
    how_is={
        "EncryptionStrength": "Quality: Fernet AES-128-CBC + HMAC-SHA256 encryption",
        "CategoryInference": "Quality: auto-infer category from key name (API_KEY->api_keys, TOKEN->tokens)",
    },
    why_roles={
        "SecretKeeperRole": "Role realized when system encrypts and stores sensitive values"
    },
    why_disps={
        "FernetEncryptionDisposition": "Disposition to encrypt values using Fernet derived from SHA-256 of secret_key",
        "CategoryInferenceDisposition": "Disposition to auto-categorize based on key name patterns",
    },
    steps=[
        "Validate workspace access (admin/owner required)",
...
p(
    folder="secrets_management",
    filename="secret_resolution_process",
    ...
    who={
        "ProviderResolver": "Internal service needing decrypted API key",
    ...
    how_know={
        "DecryptedValue": "Plaintext API key after Fernet decryption",
        "SecretKeyLookup": "Query: SELECT * FROM secrets WHERE workspace_id=? AND key=?",
    },
    how_is={
        "InternalOnly": "Quality: this function is never exposed as an API endpoint",
        "NullSafety": "Quality: returns None if secret not found or decryption fails",
    },
    why_roles={
        "SecretDecryptorRole": "Role realized when system decrypts secrets for internal provider authentication"
    },
    why_disps={
        "GracefulFailureDisposition": "Disposition to return None on decryption error instead of raising"
    },
    steps=[
        "Query secrets table: WHERE workspace_id=? AND key=?",
...
p(
    folder="secrets_management",
    filename="bulk_secret_import_process",
    ...
    who={
        "EnvFileParser": "Parser extracting KEY=VALUE pairs from .env content",
        "BulkEncryptor": "Fernet encryption applied to each value",
    },
    where={"ImportEndpoint": "API endpoint POST /api/secrets/import"},
    when={
        "ImportDuration": "100ms + 30ms per secret for parsing + encryption + upsert"
    },
    how_know={
        "SecretBulkImport": "Pydantic model: workspace_id, env_content (1-500000 chars)",
    ...
    how_is={
        "QuoteHandling": "Quality: strips single and double quotes from values",
        "CommentSkipping": "Quality: skips empty lines and lines starting with #",
    },
    why_roles={
        "BulkImporterRole": "Role realized when system imports multiple secrets at once"
    },
    why_disps={
        "UpsertDisposition": "Disposition to update existing secrets or create new ones"
    },
    steps=[
        "Validate workspace access (admin/owner)",
...
p(
    folder="realtime_collaboration",
    filename="websocket_connection_process",
    ...
    who={
        "WebSocketClient": "Browser Socket.IO client connecting from frontend",
    ...
    how_know={
        "SessionData": "Dict: user_id (str), workspaces (empty set)",
        "AuthPayload": "Dict from client: {user_id: string}",
        "SessionId": "Socket.IO sid (unique per connection)",
    },
    how_is={
        "AuthenticationLevel": "Quality: currently allows all connections (debugging mode)",
        "SessionIsolation": "Quality: each connection has independent session data",
    },
    why_roles={
        "ConnectionManagerRole": "Role realized when server accepts WebSocket connections"
    },
    why_disps={
        "DebugModeDisposition": "Disposition to allow all connections without JWT validation (temporary)"
    },
    steps=[
        "Extract user_id from auth dict (or query string fallback)",
...
p(
    folder="realtime_collaboration",
    filename="workspace_presence_process",
    ...
    who={
        "PresenceTracker": "In-memory presence tracking system",
        "WorkspaceRoom": "Socket.IO room grouping users in same workspace",
    },
    where={
        "PresenceEndpoints": "Socket.IO events: join_workspace, leave_workspace, disconnect"
    },
    when={"PresenceUpdateDuration": "Less than 1ms (in-memory set operations)"},
    how_know={
        "WorkspacePresence": "Dict[str, Set[str]]: workspace_id -> set of user_ids",
    ...
    how_is={
        "RealTimeAccuracy": "Quality: presence updates immediately on join/leave/disconnect",
        "DisconnectCleanup": "Quality: user removed from all workspaces on disconnect",
    },
    why_roles={
        "PresenceManagerRole": "Role realized when system tracks active users per workspace"
    },
    why_disps={
        "BroadcastDisposition": "Disposition to broadcast user_joined/user_left events to room"
    },
    steps=[
        "Join: enter Socket.IO room workspace:{id}, add to presence dicts, broadcast user_joined",
...
p(
    folder="realtime_collaboration",
    filename="typing_indicator_process",
    ...
    who={
        "TypingUser": "User currently composing a message",
    ...
    how_know={
        "TypingEvent": "Broadcast: {user_id, conversation_id, typing: true/false}",
        "TypingData": "Input: {workspace_id, conversation_id}",
    },
    how_is={
        "SenderExclusion": "Quality: typing indicator not sent back to the typing user (skip_sid)",
        "ConversationScoping": "Quality: typing is scoped to specific conversation within workspace",
    },
    why_roles={
        "TypingNotifierRole": "Role realized when system broadcasts typing status"
    },
    why_disps={
        "SkipSenderDisposition": "Disposition to exclude sender from receiving their own typing event"
    },
    steps=[
...
p(
    folder="realtime_collaboration",
    filename="message_broadcast_process",
    ...
    who={
        "MessageSender": "User or system that created the new message",
    ...
    how_know={
        "NewMessageEvent": "Broadcast: {conversation_id, message (dict), timestamp}",
        "MessageData": "Input: {workspace_id, conversation_id, message}",
    },
    how_is={
        "IncludeSender": "Quality: sender receives their own message broadcast (no skip_sid)",
        "RealtimeDelivery": "Quality: message delivered instantly to all workspace subscribers",
    },
    why_roles={
        "MessageNotifierRole": "Role realized when system broadcasts new messages"
    },
    why_disps={
        "InclusiveBroadcastDisposition": "Disposition to include sender in broadcast (unlike typing)"
    },
    steps=[
        "Receive new_message event with workspace_id, conversation_id, message",
...
p(
    folder="realtime_collaboration",
    filename="collaborative_cursor_process",
    ...
    who={
        "CursorOwner": "User whose cursor position is being tracked",
    ...
    how_know={
        "CursorUpdate": "Broadcast: {user_id, document_id, position (dict)}",
        "Position": "Cursor coordinates: line, column, or x, y depending on editor",
    },
    how_is={
        "SenderExclusion": "Quality: cursor position not sent back to cursor owner (skip_sid)",
        "DocumentScoping": "Quality: cursor updates scoped to specific document_id",
    },
    why_roles={
        "CursorBroadcasterRole": "Role realized when system shares cursor positions"
    },
    why_disps={
        "PositionTrackingDisposition": "Disposition to broadcast cursor coordinates in real-time"
    },
    steps=[
        "Get user_id from session",
...
p(
    folder="model_registry",
    filename="model_discovery_process",
    ...
    who={
        "ModelRegistry": "In-memory Python dict with MODEL_REGISTRY[provider] = list[ModelInfo]"
    },
    where={
        "RegistryModule": "apps/api/app/services/model_registry.py (in-memory, no database)"
    },
    when={"DiscoveryLatency": "Less than 1ms (in-memory dict lookup)"},
    how_know={
        "ModelInfo": "Dict: id, name, provider, context_window, supports_streaming, supports_vision, supports_function_calling, max_output_tokens, released",
    ...
    how_is={
        "CatalogCompleteness": "Quality: 60+ models manually cataloged across 9 providers",
        "ManualMaintenance": "Quality: registry requires manual updates for new models",
    },
    why_roles={
        "ModelCatalogRole": "Role realized when system provides model information for UI and routing"
    },
    why_disps={
        "StaticRegistryDisposition": "Disposition to use static in-memory registry (not auto-discovered)"
    },
    steps=[
        "If provider specified: return MODEL_REGISTRY[provider] list",
...
p(
    folder="model_registry",
    filename="provider_availability_check_process",
    ...
    impl="apps/api/app/api/endpoints/providers.py:66-212 list_available_providers()",
    who={
        "AvailabilityChecker": "Backend logic scanning secrets and environment for provider keys",
    ...
    how_know={
        "Provider": "Response object: id, name, type, has_api_key (bool), models list",
        "APIKeyPatterns": "Secret key patterns checked: OPENAI_API_KEY, ANTHROPIC_API_KEY, XAI_API_KEY, MISTRAL_API_KEY, etc.",
    },
    how_is={
        "FallbackChain": "Quality: checks secrets first, then environment variables",
        "OllamaInclusion": "Quality: Ollama always included (no API key needed)",
    },
    why_roles={
        "AvailabilityReporterRole": "Role realized when system reports which providers are usable"
    },
    why_disps={
        "EnvironmentFallbackDisposition": "Disposition to check env vars when no database secret found"
    },
    steps=[
        "Get user workspace IDs from workspace_members",
...
p(
    folder="frontend_state",
    filename="auth_state_process",
    ...
    who={
        "ZustandAuthStore": "Zustand store managing auth state in browser",
        "LocalStorage": "Browser localStorage persisting auth tokens",
    },
    where={"AuthStorageKey": "localStorage key: nexus-auth"},
    when={
        "LoginDuration": "200-1000ms for API call + state update + localStorage write"
    },
    how_know={
        "AuthState": "State: user (User|null), token (string|null), isAuthenticated (bool), isLoading, error",
    ...
    how_is={
        "TokenPersistence": "Quality: token persisted in localStorage, survives page refresh",
        "AutoLogoutOn401": "Quality: any 401 response triggers automatic logout + redirect",
    },
    why_roles={
        "SessionManagerRole": "Role realized when frontend manages user session lifecycle"
    },
    why_disps={
        "Auto401DetectionDisposition": "Disposition to intercept 401 responses and force re-authentication"
    },
    steps=[
        "Login: POST /api/auth/login, store user+token in state and localStorage",
...
p(
    folder="frontend_state",
    filename="workspace_state_process",
    ...
    who={
        "ZustandWorkspaceStore": "Zustand store managing workspace state",
    ...
    when={
        "StreamingThrottle": "1000ms batching interval for localStorage writes during streaming"
    },
    how_know={
        "WorkspaceState": "State: conversations, activeConversationId, selectedAgent, workspaces, currentWorkspaceId, projects, branches",
    ...
    how_is={
        "ThrottlePerformance": "Quality: prevents JSON.stringify + localStorage.setItem on every streaming token",
        "WorkspaceIsolation": "Quality: conversations filtered by currentWorkspaceId",
    },
    why_roles={
        "StateOrchestratorRole": "Role realized when store coordinates navigation, chat, and workspace data"
    },
    why_disps={
        "ThrottledPersistDisposition": "Disposition to batch localStorage writes during high-frequency updates"
    },
    steps=[
        "On mount: fetchWorkspaces() from /api/workspaces/",
...
p(
    ...
    how_know={
        "Agent": "Interface: id, name, description, icon, systemPrompt, provider, modelId, logoUrl, tools, capabilities",
        "AgentActions": "fetchAgents(workspaceId), addAgent(), updateAgent(), deleteAgent(), syncAgentsFromProviders()",
    },
    how_is={
        "WorkspaceScopedFetch": "Quality: agents fetched per workspace, filtered server-side",
        "SelectedAgentValidation": "Quality: resets selectedAgent if current selection invalid after fetch",
    },
    why_roles={
        "AgentCatalogRole": "Role realized when store provides agent list for UI selection"
    },
    why_disps={
        "AutoResetDisposition": "Disposition to reset selected agent to first available if current becomes invalid"
    },
    steps=[
        "On workspace change: fetchAgents(workspaceId) from /api/agents/?workspace_id={id}",
...
p(
    folder="frontend_state",
    filename="knowledge_graph_state_process",
    ...
    how_know={
        "GraphState": "State: graphs, nodes, edges, selectedNodeId, selectedEdgeId, views, activeViewType, queries, zoomLevel",
    ...
    how_is={
        "PositionPersistence": "Quality: node x,y positions persisted across sessions",
        "MultiGraphSupport": "Quality: supports multiple named graphs with visibility toggling",
    },
    why_roles={
        "GraphVisualizerRole": "Role realized when store manages graph display and interaction"
    },
    why_disps={
        "BFOColorSchemeDisposition": "Disposition to color nodes by BFO 7 Buckets category"
    },
    steps=[
        "Load graph from API: GET /api/graph/workspaces/{workspace_id}",
...
p(
    ...
    where={
        "FileBrowserUI": "Sidebar files section + lab section",
        "FileAPI": "Backend endpoints /api/files/*",
    },
    when={
        "FileListDuration": "50-500ms for directory listing from API",
        "FileReadDuration": "50-2000ms depending on file size",
    },
    how_know={
        "FilesState": "State: files, currentPath, openFiles, activeFile, fileContents, unsavedChanges, storageSources",
    ...
    how_is={
        "DualStorageSupport": "Quality: supports both local filesystem and cloud (S3) storage",
    ...
    why_roles={
        "FileManagerRole": "Role realized when store manages file browsing, editing, and storage"
    },
    why_disps={
        "LazyContentLoadDisposition": "Disposition to load file contents only when file is opened (not on list)"
    },
    steps=[
        "fetchFiles(path): GET /api/files/?path={path}",
...
p(
    folder="abi_server_config",
    filename="abi_server_crud_process",
    ...
    who={
        "ABIServerManager": "Backend logic managing ABI server configurations",
        "WorkspaceAdmin": "Admin/owner user configuring ABI servers",
    },
    where={
        "ABIEndpoints": "API endpoints /api/abi/workspaces/{id}/abi-servers/*",
        "ABIServersTable": "PostgreSQL abi_servers table with workspace_id FK",
    },
    when={"CRUDDuration": "30-60ms per operation"},
    how_know={
        "ABIServerCreate": "Pydantic model: name (1-255), endpoint (URL), api_key (optional)",
    ...
    how_is={
        "EndpointUniqueness": "Quality: unique(workspace_id, endpoint) prevents duplicate server configs",
        "APIKeyStorage": "Quality: api_key stored in plaintext (TODO: encrypt with Fernet)",
    },
    why_roles={
        "ServerConfiguratorRole": "Role realized when admin configures external ABI server connections"
    },
    why_disps={
        "CascadeDeleteDisposition": "Disposition to cascade delete when workspace is deleted"
    },
    steps=[
        "List: SELECT FROM abi_servers WHERE workspace_id=?",
...
p(
    folder="background_cleanup",
    filename="token_cleanup_process",
    ...
    who={
        "CleanupScheduler": "Background task executor (not yet scheduled, manual call)"
    },
    where={
        "RefreshTokensTable": "PostgreSQL refresh_tokens table",
        "RevokedTokensTable": "PostgreSQL revoked_tokens table",
    },
    when={
        "CleanupDuration": "50-200ms for 2 DELETE queries",
        "ScheduleFrequency": "Intended: daily (not yet implemented)",
    },
    how_know={
        "ExpiredRefreshTokens": "Rows WHERE expires_at < NOW() in refresh_tokens",
        "ExpiredRevokedTokens": "Rows WHERE expires_at < NOW() in revoked_tokens",
        "DeletedCount": "Number of rows removed per table",
    },
    how_is={
        "TableGrowthPrevention": "Quality: prevents unbounded growth of token tables",
        "NotYetScheduled": "Quality: requires manual invocation or future cron job",
    },
    why_roles={
        "DatabaseJanitorRole": "Role realized when system cleans expired security tokens"
    },
    why_disps={
        "ExpirationBasedDisposition": "Disposition to delete based on expires_at timestamp comparison"
    },
    steps=[
...
p(
    folder="background_cleanup",
    filename="rate_limit_cleanup_process",
    ...
    when={
        "CleanupDuration": "50-200ms for single DELETE query",
        "RetentionWindow": "24 hours of rate limit history retained",
    },
    how_know={
        "CutoffTimestamp": "datetime: NOW() - 24 hours",
        "DeletedEvents": "Rows WHERE created_at < cutoff",
    },
    how_is={
        "RetentionPeriod": "Quality: 24-hour retention window for rate limit forensics",
        "NotYetScheduled": "Quality: requires manual invocation or cron job setup",
    },
    why_roles={
        "EventCleanerRole": "Role realized when system purges old rate limit records"
    },
    why_disps={
        "TimeBasedPurgeDisposition": "Disposition to delete events older than 24-hour threshold"
    },
    steps=[
