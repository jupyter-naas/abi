...
import { type GraphSelection } from '@/lib/api/graph';
...
import { GraphNetworkView } from './_components/GraphNetworkView';
import type {
  GraphEdge,
  GraphNode,
  GraphOption,
...
export default function GraphPage() {
  ...
  // Selection state — shared between the network canvas, the table view, and
  // the node inspector. The canvas-local UI state (search / buckets / hidden
  // nodes / parents hierarchy / etc.) lives inside `<GraphNetworkView>`.
  ...
  // Reset selection when the active graph identity changes. Canvas-local
  // filters (search / buckets / parents / display limit / relations toggle)
  // reset themselves inside <GraphNetworkView> on the same trigger.
  useEffect(() => {
    setSelectedNodeId(null);
