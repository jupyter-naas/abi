 ...
 * history (see the backend's `_semver_from_commits`). A clickable header
 * like Suggestions/Files that expands in place to the commit log behind
 * that version — always the *last* block in the stack, directly touching
 * the composer input box. Picks up the top border/radius itself via
 * `first:` only when Suggestions/Files are both absent (e.g. brand-new
 * deck, no suggestions for the agent).
 */
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        if (!res.ok || cancelled) return;
        const body = (await res.json()) as DeckCommit[];
