...
import './slides-project-menu.css';

/**
 * Same overflow as a chat conversation row: hover kebab, Rename, Archive.
 * Pin/Delete stay off; decks are git projects, not chat threads.
 *
 ...
 * on the slides pane and the status footer. Portal + fixed + flip so last-row
...
export type SlidesProjectMenuPlacement = {
  top: number;
  left: number;
  placement: 'above' | 'below';
};

export function placeSlidesProjectMenu(
...
function SlidesProjectMenuPanel({
  onRename,
  onArchive,
  onOpenChange,
  pos,
  ...
  pos: SlidesProjectMenuPlacement | null;
  menuRef?: Ref<HTMLDivElement>;
}) {
  return (
    <>
      <div
        className="chat-context-menu-backdrop slides-project-menu-backdrop"
        onClick={(event) => {
      ...
      <div
        ref={menuRef}
        className={`chat-context-menu slides-project-menu-panel${pos ? '' : ' is-measuring'}`}
        data-testid="slides-project-menu-panel"
...
export function SlidesProjectOverflowMenu({
  open,
  onOpenChange,
  onRename,
  onArchive,
  ...
  const [pos, setPos] = useState<SlidesProjectMenuPlacement | null>(null);
  ...
  return (
    <div className="chat-list-row-wrap" data-testid="slides-project-menu">
      <div
        ref={triggerRef}
