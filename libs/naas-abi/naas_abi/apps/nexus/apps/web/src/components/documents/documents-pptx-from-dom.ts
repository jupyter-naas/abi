/**
 * HTML is the live Documents source. PDF is a 816px prose reconstruction of the
 * current `.section` DOM (pdfgenjs). Preview is the HTML stage.
 *
 * pdfgenjs cannot match CSS pixel-for-pixel (web fonts, wrapping,
 * letter-spacing). Export must still carry the same section count, cover h1,
 * body copy, layout regions, and theme colors.
 */

export const SLIDES_PDF_FROM_DOM_SCRIPT_ID = 'nexus-documents-pdf-from-dom';
export const SLIDES_PDF_FROM_DOM_FINGERPRINT = 'NEXUS_DOCUMENTS_PDF_FROM_DOM_V1';

export const SLIDES_PDF_STAGE_WIDTH_PX = 1280;
export const SLIDES_PDF_STAGE_HEIGHT_PX = 720;
export const SLIDES_PDF_DPI = 96;
export const SLIDES_PDF_WIDTH_IN = 13.333;
export const SLIDES_PDF_HEIGHT_IN = 7.5;

export type SectionsPptxSectionKind =
  | 'cover'
  | 'divider'
  | 'agenda'
  | 'cards'
...
export type SectionsPptxPlanSection = {
  kind: SectionsPptxSectionKind;
  eyebrow?: string;
  title?: string;
  subtitle?: string;
  hook?: string;
...
export type SectionsPptxPlan = {
  ...
  sectionCount: number;
  sections: SectionsPptxPlanSection[];
};
...
function classify(section: string): SectionsPptxSectionKind {
...
function planSection(section: string): SectionsPptxPlanSection {
...
/** Derive the PDF reconstruction plan from document HTML (no pdfgenjs). */
export function planSectionsPptxFromHtml(html: string): SectionsPptxPlan {
  const vars = cssVars(html);
  const sectionBlocks = html.match(SECTION_RE) ?? [];
  const sections = sectionBlocks.map(planSection);
  const cover = sections.find((s) => s.kind === 'cover') ?? sections[0];
  return {
    stage: {
      widthPx: SLIDES_PDF_STAGE_WIDTH_PX,
      heightPx: SLIDES_PDF_STAGE_HEIGHT_PX,
      widthIn: SLIDES_PDF_WIDTH_IN,
      heightIn: SLIDES_PDF_HEIGHT_IN,
      dpi: SLIDES_PDF_DPI,
    },
    colors: {
...
/**
 * Classic-script walker installed as window.buildPptx. Reads live .section
...
async function buildPptx() {
  ...
  for (var i = 0; i < sections.length; i++) {
    ...
    if (agendaRows.length) {
      for (var ar = 0; ar < agendaRows.length; ar++) {
        ...
        var y = 140 + ar * 90;
        s.addShape(pdf.ShapeType.rect, { x: I(56), y: I(y), w: I(1168), h: I(76), fill: { color: C.card } });
    ...
    if (stepNodes.length) {
      for (var st = 0; st < stepNodes.length; st++) {
        ...
        var sx = 56 + col * 596;
        var sy = 140 + srow * 220;
        s.addShape(pdf.ShapeType.rect, { x: I(sx), y: I(sy), w: I(576), h: I(200), fill: { color: C.card } });
        s.addShape(pdf.ShapeType.rect, { x: I(sx), y: I(sy), w: I(6), h: I(200), fill: { color: C.accent } });
    ...
    if (cards.length) {
      ...
      for (var ci = 0; ci < n; ci++) {
        ...
        s.addShape(pdf.ShapeType.rect, { x: I(cx), y: I(cy), w: I(cw), h: I(ch), fill: { color: C.card } });
  ...
  var title = coverH1 || textOf(document.querySelector("main.document h1, .document h1")) || "Presentation";
  pdf.title = title;
  var safe = title.replace(/[^\\w]+/g, "_").replace(/^_|_$/g, "") || "Presentation";
  return pdf.writeFile({ fileName: safe + ".pdf" });
}`;

export const SLIDES_PDF_FROM_DOM_SCRIPT = `<script id="${SLIDES_PDF_FROM_DOM_SCRIPT_ID}">
(function () {
