...
export function invalidateSlidesCover(workspaceId: string, slug?: string | null) {
...
export function SlidesCoverFallback({
  title,
  preview,
}: {
  title: string;
  preview: SlidesTemplatePreview;
}) {
  return (
    <div
      className="absolute inset-0"
      style={{ background: preview.preview_bg }}
      data-testid="slides-cover-fallback"
    >
      <div
...
export function SlidesCoverThumb({
  workspaceId,
  slug,
  title,
  preview,
  ...
  return (
    <div
      ...
      data-testid="slides-cover-thumb"
    >
      {!coverHtml ? <SlidesCoverFallback title={title} preview={preview} /> : null}
      {coverHtml ? (
        <iframe
          title={`${title} cover`}
          sandbox=""
