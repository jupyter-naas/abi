...
export default function OrganizationBillingPage() {
  ...
  useEffect(() => {
    const fetchBillingInfo = async () => {
      ...
      try {
        ...
        const response = await authFetch(`/api/organizations/${org.id}/billing`);
  ...
  const handleUpgrade = async (planId: string) => {
    ...
    try {
      const { authFetch } = await import('@/stores/auth');
      const response = await authFetch(`/api/organizations/${org.id}/billing/upgrade`, {
        method: 'POST',
  ...
  const handleAddPaymentMethod = async () => {
    ...
    try {
      const { authFetch } = await import('@/stores/auth');
      const response = await authFetch(`/api/organizations/${org.id}/billing/payment-method`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
