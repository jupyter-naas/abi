/**
 * The deck card has to open the deck inside the app shell.
 *
 * A forced new window (target="_blank" or window.open) loses the workspace
 * layout and the chat panel beside the deck, so the card must stay a plain
...
import { SlidesDeckCardView } from './slides-deck-card-view';
...
function markup(): string {
  return renderToStaticMarkup(
    createElement(SlidesDeckCardView, {
      card: {
...
describe('SlidesDeckCardView', () => {
  it('links into the app for the deck workspace', () => {
    expect(markup()).toContain('href="/workspace/ws-1/slides/materiaux-de-construction"');
  });
