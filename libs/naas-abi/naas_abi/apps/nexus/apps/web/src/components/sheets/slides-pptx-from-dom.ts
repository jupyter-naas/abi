/**
 * HTML is the live Sheets source. PPTX is a 1280x720 reconstruction of the
...
export const SHEETS_PPTX_FROM_DOM_SCRIPT_ID = 'nexus-sheets-pptx-from-dom';
export const SHEETS_PPTX_FROM_DOM_FINGERPRINT = 'NEXUS_SHEETS_PPTX_FROM_DOM_V1';

export const SHEETS_PPTX_STAGE_WIDTH_PX = 1280;
export const SHEETS_PPTX_STAGE_HEIGHT_PX = 720;
export const SHEETS_PPTX_DPI = 96;
export const SHEETS_PPTX_WIDTH_IN = 13.333;
export const SHEETS_PPTX_HEIGHT_IN = 7.5;

export type SheetsPptxSlideKind =
  | 'cover'
  | 'divider'
  | 'agenda'
  | 'cards'
...
export type SheetsPptxPlanSlide = {
  kind: SheetsPptxSlideKind;
  eyebrow?: string;
  title?: string;
  subtitle?: string;
  hook?: string;
...
function classify(section: string): SheetsPptxSlideKind {
...
function planSlide(section: string): SheetsPptxPlanSlide {
...
/** Derive the PPTX reconstruction plan from workbook HTML (no pptxgenjs). */
export function planSheetsPptxFromHtml(html: string): SheetsPptxPlan {
  const vars = cssVars(html);
  const sections = html.match(SECTION_RE) ?? [];
  const sheets = sections.map(planSlide);
  const cover = sheets.find((s) => s.kind === 'cover') ?? sheets[0];
  return {
    stage: {
      widthPx: SHEETS_PPTX_STAGE_WIDTH_PX,
      heightPx: SHEETS_PPTX_STAGE_HEIGHT_PX,
      widthIn: SHEETS_PPTX_WIDTH_IN,
      heightIn: SHEETS_PPTX_HEIGHT_IN,
      dpi: SHEETS_PPTX_DPI,
    },
    colors: {
 ...
 */
export const SHEETS_PPTX_FROM_DOM_FN = `/* ${SHEETS_PPTX_FROM_DOM_FINGERPRINT} */
async function buildPptx() {
  if (typeof PptxGenJS === "undefined") throw new Error("PptxGenJS failed to load");
  var sheets = document.querySelectorAll("main.workbook > section.slide, .workbook > section.slide");
  if (!sheets.length) throw new Error("No .slide sections to export");
  ...
  var title = coverH1 || textOf(document.querySelector("main.workbook h1, .workbook h1")) || "Workbook";
  pptx.title = title;
  var safe = title.replace(/[^\\w]+/g, "_").replace(/^_|_$/g, "") || "Workbook";
  return pptx.writeFile({ fileName: safe + ".pptx" });
}`;

export const SHEETS_PPTX_FROM_DOM_SCRIPT = `<script id="${SHEETS_PPTX_FROM_DOM_SCRIPT_ID}">
(function () {
