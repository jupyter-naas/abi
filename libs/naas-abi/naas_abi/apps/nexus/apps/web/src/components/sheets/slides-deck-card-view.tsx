...
import { Workbook } from 'lucide-react';
import { sheetsWorkbookHref, type SheetsWorkbookCard } from './sheets-workbook-card';
  ...
  return (
    ...
    >
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-background">
        <Workbook className="h-5 w-5 text-muted-foreground" />
      </span>
      <span className="min-w-0 flex-1">
        ...
        <span className="block text-xs text-muted-foreground">
          Workbook. Open in Sheets to view and export PPTX.
        </span>
      </span>
    </Link>
  );
