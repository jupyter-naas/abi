export type SheetsTemplateSlide = {
  index: number;
  id?: string | null;
  eyebrow: string;
  title: string;
...
export type SheetsTemplateAsset = {
  name: string;
  kind: string;
};

export type SheetsSeedTemplate = {
  id: string;
  ...
  preview_ink: string;
  sheets: SheetsTemplateSlide[];
  assets: SheetsTemplateAsset[];
};
 ...
 * a workbook created before namespaces existed still carries. No default source
 * name here: the set of sources is configuration, and guessing one would put
 * a wrong label on a real template.
 */
export function templateNamespace(template: Pick<SheetsSeedTemplate, 'id' | 'source'>): string {
...
export function templateNamespacesAreAmbiguous(
  templates: Pick<SheetsSeedTemplate, 'id' | 'source'>[],
): boolean {
...
export type SheetsTemplateMenuRow =
  | { kind: 'heading'; id: string; label: string }
  | { kind: 'template'; id: string; label: string; swatch?: string };

export const SHEETS_HOME_BLANK_TEMPLATE_ID = 'abi/grid-light-v1';

export type SheetsHomeTemplateCard = {
  id: string;
  label: string;
  blank?: boolean;
};
 ...
 */
export function sheetsHomeTemplateCards(
  templates: Array<Pick<SheetsSeedTemplate, 'id' | 'name'>>,
): SheetsHomeTemplateCard[] {
  const seen = new Set<string>([
    SHEETS_HOME_BLANK_TEMPLATE_ID,
    templateStem(SHEETS_HOME_BLANK_TEMPLATE_ID),
  ]);
  const cards: SheetsHomeTemplateCard[] = [
    { id: SHEETS_HOME_BLANK_TEMPLATE_ID, label: 'Blank', blank: true },
  ];
...
/**
 * Rows for the New Sheets picker: human names only.
 *
 ...
 * id (`acme/industry-v2`).
 */
export function sheetsTemplateMenuRows(
  templates: Array<
    Pick<SheetsSeedTemplate, 'id' | 'source' | 'name'> &
      Partial<Pick<SheetsSeedTemplate, 'preview_accent' | 'preview_bg'>>
  >,
): SheetsTemplateMenuRow[] {
  ...
  const rows: SheetsTemplateMenuRow[] = [];
...
export type SheetsTemplatePreview = Pick<
  SheetsSeedTemplate,
  'preview_bg' | 'preview_panel' | 'preview_accent' | 'preview_ink'
>;

export const DEFAULT_TEMPLATE_PREVIEW: SheetsTemplatePreview = {
  preview_bg: '#f4f4f4',
...
export function templatePreviewColors(
  ...
  templates: Array<Pick<SheetsSeedTemplate, 'id'> & Partial<SheetsTemplatePreview>>,
): SheetsTemplatePreview {
