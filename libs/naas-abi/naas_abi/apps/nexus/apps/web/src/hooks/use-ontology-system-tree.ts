...
export function usePublishOntologySystemTree(enabled: boolean, projection: SystemTreeProjection) {
  ...
  const { nodes, edges, selectedNodeId, onSelect, onOpen, canOpen, openLabel } = projection;
  useEffect(() => {
    if (!enabled) return;
    const owner = {};
    useOntologySystemTreeStore.getState().publish({ scope, owner, nodes, edges, selectedNodeId, onSelect, onOpen, canOpen, openLabel });
    return () => useOntologySystemTreeStore.getState().release(owner);
  }, [enabled, scope, nodes, edges, selectedNodeId, onSelect, onOpen, canOpen, openLabel]);
}
