"""Call-site coverage for the sections model override in the streaming adapter.

The policy unit tests in ``documents/policy_test.py`` already pass, so they cannot
...
import pytest
from naas_abi.agents.documents import configured_documents_model
...
def _reset_sections_context():
    tokens = [
        (documents_active_slug, documents_active_slug.set("")),
        (documents_creation_intent, documents_creation_intent.set(False)),
        (documents_research_required, documents_research_required.set(False)),
        (documents_research_queries, documents_research_queries.set(None)),
    ]
    yield
...
@pytest.mark.asyncio
async def test_french_document_request_from_main_chat_upgrades_the_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The turn that writes the whole document must not run on a mini model.

    Creating a document from the main chat is a supported flow, so no document is open
    ...
    capture = await _run_turn(
        monkeypatch,
        "Fais-moi une document sur la souveraineté numérique européenne",
    )
    assert capture.llm_model == configured_documents_model()


@pytest.mark.asyncio
async def test_english_document_request_from_main_chat_upgrades_the_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    capture = await _run_turn(
        monkeypatch,
        "create a document on the latest developments in EU chip policy",
    )
    assert capture.llm_model == configured_documents_model()


@pytest.mark.asyncio
async def test_open_document_still_upgrades_the_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    capture = await _run_turn(
        monkeypatch,
...
async def test_persisted_metadata_reports_the_model_actually_used(
    ...
    document is really being written by the sections model, which is how the
    downgrade stayed invisible.
    """
    capture = await _run_turn(
        monkeypatch,
        "Fais-moi un document sur la situation au Sahel",
    )
    assert capture.metadata is not None
    assert capture.metadata["llm_model"] == configured_documents_model()
...
async def test_opening_frame_announces_the_model_to_the_client(
    ...
    capture = await _run_turn(
        monkeypatch,
        "Fais-moi un document sur la souverainet\u00e9 num\u00e9rique",
    )
    ...
    assert opening.get("llm_model") == configured_documents_model()
