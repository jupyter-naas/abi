...
async def _get_user_by_id(user_id: str) -> UserModel | None:
    ...
    async with async_engine.begin() as conn:
        result = await conn.execute(
            text("SELECT id, email, name, hashed_password, avatar, created_at FROM users WHERE id = :id"),
            {"id": user_id}
        )
        row = result.fetchone()
        if not row:
        ...
        user = UserModel(
            ...
            hashed_password=row.hashed_password, avatar=row.avatar,
            created_at=row.created_at,
        )
        return user
    ...
    async with async_engine.connect() as conn:
        ...
        if not row:
            owner_result = await conn.execute(
                text("SELECT owner_id FROM workspaces WHERE id = :wid"),
                {"wid": workspace_id}
            )
...
async def login(
    ...
    if not user_row:
        await log_login(None, success=False, request=request, details={"reason": "user_not_found", "email": email})
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if not verify_password(credentials.password, user_row.hashed_password):
        await log_login(user_row.id, success=False, request=request, details={"reason": "invalid_password"})
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid email or password",
        )
...
async def change_password(
    ...
    return {"status": "password_changed", "message": "All sessions have been invalidated. Please log in again."}
