"""Documents FastAPI primary adapter.

Business-facing documents stored in Forgejo under a workspace namespace
(branch ``documents/<workspace_id>/<slug>``, path
``documents/<workspace_id>/<slug>/document.html``). Legacy ``documents/<slug>`` documents
remain available only when ``project.json.workspace_id`` matches (unscoped
legacy documents require a verified workspace_id owner). A Coder ``abi-documents`` workspace is
...
from naas_abi.apps.nexus.apps.api.app.core.config import (
    ABI_DOCUMENTS_TEMPLATE_NAMESPACE,
    settings,
)
...
_DEFAULT_TEMPLATE = "article-light-v1"
_ABI_NAMESPACE = ABI_DOCUMENTS_TEMPLATE_NAMESPACE
_ABI_PACKAGE = "naas_abi.apps.nexus.assets.documents.templates"
...
# and the document view shows a permanent "Coder runtime unavailable" banner.
_DOCUMENTS_TEMPLATE_NAMES = ("abi-documents", "abi-code-server", "local-directory")
...
# One provision at a time per document: create starts this in the
...
def _get_source_control(request: Request) -> SourceControlService:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Forgejo is not configured. Documents needs git storage.",
        ) from exc
...
_FORGEJO_NOT_CONFIGURED = "Forgejo is not configured. Documents needs git storage."
_FORGEJO_UNREACHABLE = "Forgejo is not reachable. Documents needs git storage."
...
def _conventional_message(message: str, *, default_type: str = "chore") -> str:
    """Coerce a commit message into Conventional Commits for a real document history."""
...
# this reflects at most the most recent 250 commits. A document with more history
...
def _versions_from_commits(commits: list[Commit]) -> dict[str, str]:
    ...
    A document never has a "stable public API" milestone the way a library does,
...
def _semver_from_commits(commits: list[Commit]) -> str:
    """The document's current version: the newest commit's cumulative semver."""
    if not commits:
        return "0.0.0"
...
def _sections_sc(request: Request) -> tuple[SourceControlService, str]:
...
def _document_path(workspace_id: str, slug: str) -> str:
    return f"documents/{_workspace_segment(workspace_id)}/{slug}/document.html"
...
def _project_path(workspace_id: str, slug: str) -> str:
    return f"documents/{_workspace_segment(workspace_id)}/{slug}/project.json"
...
def _assets_dir(workspace_id: str, slug: str) -> str:
    return f"documents/{_workspace_segment(workspace_id)}/{slug}/assets"


def _legacy_assets_dir(slug: str) -> str:
    return f"documents/{slug}/assets"
...
def _paths_for(workspace_id: str, slug: str, *, legacy: bool = False) -> dict[str, str]:
    ...
    return {
        ...
        "document_path": _document_path(workspace_id, slug),
...
def _resolve_project_paths(
    ...
    *,
    ...
    """Namespaced or legacy paths when the document belongs to workspace_id."""
...
def _claim_workspace_in_meta(
    ...
    *,
    ...
    sc.upsert_file(
        ...
        message=f"chore(sections): claim {slug} for workspace",
        branch=paths["branch"],
...
Images for this document live here. ``document.html`` references them as relative
``assets/<file>`` paths.

## Seed

New documents and Apply template copy the HTML plus any ``assets/``
folder from the template catalog. Preview resolves those paths through the
sections asset route. File, Export HTML inlines them again so the download
is one file.
...
def _assets_note(*, copied: int, embedded: int) -> str | None:
    ...
    if embedded:
        return "assets/ seeded empty; template images remain as data-URLs in document.html"
    return None
...
def _seed_file_writes(
    *,
    ...
    writes.append(FileWrite(path=paths["document_path"], content=seed))
    if assets:
...
class _TemplateSource(NamedTuple):
    """One namespaced tree of seed documents.
...
def _abi_template_dirs() -> list[Path]:
    ...
    dirs = [
        here.parents[7] / "assets" / "sections" / "templates",
        Path("/app/assets/documents/templates"),
        Path("assets/documents/templates"),
    ]
    try:
...
def _template_sources() -> list[_TemplateSource]:
    """ABI's own seeds, then whatever ``documents_template_sources`` adds.
    ...
    for entry in getattr(settings, "documents_template_sources", None) or []:
        ...
        if not usable:
            ...
            logger.warning(
                "Documents template source '%s' is not a directory: %s",
                entry.namespace,
                directory,
            )
            continue
...
def _seed_template_meta(template_id: str) -> dict[str, str]:
    ...
    return {
        ...
        "origin": source.origin,
        "name": title,
        "description": f"Document seed ({stem})",
        "preview_bg": "#f4f4f4",
...
def _list_seed_template_records() -> list[dict]:
    ...
    for tid in _discover_seed_ids():
        ...
        try:
            ...
            seed = ""
        meta["sections"] = _parse_section_outline(seed) if seed else []
        file_assets = [
...
def _load_seed_html(template_id: str = _DEFAULT_TEMPLATE_ID) -> str:
    ...
    if namespace and source is None:
        raise HTTPException(
            status_code=404,
            detail=f"Unknown sections template '{template_id}'.",
        )
...
def _load_seed_bundle(
    ...
    document until someone re-imports the template.
    """
...
class DocumentUpdateRequest(BaseModel):
    ...
    message: str = Field(default="chore(document): update sections document", max_length=200)
    template_id: str | None = Field(default=None, max_length=_TEMPLATE_ID_MAX_LEN)
...
class DocumentDiffFileResponse(BaseModel):
    path: str
    status: str
    additions: int
    deletions: int
...
class DocumentDiffResponse(BaseModel):
    base: str
    head: str
    files: list[DocumentDiffFileResponse]
...
class RuntimeResponse(BaseModel):
    ...
    # Coder dashboard URL (https://coder…/@owner/documents-<slug>) when bound.
    coder_ui_url: str | None = None


def _runtime_label(workspace_id: str, slug: str) -> str:
    return f"documents/{_workspace_segment(workspace_id)}/{slug}"
...
def _legacy_coder_workspace_name(slug: str) -> str:
    return f"sections-{slug}"[:32].rstrip("-")
...
def _read_document_via_sidecar(
    base: str | None, secret: str | None, *, document_path: str
) -> str | None:
    """Return document HTML from the Coder sidecar, or None when unavailable."""
    if not _probe_sidecar(base, secret):
        return None
    result = _sidecar_tool_call(
        base, secret, "read_file", {"path": document_path}, timeout_s=10.0
    )
...
def _write_document_via_sidecar(
    base: str | None, secret: str | None, *, document_path: str, html: str
) -> bool:
    """Best-effort write of document HTML into the Coder workspace via sidecar."""
    if not base or not secret:
        return False
    if not _probe_sidecar(base, secret):
        return False
    result = _sidecar_tool_call(
        ...
        secret,
        "write_file",
        {"path": document_path, "content": html},
        timeout_s=20.0,
    )
...
def _friendly_git_detail(exc: BaseException) -> str:
    ...
    if _is_git_write_race(exc):
        return "Git write raced on the document branch; retrying is safe"
    if _is_forgejo_unreachable(exc):
...
def _git_clone_url(
    ...
    """Clone URL the sections sidecar should use for this repo.
    ...
    def _list() -> list[ProjectResponse]:
        ...
        for branch in branches:
            ...
            if legacy and owner != workspace_id:
                # Unowned/unscoped legacy documents are not listed or claimable here.
                continue
...
async def create_project(
    ...
    def _create() -> ProjectResponse:
        ...
        # Forgejo user repo-wide write as a stand-in for per-document authorization.
        sc.ensure_user(
        ...
        # incomplete (branch exists, document.html missing), finish seeding.
        if adopted_branch:
            try:
                existing_document = sc.get_file(
                    repo_id=repo_id, path=paths["document_path"], ref=branch
                )
                if existing_document.text:
                    raise BranchNameConflictError(
                        f"Documents project '{slug}' already exists"
                    )
    ...
    try:
        ...
        logger.info(
            "sections create seed %.3fs slug=%s",
...
async def update_project(
    ...
    """Rename (title) and/or archive a document. Slug and git folder stay put."""
    await require_workspace_access(current_user.id, body.workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    ...
    def _update() -> ProjectResponse:
        ...
        sc.upsert_file(
            ...
            path=paths["project_path"],
            content=json.dumps(meta, indent=2) + "\n",
            message=f"chore(sections): update {slug} project settings",
            branch=paths["branch"],
            author_name=author_name,
            author_email=author_email,
        )
...
@router.get("/projects/{slug}/document", response_model=DocumentResponse)
async def get_document(
    ...
    """Load document HTML for Preview/Code.

    Prefer the Coder sidecar (live editing SoT) when the sections runtime is
    ...
    def _get_sidecar() -> str | None:
        return _read_document_via_sidecar(
            sidecar_base, sidecar_secret, document_path=paths["document_path"]
        )
    ...
    try:
        ...
        def _get_forgejo() -> DocumentResponse:
            file = sc.get_file(
                repo_id=repo_id, path=paths["document_path"], ref=paths["branch"]
            )
            if file.is_binary or file.text is None:
                raise ValidationError("Document is not UTF-8 text")
            return DocumentResponse(
                slug=slug,
                path=paths["document_path"],
                html=file.text,
...
async def get_project_asset(
    ...
    """Serve a file from the document ``assets/`` folder (preview / export)."""
    await require_workspace_access(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    ...
    def _get() -> Response:
        ...
        if payload is None:
            raise RepoNotFoundError(f"sections asset {filename}")
...
@router.put("/projects/{slug}/document", response_model=DocumentResponse)
async def put_document(
    ...
    """Save document: commit Forgejo snapshot and dual-write Coder sidecar when ready."""
    await require_workspace_access(current_user.id, body.workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    sc, repo_id = _sections_sc(request)
        ...
        try:
            ...
            if meta_file.text:
                ...
                sc.upsert_file(
                    ...
                    message=f"chore(sections): touch metadata for {slug}",
                    branch=paths["branch"],
                    author_name=author_name,
                    author_email=author_email,
                )
...
def _read_live_document_html(
    ...
    *,
    ...
    sidecar_html = _read_document_via_sidecar(
        sidecar_base, sidecar_secret, document_path=paths["document_path"]
    )
    if isinstance(sidecar_html, str) and sidecar_html:
        return sidecar_html
    file = sc.get_file(repo_id=repo_id, path=paths["document_path"], ref=paths["branch"])
    if file.is_binary or file.text is None:
        raise ValidationError("Document is not UTF-8 text")
    return file.text


def _save_live_document_html(
    sc: SourceControlService,
    *,
    repo_id: str,
    paths: dict[str, str],
...
@router.post("/projects/{slug}/apply-template", response_model=DocumentResponse)
async def apply_template(
    ...
    """Rewrite the open document from a seed template (HTML source of truth)."""
    ...
    def _apply() -> DocumentResponse:
        ...
        commit = sc.upsert_files(
            ...
            message=f"feat(sections): apply template {body.template_id}",
            branch=paths["branch"],
            author_name=author_name,
            author_email=author_email,
        )
...
@router.get("/projects/{slug}/history/diff", response_model=DocumentDiffResponse)
async def get_history_diff(
    ...
    """Files changed between two commits on the document branch, for the Files panel.

    ``base`` is the commit the caller wants to diff against (its own baseline,
    e.g. the branch head when the chat session opened this document). ``head``
    ...
    def _diff() -> DocumentDiffResponse:
        paths = _resolve_project_paths(
            sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug
        )
        if paths is None:
...
@router.get("/projects/{slug}/version", response_model=DocumentVersionResponse)
async def get_document_version(
    ...
    def _version() -> DocumentVersionResponse:
        paths = _resolve_project_paths(
            sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug
        )
        if paths is None:
...
def _adapter_runtime_binding(
    ...
    ``/coding-environments/sandbox/runtime``; without it Documents probes a
...
async def lookup_documents_sidecar(
    db: AsyncSession,
    *,
    workspace_id: str,
    user_id: str,
    ...
    """Return (sidecar_base, sidecar_secret) for an open Documents document, if bound."""
...
async def _ensure_runtime_impl(
    *,
    ...
    sc_gate, repo_gate = _sections_sc(request)
    ...
    if owned_paths is None:
        return RuntimeResponse(
            ensured=False,
            detail=f"sections project {slug} not found in this workspace",
            label=_runtime_label(workspace_id, slug),
            coder_workspace=_coder_workspace_name(workspace_id, slug),
            branch=_branch_for(workspace_id, slug),
        )
    ...
    def _pick_template() -> tuple[str, str] | None:
        ...
        for wanted in _DOCUMENTS_TEMPLATE_NAMES:
    ...
    if not picked:
        return RuntimeResponse(
            ensured=False,
            detail="No Coder template available (push abi-documents)",
            label=label,
            coder_workspace=name,
            branch=branch,
        )
    ...
    # Reuse the dedicated env for this documents/<slug> when already bound.
    if db is not None:
        labels = _runtime_labels(workspace_id, slug)
        result = await db.execute(
            select(CodingEnvironmentModel).where(
            ...
            try:
                ...
                if has_creds:
                    ...
                    if not sidecar_ready and status.phase == "running":
                        logger.warning(
                            "sections sidecar unhealthy for %s; restarting workspace %s",
                            slug,
                            existing.id,
                        )
                        try:
                ...
                logger.warning(
                    "sections runtime reuse failed for %s (%s); reprovisioning",
                    slug,
                    exc,
                )
    ...
    if db is not None:
        try:
            ...
            await db.rollback()
            logger.exception("Failed to persist sections runtime binding for %s", slug)
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            if not runtime.ensured:
                logger.warning(
                    "sections runtime not ensured for %s after %.3fs: %s",
                    slug,
                    elapsed,
                    runtime.detail,
                )
                ...
                logger.info(
                    "sections runtime ready in %.3fs slug=%s sidecar_ready=%s",
                    elapsed,
                    slug,
                    runtime.sidecar_ready,
                )
    except Exception:
        logger.exception("sections runtime ensure failed for %s", slug)
...
async def get_project_tree(
    ...
    """List the document folder (document.html, assets/, …) for the sidebar tree."""
    await require_workspace_access(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    sc, repo_id = _sections_sc(request)
    ...
    def _tree() -> ProjectTreeResponse:
        ...
        root = (
            ...
            else f"documents/{_workspace_segment(workspace_id)}/{slug}"
        )
        try:
        ...
        if "document.html" not in names:
            entries.append(
                TreeEntryResponse(
                    name="document.html", path=paths["document_path"], type="file"
                )
            )
        ...
        if not embedded:
            try:
                ...
                if document.text:
                    embedded = _count_embedded_images(document.text)
            except SourceControlError:
                pass
...
class SectionInsertRequest(BaseModel):
    workspace_id: str = Field(..., min_length=1, max_length=100)
    after_index: int = -1
...
class SectionIndexRequest(BaseModel):
    workspace_id: str = Field(..., min_length=1, max_length=100)
    index: int
...
def _mutation_outline(html: str) -> tuple[list[SectionOutlineItem], list[str | None]]:
    ...
    items = _section_outline_items(sections)
    sections = [
        SectionOutlineItem(
    ...
    return sections, [s.id for s in sections]


def _mutation_http_error(result: dict) -> HTTPException:
    detail = str(result.get("error") or "Section mutation failed")
    status = 409 if "last section" in detail.lower() else 422
    return HTTPException(status_code=status, detail=detail)


async def _run_section_html_mutation(
    *,
    slug: str,
    workspace_id: str,
    request: Request,
    ...
    def _run() -> SectionMutationResponse:
        paths = _resolve_project_paths(
            sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug
        )
        if paths is None:
        ...
        sections = [
            ...
            for item in mutated.get("sections") or []
        ]
        return SectionMutationResponse(
            ok=True,
            slug=slug,
...
@router.get("/projects/{slug}/sections", response_model=SectionsListResponse)
async def list_sections(
    ...
    """Outline of ``<section>`` documents. No HTML body."""
    await require_workspace_access(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    sc, repo_id = _sections_sc(request)
    ...
    def _list() -> SectionsListResponse:
        paths = _resolve_project_paths(
            sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug
        )
        if paths is None:
        ...
        sections, ids = _mutation_outline(html)
...
@router.post("/projects/{slug}/sections/insert", response_model=SectionMutationResponse)
async def insert_section(
    slug: str,
    body: SectionInsertRequest,
    request: Request,
    current_user: User = Depends(get_current_user_required),
    db: AsyncSession = Depends(get_db),
) -> SectionMutationResponse:
    ...
    return await _run_section_html_mutation(
        ...
        current_user=current_user,
        db=db,
        mutate=lambda html: _insert_section_html(
            html,
            after_index=body.after_index,
            layout=body.layout,
            title=body.title,
        ),
        message=f"feat(sections): insert section in {slug}",
    )


@router.post("/projects/{slug}/sections/delete", response_model=SectionMutationResponse)
async def delete_section(
    slug: str,
    body: SectionIndexRequest,
    request: Request,
    current_user: User = Depends(get_current_user_required),
    db: AsyncSession = Depends(get_db),
) -> SectionMutationResponse:
    ...
    return await _run_section_html_mutation(
        ...
        current_user=current_user,
        db=db,
        mutate=lambda html: _delete_section_html(html, body.index),
        message=f"refactor(sections): delete section in {slug}",
    )


@router.post("/projects/{slug}/sections/duplicate", response_model=SectionMutationResponse)
async def duplicate_section(
    ...
    return await _run_section_html_mutation(
        ...
        current_user=current_user,
        db=db,
        mutate=lambda html: _duplicate_section_html(html, body.index),
        message=f"feat(sections): duplicate section in {slug}",
    )


@router.post("/projects/{slug}/sections/reorder", response_model=SectionMutationResponse)
async def reorder_sections(
    ...
    return await _run_section_html_mutation(
        ...
        current_user=current_user,
        db=db,
        mutate=lambda html: _reorder_sections_html(
            html,
