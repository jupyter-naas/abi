...
class OntologyConfigSecondaryAdapterPostgres(OntologyConfigPersistencePort):
    def __init__(
        self,
    ...
    def db(self) -> AsyncSession:
        ...
        if self._db_getter is None:
            raise RuntimeError(
                "OntologyConfigSecondaryAdapterPostgres has no database binding"
            )
        db = self._db_getter()
        if db is None:
    ...
    def _to_record(model: OntologyConfigModel) -> OntologyConfigRecord:
        return OntologyConfigRecord(
            ...
            ontology_id=str(model.ontology_id),
