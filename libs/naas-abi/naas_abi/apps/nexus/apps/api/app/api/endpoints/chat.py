...
VALID_PROVIDER_TYPES = get_all_provider_names() + [
    "custom",
    "abi",
]  # Add "custom" for user-defined, "abi" for external ABI servers
...
async def _resolve_provider(
            ...
            try:
                ...
                if agent and agent.provider and agent.model_id:
                    ...
                    if secret_key:
                        ...
                        if secret:
                            ...
                            logging.info(
                                f"✓ Resolved agent provider: {agent.provider} (model: {agent.model_id})"
                            )
                            ...
                            logging.warning(
                                f"No API key found for {agent.provider} (key: {secret_key})"
                            )
                ...
                if not agent and workspace_id and agent_id:
                    ...
                    if abi_server:
                        ...
                        logging.info(
                            f"✓ Resolved discovered ABI agent '{agent_id}' via {abi_server.endpoint}"
                        )
                        return ProviderConfigRequest(
                            id=f"abi-{abi_server.id}",
                            name=f"ABI ({abi_server.name})",
    ...
    if ollama_status["status"] == "online" and ollama_status["models"]:
        ...
        model = (
            ...
            else _select_provider_model(ollama_status, has_images)
        )
        return ProviderConfigRequest(
            id="ollama-fallback",
            name="Ollama (Auto)",
...
async def _build_provider_messages_with_agents(
    request: ChatRequest, current_agent_id: str, db: AsyncSession
    ...
    for m in request.messages:
        ...
        if m.role == "assistant" and m.agent and m.agent != current_agent_id:
            ...
            messages.append(ProviderMessage(role="system", content=system_msg, images=None))

        messages.append(ProviderMessage(role=m.role, content=m.content, images=m.images))
...
async def _run_search_if_needed(request: ChatRequest) -> str | None:
    ...
    try:
        wiki_results = await execute_tool(
            "search_web", {"query": request.message, "engine": "wikipedia"}
        )
    ...
    try:
        ddg_results = await execute_tool(
            "search_web", {"query": request.message, "engine": "duckduckgo"}
        )
...
Use these search results to provide an accurate, well-sourced answer. Cite sources with URLs when available. If the results don't fully answer the question, supplement with your knowledge."""
        ...
        context = (
            "Web search was attempted but returned no results. Answer based on your knowledge."
        )
...
async def complete_chat(
    ...
    if provider:
        try:
            provider_messages = await _build_provider_messages_with_agents(
                request, request.agent, db
            )
...
async def stream_chat(
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"🎯 Stream request: agent={request.agent}, provider={'None' if not request.provider else request.provider.type}"
    )
    ...
    logger.info(
        f"✓ Resolved provider: {provider.type if provider else 'None'}, model={provider.model if provider else 'N/A'}"
    )
    ...
    try:
        ...
        _logging.getLogger(__name__).error(
            "Failed to pre-persist streaming messages", exc_info=True
        )
        ...
        try:
            ...
            if provider.type == "ollama":
                ...
                async for chunk in stream_with_ollama(
                    provider_messages, provider_config, system_prompt
                ):
                    full_response += chunk
                ...
                async for chunk in stream_with_cloudflare(
                    provider_messages, provider_config, system_prompt
                ):
                    full_response += chunk
                    buffered_chars += len(chunk)
...
async def delete_conversation(
    ...
    await db.execute(delete(MessageModel).where(MessageModel.conversation_id == conversation_id))
    await db.delete(row)
