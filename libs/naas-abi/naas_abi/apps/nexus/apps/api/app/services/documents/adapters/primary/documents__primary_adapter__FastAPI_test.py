...
def test_create_project_returns_before_runtime_ensure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(documents_api, "_schedule_created_runtime", _schedule)
    monkeypatch.setattr(documents_api, "_ensure_runtime_impl", _boom)
    monkeypatch.setattr(documents_api, "_ensure_runtime_serialized", _boom)
    ...
    document = client.get(
        "/documents/projects/fast-create/document",
        params={"workspace_id": "ws-test"},
    )
    assert document.status_code == 200, document.text
    assert document.json()["html"]
