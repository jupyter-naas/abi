"""Sheets FastAPI primary adapter.

Business-facing sheet workbooks stored in Forgejo under a workspace namespace
(branch ``sheets/<workspace_id>/<slug>``, path
``sheets/<workspace_id>/<slug>/workbook.html``). Legacy ``sheets/<slug>`` decks
remain available only when ``project.json.workspace_id`` matches (unscoped
legacy decks require a verified workspace_id owner). A Coder ``abi-sheets`` workspace is
...
async def require_sheets_write(user_id: str, workspace_id: str) -> None:
    role = await require_workspace_access(user_id, workspace_id)
    if role not in {"owner", "admin", "member"}:
        raise HTTPException(
            status_code=403, detail="Sheets editing requires a workspace writer role"
        )
...
_DEFAULT_TEMPLATE = "grid-light-v1"
_ABI_NAMESPACE = ABI_SHEETS_TEMPLATE_NAMESPACE
_ABI_PACKAGE = "naas_abi.apps.nexus.assets.sheets.templates"
...
_SHEETS_TEMPLATE_NAMES = ("abi-sheets", "abi-code-server", "local-directory")
...
def _get_source_control(request: Request) -> SourceControlService:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Forgejo is not configured. Sheets needs git storage.",
        ) from exc
...
_FORGEJO_NOT_CONFIGURED = "Forgejo is not configured. Sheets needs git storage."
_FORGEJO_UNREACHABLE = "Forgejo is not reachable. Sheets needs git storage."
...
def _sheets_sc(request: Request) -> tuple[SourceControlService, str]:
...
def _workbook_path(workspace_id: str, slug: str) -> str:
    return f"sheets/{_workspace_segment(workspace_id)}/{slug}/workbook.html"
...
def _project_path(workspace_id: str, slug: str) -> str:
    return f"sheets/{_workspace_segment(workspace_id)}/{slug}/project.json"
...
def _assets_dir(workspace_id: str, slug: str) -> str:
    return f"sheets/{_workspace_segment(workspace_id)}/{slug}/assets"


def _legacy_assets_dir(slug: str) -> str:
    return f"sheets/{slug}/assets"
...
def _paths_for(workspace_id: str, slug: str, *, legacy: bool = False) -> dict[str, str]:
    ...
    return {
        ...
        "workbook_path": _workbook_path(workspace_id, slug),
...
def _resolve_project_paths(
    ...
    *,
    ...
    names = (
        ...
        else {b.name for b in sc.list_branches(repo_id=repo_id)}
    )
    ns = _paths_for(workspace_id, slug, legacy=False)
    if ns["branch"] in names:
        meta = _load_project_meta(sc, repo_id=repo_id, path=ns["project_path"], ref=ns["branch"])
        if meta is None:
            return None
...
def _claim_workspace_in_meta(
    ...
    *,
    ...
    meta = _load_project_meta(sc, repo_id=repo_id, path=paths["project_path"], ref=paths["branch"])
    ...
    sc.upsert_file(
        ...
        message=f"chore(sheets): claim {slug} for workspace",
        branch=paths["branch"],
...
_ASSETS_README = """# Workbook assets

Images for this deck live here. ``workbook.html`` references them as relative
``assets/<file>`` paths.

## Seed

New workbooks and Apply template copy the HTML plus any ``assets/``
folder from the template catalog. Preview resolves those paths through the
sheets asset route. File, Export HTML inlines them again so the download
is one file.
...
def _assets_note(*, copied: int, embedded: int) -> str | None:
    ...
    if embedded:
        return "assets/ seeded empty; template images remain as data-URLs in workbook.html"
    return None
...
def _seed_file_writes(
    *,
    ...
    writes.append(FileWrite(path=paths["workbook_path"], content=seed))
    if assets:
    ...
    if include_readme:
        writes.append(FileWrite(path=paths["assets_readme"], content=_ASSETS_README))
    return writes
...
class _TemplateSource(NamedTuple):
    """One namespaced tree of seed workbooks.
...
def _abi_template_dirs() -> list[Path]:
    ...
    dirs = [
        here.parents[7] / "assets" / "sheets" / "templates",
        Path("/app/assets/sheets/templates"),
        Path("assets/sheets/templates"),
    ]
    try:
...
def _template_sources() -> list[_TemplateSource]:
    """ABI's own seeds, then whatever ``sheets_template_sources`` adds.
    ...
    for entry in getattr(settings, "sheets_template_sources", None) or []:
        ...
        if not usable:
            ...
            logger.warning(
                "Sheets template source '%s' is not a directory: %s",
                entry.namespace,
                directory,
            )
            continue
        sources.append(_TemplateSource(entry.namespace, str(directory), (directory,)))
    return sources
...
def _seed_template_meta(template_id: str) -> dict[str, str]:
    ...
    return {
        ...
        "origin": source.origin,
        "name": title,
        "description": f"Workbook seed ({stem})",
        "preview_bg": "#f4f4f4",
...
def _list_seed_template_records() -> list[dict]:
    ...
    for tid in _discover_seed_ids():
        ...
        file_assets = [{"name": name, "kind": "file"} for name in _list_seed_asset_names(tid)]
        meta["assets"] = file_assets or (_parse_template_assets(seed) if seed else [])
        rows.append(meta)
    return rows
...
def _load_seed_html(template_id: str = _DEFAULT_TEMPLATE_ID) -> str:
    ...
    if namespace and source is None:
        raise HTTPException(
            status_code=404,
            detail=f"Unknown sheets template '{template_id}'.",
        )
...
def _iter_seed_asset_entries(template_id: str):
    ...
    for candidate in candidates:
        for root in candidate.roots:
            ...
            for entry in entries:
                name = getattr(entry, "name", "") or ""
                if not name or name in _SKIP_ASSET_NAMES or not _ASSET_FILENAME_RE.match(name):
                    continue
                try:
...
class WorkbookDiffFileResponse(BaseModel):
    path: str
    status: str
    additions: int
    deletions: int
...
class WorkbookDiffResponse(BaseModel):
    base: str
    head: str
    files: list[WorkbookDiffFileResponse]
...
def _runtime_label(workspace_id: str, slug: str) -> str:
    return f"sheets/{_workspace_segment(workspace_id)}/{slug}"


def _legacy_runtime_label(slug: str) -> str:
    return f"sheets/{slug}"
...
def _read_workbook_via_sidecar(
    base: str | None, secret: str | None, *, workbook_path: str
) -> str | None:
    """Return deck HTML from the Coder sidecar, or None when unavailable."""
    if not _probe_sidecar(base, secret):
        return None
    result = _sidecar_tool_call(base, secret, "read_file", {"path": workbook_path}, timeout_s=10.0)
...
def _write_workbook_via_sidecar(
    base: str | None, secret: str | None, *, workbook_path: str, html: str
) -> bool:
    ...
    result = _sidecar_tool_call(
        ...
        secret,
        "write_file",
        {"path": workbook_path, "content": html},
        timeout_s=20.0,
    )
...
def _friendly_git_detail(exc: BaseException) -> str:
    ...
    if len(text) > 180 or text.startswith("{") or "forgejo api request failed" in text.lower():
...
def _git_clone_url(sc: SourceControlService, repo_id: str, *, username: str, token: str) -> str:
...
async def create_project(
    ...
    await require_sheets_write(current_user.id, body.workspace_id)
    ...
    def _create() -> ProjectResponse:
        ...
        meta = {
            ...
            "assets_note": _assets_note(copied=len(catalog_assets), embedded=embedded),
        }
        # Concurrent/prior create already finished: keep 409. If seed is
        # incomplete (branch exists, workbook.html missing), finish seeding.
        if adopted_branch:
            try:
                existing_deck = sc.get_file(
                    repo_id=repo_id, path=paths["workbook_path"], ref=branch
                )
                if existing_deck.text:
                    raise BranchNameConflictError(f"Sheets project '{slug}' already exists")
        ...
        commit = sc.upsert_files(
            ...
            message=f"feat(sheets): create {slug} from {body.template_id}",
            branch=branch,
            author_name=author_name,
            author_email=author_email,
        )
    ...
    try:
        ...
        if not runtime.ensured:
            logger.warning("sheets runtime not ensured for %s: %s", slug, runtime.detail)
    except Exception:
        logger.exception("sheets runtime ensure failed for %s", slug)
...
async def get_project(
    ...
    def _get() -> ProjectResponse:
        paths = _resolve_project_paths(sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug)
        if paths is None:
            raise RepoNotFoundError(f"sheets project {slug}")
...
async def update_project(
    ...
    def _update() -> ProjectResponse:
        ...
        sc.upsert_file(
            ...
            path=paths["project_path"],
            content=json.dumps(meta, indent=2) + "\n",
            message=f"chore(sheets): update {slug} project settings",
            branch=paths["branch"],
            author_name=author_name,
            author_email=author_email,
        )
...
@router.get("/projects/{slug}/workbook", response_model=WorkbookResponse)
async def get_workbook(
    ...
    Git storage is authoritative; sidecars are best-effort mirrors.
    """
    await require_workspace_access(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    ...
    try:
        ...
        def _get_forgejo() -> WorkbookResponse:
            file = sc.get_file(repo_id=repo_id, path=paths["workbook_path"], ref=paths["branch"])
...
@router.get("/projects/{slug}/export/xlsx")
async def export_workbook_xlsx(
    ...
    def _read() -> bytes:
        ...
        try:
            ...
            workbook = grid_from_table_html(html)
            if workbook is None:
                raise ValueError(
    ...
    try:
        payload = await run_in_threadpool(_read)
...
async def evaluate_workbook_preview(
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if len(workbook.sheets) > 256 or sum(len(row) for tab in workbook.sheets for row in tab.rows) > 100_000:
        raise HTTPException(status_code=422, detail="Calculate up to 100,000 cells")

    def calculate() -> dict:
        calculated, errors = calculate_workbook(workbook)
        return {"workbook": calculated.model_dump(), "errors": errors}
        ...
        try:
            ...
            if meta_file.text:
                ...
                sc.upsert_file(
                    ...
                    message=f"chore(sheets): touch metadata for {slug}",
                    branch=paths["branch"],
                    author_name=author_name,
                    author_email=author_email,
                )
...
def _save_live_deck_html(
    ...
    *,
    ...
    try:
        meta_file = sc.get_file(repo_id=repo_id, path=paths["project_path"], ref=paths["branch"])
        if meta_file.text:
            data = _parse_meta(meta_file.text)
            from datetime import UTC, datetime
...
@router.post("/projects/{slug}/apply-template", response_model=WorkbookResponse)
async def apply_template(
    ...
    def _apply() -> WorkbookResponse:
        ...
        meta = (
            _load_project_meta(sc, repo_id=repo_id, path=paths["project_path"], ref=paths["branch"])
            or {}
        )
        meta["workspace_id"] = body.workspace_id
...
@router.get("/projects/{slug}/history/diff", response_model=WorkbookDiffResponse)
async def get_history_diff(
    ...
    def _diff() -> WorkbookDiffResponse:
        paths = _resolve_project_paths(sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug)
        if paths is None:
            raise RepoNotFoundError(f"sheets project {slug}")
        resolved_head = head
...
@router.get("/projects/{slug}/version", response_model=WorkbookVersionResponse)
async def get_workbook_version(
    slug: str,
    workspace_id: str,
    request: Request,
    current_user: User = Depends(get_current_user_required),
    ...
    def _version() -> WorkbookVersionResponse:
        paths = _resolve_project_paths(sc, repo_id=repo_id, workspace_id=workspace_id, slug=slug)
        if paths is None:
            raise RepoNotFoundError(f"sheets project {slug}")
...
async def _ensure_runtime_impl(
    *,
    ...
    sc_gate, repo_gate = _sheets_sc(request)
    ...
    if owned_paths is None:
        return RuntimeResponse(
            ensured=False,
            detail=f"sheets project {slug} not found in this workspace",
            label=_runtime_label(workspace_id, slug),
            coder_workspace=_coder_workspace_name(workspace_id, slug),
            branch=_branch_for(workspace_id, slug),
        )
    ...
    def _pick_template() -> tuple[str, str] | None:
        ...
        for wanted in _SHEETS_TEMPLATE_NAMES:
    ...
    if not picked:
        return RuntimeResponse(
            ensured=False,
            detail="No Coder template available (push abi-sheets)",
            label=label,
            coder_workspace=name,
            branch=branch,
        )
    ...
    author_email = str(current_user.email)
    coder_username = _sanitize_coder_username(current_user.name or "") or _sanitize_coder_username(
        author_email.split("@", 1)[0]
    )
    ...
    if db is not None:
        ...
        if existing is not None:
            try:
                ...
                bound = await run_in_threadpool(_adapter_runtime_binding, coding, existing.id)
                ...
                if has_creds:
                    ...
                    if not sidecar_ready and status.phase == "running":
                        ...
                        try:
                            await run_in_threadpool(coding.stop, workspace_id=existing.id)
                ...
                logger.warning(
                    "sheets runtime reuse failed for %s (%s); reprovisioning",
                    slug,
                    exc,
                )
    ...
    try:
        ...
        existing_envs = await run_in_threadpool(coding.list_environments, user_id=user_id)
        prior_ws = next(
        ...
        if prior_ws is not None:
            ...
            if prior_ws.phase in ("stopped", "stopping"):
                ...
                status = await run_in_threadpool(coding.get_status, workspace_id=prior_ws.id)
                # Prefer the secret already baked into the running sidecar.
                on_ws = await run_in_threadpool(_adapter_get_parameters, coding, prior_ws.id)
                ...
                if baked:
                    ...
                    try:
                        await run_in_threadpool(coding.stop, workspace_id=prior_ws.id)
                    except CodingEnvironmentError:
                        pass
                    status = await run_in_threadpool(
                        coding.start, workspace_id=prior_ws.id, params=params
            ...
            on_ws = await run_in_threadpool(_adapter_get_parameters, coding, status.id)
            baked = (on_ws or {}).get("sidecar_secret") or ""
        ...
        try:
            ...
            if match.phase in ("stopped", "stopping"):
                status = await run_in_threadpool(coding.start, workspace_id=match.id, params=params)
            else:
                status = await run_in_threadpool(coding.get_status, workspace_id=match.id)
                on_ws = await run_in_threadpool(_adapter_get_parameters, coding, match.id)
                baked = (on_ws or {}).get("sidecar_secret") or ""
                if baked:
                    ws_secret = baked
            bound = await run_in_threadpool(_adapter_runtime_binding, coding, status.id)
    ...
    if db is not None:
        try:
            ...
            await db.rollback()
            logger.exception("Failed to persist sheets runtime binding for %s", slug)
...
async def ensure_runtime(
    ...
    await require_sheets_write(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
...
async def get_project_tree(
    ...
    """List the presentation folder (workbook.html, assets/, …) for the sidebar tree."""
    await require_workspace_access(current_user.id, workspace_id)
    if not _SLUG_RE.match(slug):
        raise HTTPException(status_code=422, detail="Invalid slug")
    sc, repo_id = _sheets_sc(request)
    ...
    def _tree() -> ProjectTreeResponse:
        ...
        root = (
            ...
            else f"sheets/{_workspace_segment(workspace_id)}/{slug}"
        )
        try:
        ...
        if "workbook.html" not in names:
            entries.append(
                TreeEntryResponse(name="workbook.html", path=paths["workbook_path"], type="file")
            )
        if "assets" not in names:
            entries.append(TreeEntryResponse(name="assets", path=paths["assets_dir"], type="dir"))
        entries.sort(key=lambda e: (0 if e.type == "dir" else 1, e.name))

        assets: list[TreeEntryResponse] = []
        try:
            assets_raw = sc.list_contents(repo_id=repo_id, path=paths["assets_dir"], ref=branch)
            assets = [
                TreeEntryResponse(
                    name=e.name,
                    path=e.path,
        ...
        embedded = 0
        assets_note = None
        meta = _load_project_meta(sc, repo_id=repo_id, path=paths["project_path"], ref=branch) or {}
        try:
        ...
        if not embedded:
            try:
                deck = sc.get_file(repo_id=repo_id, path=paths["workbook_path"], ref=branch)
                if deck.text:
        ...
        return ProjectTreeResponse(
            ...
            assets=assets,
            embedded_images=embedded,
            assets_note=assets_note or _assets_note(copied=len(assets), embedded=embedded),
        )
