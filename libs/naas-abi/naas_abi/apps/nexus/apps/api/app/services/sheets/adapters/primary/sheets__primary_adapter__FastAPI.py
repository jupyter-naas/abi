...
def _read_workbook_via_sidecar(
    ...
    result = _sidecar_tool_call(
        base, secret, "read_file", {"path": workbook_path}, timeout_s=10.0
    )
...
async def create_project(
            ...
            try:
                ...
                if existing_deck.text:
                    raise BranchNameConflictError(
                        f"Sheets project '{slug}' already exists"
                    )
    ...
    try:
        ...
        if not runtime.ensured:
            logger.warning(
                "sheets runtime not ensured for %s: %s", slug, runtime.detail
            )
    except Exception:
...
async def get_workbook(
    ...
    Prefer the Coder sidecar (live editing SoT) when the sheets runtime is
    ...
    def _get_sidecar() -> str | None:
        return _read_workbook_via_sidecar(
            sidecar_base, sidecar_secret, workbook_path=paths["workbook_path"]
        )
...
def _read_live_deck_html(
    ...
    *,
    ...
    sidecar_html = _read_workbook_via_sidecar(
        sidecar_base, sidecar_secret, workbook_path=paths["workbook_path"]
    )
    if isinstance(sidecar_html, str) and sidecar_html:
        return sidecar_html
...
async def get_project_tree(
    ...
    def _tree() -> ProjectTreeResponse:
        ...
        if "workbook.html" not in names:
            entries.append(
                TreeEntryResponse(
                    name="workbook.html", path=paths["workbook_path"], type="file"
                )
            )
