...
def test_runtime_reports_the_adapter_binding_for_lookup(monkeypatch) -> None:
    ...
    client.post(
        "/sheets/projects/bound-deck/runtime", params={"workspace_id": "ws-test"}
    )
...
def test_history_uses_conventional_commits_and_diff_resolves_head(monkeypatch) -> None:
    ...
    history = client.get(
        "/sheets/projects/diff-deck/history", params={"workspace_id": "ws-test"}
    )
    assert history.status_code == 200, history.text
    assert history.json()[0]["message"].startswith("feat(sheets): create")

    deck = client.get(
        "/sheets/projects/diff-deck/workbook", params={"workspace_id": "ws-test"}
    )
    assert deck.status_code == 200, deck.text
    saved = client.put(
...
def test_deck_version_bumps_from_conventional_commit_history(monkeypatch) -> None:
    ...
    deck = client.get(
        "/sheets/projects/version-deck/workbook", params={"workspace_id": "ws-test"}
    )
    assert deck.status_code == 200, deck.text
    saved = client.put(
