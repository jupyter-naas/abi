...
from naas_abi.apps.nexus.apps.api.app.core.config import SheetsTemplateSourceConfig
from naas_abi.apps.nexus.apps.api.app.core.database import get_db
from naas_abi.apps.nexus.apps.api.app.services.sheets.adapters.primary import (
    sheets__primary_adapter__FastAPI as slides_api,
)
...
def test_slugify_and_paths() -> None:
    ...
    assert _branch_for("ws-abc", "demo") == "sheets/ws-abc/demo"
    assert _workbook_path("ws-abc", "demo") == "sheets/ws-abc/demo/workbook.html"
    assert _project_path("ws-abc", "demo") == "sheets/ws-abc/demo/project.json"
    assert _assets_dir("ws-abc", "demo") == "sheets/ws-abc/demo/assets"
    assert _assets_gitkeep_path("ws-abc", "demo") == "sheets/ws-abc/demo/assets/.gitkeep"
    assert _runtime_label("ws-abc", "q3-br") == "sheets/ws-abc/q3-br"
    assert _coder_workspace_name("ws-abc", "q3-br").startswith("s-")
    assert _legacy_branch_for("demo") == "sheets/demo"
    assert _legacy_workbook_path("demo") == "sheets/demo/workbook.html"
    ns = _paths_for("ws-abc", "demo", legacy=False)
    assert ns["branch"] == "sheets/ws-abc/demo"
...
def _configure_sources(monkeypatch, *sources: tuple[str, Path]) -> None:
    monkeypatch.setattr(
        slides_api.settings,
        "sheets_template_sources",
        [
            SheetsTemplateSourceConfig(namespace=namespace, path=str(path))
...
def test_a_configured_source_adds_its_own_namespace(tmp_path, monkeypatch) -> None:
    ...
    assert "abi/grid-light-v1" in ids
    assert "acme/tenant-only-v1" in ids
    assert "acme/grid-light-v1" not in ids
...
def test_an_unknown_namespace_is_a_miss_not_a_syntax_error(monkeypatch) -> None:
    ...
    with pytest.raises(HTTPException) as caught:
        _load_seed_html("nobody/grid-light-v1")
...
def test_a_source_namespace_is_refused_at_config_time(namespace: str) -> None:
    ...
    with pytest.raises(ValidationError):
        SheetsTemplateSourceConfig(namespace=namespace, path="seeds/templates")


def test_a_missing_source_directory_does_not_hide_the_abi_seeds(tmp_path, monkeypatch) -> None:
...
def test_friendly_coding_detail_hides_raw_coder_json() -> None:
    ...
    assert _friendly_coding_detail(Exception(raw)) == ("Reconnecting to existing runtime…")
...
def _slides_client(monkeypatch, source_control: SourceControlService) -> TestClient:
    ...
    app.include_router(slides_api.router, prefix="/sheets")
...
def test_list_and_create_projects_seed_in_memory_repo(monkeypatch) -> None:
    ...
    listed = client.get("/sheets/projects", params={"workspace_id": "ws-test"})
    ...
    assert body["workbook_path"] == "sheets/ws-test/untitled-local/workbook.html"
    assert body["archived"] is False
    listed2 = client.get("/sheets/projects", params={"workspace_id": "ws-test"})
    ...
    assert namespaced.json()["template_id"] == "abi/grid-light-v1"
...
def test_patch_project_renames_and_archives_without_changing_slug(monkeypatch) -> None:
    ...
    assert renamed.json()["workbook_path"] == "sheets/ws-test/untitled-patch/workbook.html"
...
def test_friendly_git_detail_hides_pushrejected_dump() -> None:
    raw = (
        ...
        "cannot lock ref 'refs/heads/sheets/demo-deck': "
...
def test_wait_for_sidecar_retries_until_ready(monkeypatch) -> None:
    ...
    sleeps: list[float] = []
    monkeypatch.setattr(
        "naas_abi.apps.nexus.apps.api.app.services.sheets.adapters.primary."
        "sheets__primary_adapter__FastAPI._probe_sidecar",
        fake_probe,
    )
    monkeypatch.setattr(
        "naas_abi.apps.nexus.apps.api.app.services.sheets.adapters.primary."
        "sheets__primary_adapter__FastAPI.time.sleep",
        sleeps.append,
    )
    assert (
        _wait_for_sidecar("http://coder-x-slides-demo:8378", "secret", attempts=5, interval_s=0.01)
        is True
    )
    assert calls["n"] == 3
    assert len(sleeps) == 2
...
def test_wait_for_sidecar_gives_up(monkeypatch) -> None:
    ...
    assert (
        _wait_for_sidecar("http://coder-x-slides-demo:8378", "secret", attempts=3, interval_s=0.01)
        is False
    )
...
def test_sidecar_tool_helpers_require_binding() -> None:
    ...
    assert _read_deck_via_sidecar(None, "s", workbook_path="sheets/ws/demo/workbook.html") is None
    assert (
        _write_deck_via_sidecar(
            None, "s", workbook_path="sheets/ws/demo/workbook.html", html="<html></html>"
        )
        is False
    )
    assert (
        _sidecar_tool_call("file:///tmp", "s", "read_file", {"path": "x"}).get("error") or ""
...
def test_slides_template_names_match_local_directory_adapter(tmp_path) -> None:
    ...
    assert advertised & set(slides_api._SHEETS_TEMPLATE_NAMES), (
        f"none of {slides_api._SHEETS_TEMPLATE_NAMES} match {advertised}"
    )
...
def test_clone_url_uses_the_local_checkout_when_git_is_on_disk() -> None:
    ...
    url = slides_api._git_clone_url(sc, "abi/monorepo", username="git", token="local")
...
def test_clone_url_keeps_authenticated_http_for_a_real_forge() -> None:
    ...
    url = slides_api._git_clone_url(sc, "abi/monorepo", username="git", token="tok en")
...
def test_clone_url_falls_back_when_the_adapter_cannot_describe_the_repo() -> None:
    ...
    url = slides_api._git_clone_url(_Broken(), "abi/monorepo", username="git", token="t")
...
class _FakeLocalCoding:
    ...
    def provision(self, *, user_id: str, template_id: str, name: str, params=None):
        ...
        return WorkspaceStatus(id="env-local-1", name=name, phase="running", agent_ready=True)
...
def test_runtime_binds_the_sidecar_the_adapter_actually_started(monkeypatch) -> None:
    ...
    runtime = client.post(
        "/sheets/projects/runtime-deck/runtime",
        params={"workspace_id": "ws-test"},
    )
...
def test_runtime_reports_the_adapter_binding_for_lookup(monkeypatch) -> None:
    ...
    client.post("/sheets/projects/bound-deck/runtime", params={"workspace_id": "ws-test"})
...
def test_runtime_rebinds_when_the_same_environment_is_adopted_twice(monkeypatch, tmp_path) -> None:
    ...
    async def _scenario():
        ...
        def _call() -> None:
            ...
            for _ in range(2):
                res = client.post(
                    "/sheets/projects/adopted-deck/runtime",
                    params={"workspace_id": "ws-test"},
                )
        ...
        rows = (await session.execute(select(CodingEnvironmentModel))).scalars().all()
        count = len(rows)
...
def test_create_and_apply_template_copies_catalog_assets(tmp_path, monkeypatch) -> None:
    ...
    assert created.status_code == 200, created.text
    deck = client.get(
        "/sheets/projects/pixel-deck/workbook",
        params={"workspace_id": "ws-test"},
    )
    assert deck.status_code == 200, deck.text
    html = deck.json()["html"]
    ...
    commits = sc.list_commits(
        repo_id=slides_api._repo_id(), ref="sheets/ws-test/pixel-deck", limit=20
    )
    seed_commits = [c for c in commits if "feat(sheets): create" in c.message]
    assert len(seed_commits) == 1

    asset = client.get(
        "/sheets/projects/pixel-deck/assets/hero.png",
        params={"workspace_id": "ws-test"},
    )
    ...
    traversal = client.get(
        "/sheets/projects/pixel-deck/assets/..png",
        params={"workspace_id": "ws-test"},
    )
    assert traversal.status_code == 422

    tree = client.get(
        "/sheets/projects/pixel-deck/tree",
        params={"workspace_id": "ws-test"},
    )
    ...
    applied = client.post(
        "/sheets/projects/pixel-deck/apply-template",
        json={
            "workspace_id": "ws-test",
            "template_id": "office/pixel-v1",
    ...
    apply_commits = [
        ...
        if c.message.startswith("feat(sheets): apply template")
    ]
...
def test_slide_mutations_insert_delete_duplicate_reorder(monkeypatch) -> None:
    ...
    assert created.status_code == 200, created.text
    listed = client.get(
        "/sheets/projects/mutation-deck/slides",
        params={"workspace_id": "ws-test"},
    )
    assert listed.status_code == 200, listed.text
    start = listed.json()
    ...
    inserted = client.post(
        "/sheets/projects/mutation-deck/sheets/insert",
        json={
            "workspace_id": "ws-test",
            "after_index": 0,
            "layout": "content",
    ...
    duplicated = client.post(
        "/sheets/projects/mutation-deck/sheets/duplicate",
    ...
    reordered = client.post(
        "/sheets/projects/mutation-deck/sheets/reorder",
    ...
    deleted = client.post(
        "/sheets/projects/mutation-deck/sheets/delete",
    ...
    deck = client.get(
        "/sheets/projects/mutation-deck/workbook",
        params={"workspace_id": "ws-test"},
    )
...
def test_delete_last_slide_is_refused(monkeypatch) -> None:
    ...
    assert created.status_code == 200, created.text
    listed = client.get(
        "/sheets/projects/one-slide-deck/slides",
        params={"workspace_id": "ws-test"},
    )
    count = listed.json()["section_count"]
    for index in range(count - 1, 0, -1):
        gone = client.post(
            "/sheets/projects/one-slide-deck/sheets/delete",
...
def test_history_uses_conventional_commits_and_diff_resolves_head(monkeypatch) -> None:
    ...
    history = client.get("/sheets/projects/diff-deck/history", params={"workspace_id": "ws-test"})
    assert history.status_code == 200, history.text
    assert history.json()[0]["message"].startswith("feat(sheets): create")

    deck = client.get("/sheets/projects/diff-deck/workbook", params={"workspace_id": "ws-test"})
    assert deck.status_code == 200, deck.text
    saved = client.put(
        "/sheets/projects/diff-deck/workbook",
        json={
            "workspace_id": "ws-test",
            "html": deck.json()["html"],
            "expected_revision": deck.json()["revision"],
    ...
    history2 = client.get("/sheets/projects/diff-deck/history", params={"workspace_id": "ws-test"})
    # A free-text message with no Conventional Commits type is bucketed as
    # `chore(sheets): ...` rather than saved as raw free text. put_deck also
    ...
    missing = client.get(
        "/sheets/projects/does-not-exist/history/diff",
        params={"workspace_id": "ws-test", "base": base_sha},
    )
    assert missing.status_code == 404
...
def test_deck_version_bumps_from_conventional_commit_history(monkeypatch) -> None:
    ...
    version = client.get(
        "/sheets/projects/version-deck/version", params={"workspace_id": "ws-test"}
    )
    assert version.status_code == 200, version.text
    # Seed commit is `feat(sheets): create ...` -> one minor bump.
    assert version.json() == {"version": "0.1.0", "commit_count": 1}

    deck = client.get("/sheets/projects/version-deck/workbook", params={"workspace_id": "ws-test"})
    assert deck.status_code == 200, deck.text
    saved = client.put(
        "/sheets/projects/version-deck/workbook",
        json={
            "workspace_id": "ws-test",
            "html": deck.json()["html"],
            "expected_revision": deck.json()["revision"],
    ...
    inserted = client.post(
        "/sheets/projects/version-deck/sheets/insert",
    ...
    # `feat(sheets): insert slide ...` resets patch and bumps minor.
    assert version3.json()["version"] == "0.2.0"

    missing = client.get(
        "/sheets/projects/does-not-exist/version", params={"workspace_id": "ws-test"}
    )
    assert missing.status_code == 404
...
def test_history_tracks_version_per_commit(monkeypatch) -> None:
    ...
    deck = client.get(
        "/sheets/projects/history-version-deck/workbook", params={"workspace_id": "ws-test"}
    )
    assert deck.status_code == 200, deck.text
    saved = client.put(
        "/sheets/projects/history-version-deck/workbook",
        json={
            "workspace_id": "ws-test",
            "html": deck.json()["html"],
            "expected_revision": deck.json()["revision"],
    ...
    inserted = client.post(
        "/sheets/projects/history-version-deck/sheets/insert",
        json={"workspace_id": "ws-test", "after_index": -1},
    )
    assert inserted.status_code == 200, inserted.text

    version = client.get(
        "/sheets/projects/history-version-deck/version", params={"workspace_id": "ws-test"}
    )
    assert version.status_code == 200, version.text
    assert version.json()["version"] == "0.2.0"

    history = client.get(
        "/sheets/projects/history-version-deck/history", params={"workspace_id": "ws-test"}
    )
    assert history.status_code == 200, history.text
...
def test_semver_from_commits_ignores_unrecognized_and_non_bumping_types() -> None:
    ...
    oldest_to_newest = [
        ...
        Commit(sha="2", message="feat(sheets): create x", author="a"),
        Commit(sha="3", message="style(slides): reorder slides", author="a"),
        Commit(sha="4", message="fix(sheets): replace text", author="a"),
        Commit(sha="5", message="feat(sheets)!: breaking layout change", author="a"),
        Commit(sha="6", message="fix(sheets): another fix", author="a"),
    ]
...
def test_evaluate_local_draft_preserves_formula_and_requires_workspace(monkeypatch):
    ...
    assert (
        client.post("/sheets/evaluate", json={"workspace_id": "other", "html": html}).status_code
        == 403
    )
...
def test_viewers_cannot_mutate_sheets(monkeypatch, role):
    ...
    assert (
        client.post(
            "/sheets/projects", json={"workspace_id": "ws-test", "title": "Roles", "slug": "roles"}
        ).status_code
        == 200
    )
    revision = client.get(
    ...
    monkeypatch.setattr(slides_api, "require_workspace_access", access)
    assert (
        client.get(
            "/sheets/projects/roles/workbook", params={"workspace_id": "ws-test"}
        ).status_code
    ...
    writes = [
        ("post", "/sheets/projects", {"title": "Blocked", "slug": "blocked"}),
        (
            "put",
    ...
    for method, url, body in writes:
        response = getattr(client, method)(url, json={"workspace_id": "ws-test", **body})
