...
import naas_abi_core.services.keyvalue.adapters.secondary.KeyValueSecondaryAdapterNATSClient as _client_module
import nats
import pytest
...
def test_init_is_lazy(monkeypatch):
    ...
    KeyValueSecondaryAdapterNATSClient("nats://127.0.0.1:4222", JWT_SECRET, "api")
...
# KeyValuePrimaryAdapterNATS encodes CallError.code.
# ---------------------------------------------------------------------------


def test_raise_for_error_maps_kv_not_found():
    with pytest.raises(KVNotFoundError):
        _raise_for_error(common_pb2.CallError(code="KV_NOT_FOUND", message="x"))


def test_raise_for_error_maps_kv_lock_timeout_with_detail():
    detail = keyvalue_pb2.KVLockTimeoutDetail(key="k", attempts=3, timeout_seconds=1.5)
    with pytest.raises(KVLockTimeoutError) as exc_info:
        _raise_for_error(
            common_pb2.CallError(code="KV_LOCK_TIMEOUT", message="x"), detail
        )
...
# ---------------------------------------------------------------------------
# Real round trip: a live NATS container, a real KeyValuePrimaryAdapterNATS
# wrapping a real PythonAdapter, and the client talking to it over the wire.
# Skips cleanly when Docker/testcontainers is unavailable.
# ---------------------------------------------------------------------------


class _PrimaryAdapterServer:
    """Runs KeyValuePrimaryAdapterNATS on its own background event loop for
    ...
    def __init__(self, nats_url: str, jwt_secret: str, wrapped_adapter) -> None:
        ...
        self._primary: KeyValuePrimaryAdapterNATS | None = None
    ...
    def start(self) -> None:
        ...
        thread = Thread(
            target=_run, daemon=True, name="keyvalue-nats-primary-test-loop"
        )
        thread.start()
    ...
    async def _start_async(self) -> None:
        ...
        self._primary = KeyValuePrimaryAdapterNATS(
...
class TestKeyValueSecondaryAdapterNATSClient(GenericKVSecondaryAdapterTest):
    """Round-trips the shared adapter contract through a real primary adapter
    (wrapping a real in-memory PythonAdapter) and the real client, over a
    live NATS server."""

    @pytest.fixture
    def adapter_class(self):
        return KeyValueSecondaryAdapterNATSClient

    @pytest.fixture
    def adapter(self, nats_url):
        wrapped = PythonAdapter()
