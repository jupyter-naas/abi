...
class DatasetSecondaryAdapterNATSClient(IDatasetPort):
        ...
        if nc is not None and self._loop is not None and self._loop.is_running():
            try:
                ...
                logger.opt(exception=True).debug(
                    "DatasetSecondaryAdapterNATSClient: error closing stale connection"
                )
            ...
            try:
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"dataset NATS RPC to {subject!r} failed"
                    ) from exc
