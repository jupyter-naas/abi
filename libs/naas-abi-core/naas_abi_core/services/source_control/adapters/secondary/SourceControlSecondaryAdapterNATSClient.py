...
class SourceControlSecondaryAdapterNATSClient(ISourceControlAdapter):
    ...
    def _ensure_loop(self) -> asyncio.AbstractEventLoop:
        ...
        thread = Thread(
            target=_run,
            daemon=True,
            name="source-control-nats-client-loop",
        )
        thread.start()
        ready.wait()
        ...
        if nc is not None and self._loop is not None and self._loop.is_running():
            try:
                ...
                logger.opt(exception=True).debug(
                    "SourceControlSecondaryAdapterNATSClient: error closing stale connection"
                )
            ...
            try:
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"source_control NATS RPC to {subject!r} failed"
                    ) from exc
    ...
    def submit_review(
        ...
        request = source_control_pb2.SubmitReviewRequest(
            context=self._context(), repo_id=repo_id, number=number, event=event, body=body
        )
        response = self._call(
