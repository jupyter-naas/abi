...
import naas_abi_core.services.email.adapters.secondary.EmailSecondaryAdapterNATSClient as _client_module
import nats
import pytest
...
def test_init_is_lazy(monkeypatch):
    ...
    EmailSecondaryAdapterNATSClient("nats://127.0.0.1:4222", JWT_SECRET, "api")
    connect.assert_not_called()


def test_close_without_connecting_is_a_noop():
    client = EmailSecondaryAdapterNATSClient("nats://127.0.0.1:4222", JWT_SECRET, "api")
...
# ---------------------------------------------------------------------------
# _to_list: normalizes the port's list[str] | str | None recipient shape
# into the plain list the wire's `repeated string` field carries.
# ---------------------------------------------------------------------------


def test_to_list_normalizes_none_to_empty_list():
    assert _to_list(None) == []


def test_to_list_normalizes_bare_string_to_one_element_list():
    assert _to_list("a@example.com") == ["a@example.com"]


def test_to_list_passes_through_a_list_unchanged():
    assert _to_list(["a@example.com", "b@example.com"]) == [
        "a@example.com",
        "b@example.com",
    ]
...
# ---------------------------------------------------------------------------
# Error mapping: EmailPorts.py declares no domain exceptions, so every
# error code the server can report -- including its own INTERNAL catch-all
# and UNAUTHENTICATED -- falls through to the generic RuntimeError.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Real round trip: a live NATS container, a real EmailPrimaryAdapterNATS
# wrapping a real FilesystemAdapter, and the client talking to it over the
# wire. Skips cleanly when Docker/testcontainers is unavailable.
# ---------------------------------------------------------------------------


class _PrimaryAdapterServer:
    """Runs EmailPrimaryAdapterNATS on its own background event loop for as
    ...
    def __init__(self, nats_url: str, jwt_secret: str, wrapped_adapter) -> None:
        ...
        self._primary: EmailPrimaryAdapterNATS | None = None
    ...
    def start(self) -> None:
        ...
        thread = Thread(target=_run, daemon=True, name="email-nats-primary-test-loop")
        thread.start()
    ...
    async def _start_async(self) -> None:
        ...
        self._primary = EmailPrimaryAdapterNATS(self._wrapped_adapter, self._jwt_secret)
...
class TestEmailSecondaryAdapterNATSClient(GenericEmailSecondaryAdapterTest):
    """Round-trips the shared adapter contract through a real primary
    adapter (wrapping a real filesystem adapter) and the real client, over a
    live NATS server."""

    @pytest.fixture
    def adapter_class(self):
        return EmailSecondaryAdapterNATSClient

    @pytest.fixture
    def adapter(self, nats_url, tmp_path):
        wrapped = FilesystemAdapter(directory=str(tmp_path))
    ...
    def test_send_round_trips_through_real_nats(
        self, adapter: EmailSecondaryAdapterNATSClient, tmp_path: Path
    ) -> None:
        adapter.send(
