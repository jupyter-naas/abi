...
reconciliation_period = st.sidebar.selectbox(
    "Reconciliation Period", ["Current Month", "Last Month", "Custom Range"]
)

if reconciliation_period == "Custom Range":
    date_range = st.sidebar.date_input(
        "Select Date Range", value=(datetime.now() - timedelta(days=30), datetime.now())
    )
...
def load_reconciliation_data():
    ...
    outstanding_items = pd.DataFrame(
        {
            "Date": pd.date_range(
                start=datetime.now() - timedelta(days=15), periods=8, freq="2D"
            ),
            "Description": [
                "Check #1234",
                "Wire Transfer",
...
reconciled_count = sum(
    [1 for acc in accounts_data.values() if acc["status"] == "Reconciled"]
)
pending_count = sum([1 for acc in accounts_data.values() if acc["status"] == "Pending"])
discrepancy_count = sum(
    [1 for acc in accounts_data.values() if acc["status"] == "Discrepancy"]
)
...
with col1:
    st.metric(
        ...
        f"${total_variance:,.0f}",
        "🟢"
        if abs(total_variance) < 1000
        else "🟡"
        if abs(total_variance) < 5000
        else "🔴",
    )
...
with col4:
    st.metric(
        "Discrepancies", discrepancy_count, "🚨" if discrepancy_count > 0 else "✅"
    )
...
with col2:
    ...
    priority_accounts = sorted(
        accounts_data.items(), key=lambda x: abs(x[1]["variance"]), reverse=True
    )
    ...
    for account, data in priority_accounts[:5]:
        ...
        st.markdown(
            f"<span style='color: {status_color}'>{data['status']}</span>",
...
styled_df = recon_df.style.map(style_variance, subset=["Variance"]).map(
...
with col2:
    ...
    fig_aging = px.scatter(
        ...
        y="Amount",
        color="Type",
        size=abs(outstanding_items["Amount"]),
...
outstanding_display["Amount"] = outstanding_display["Amount"].apply(
    lambda x: f"${x:,.0f}"
)
...
styled_outstanding = outstanding_display.style.map(
    color_age, subset=["Days_Outstanding"]
)
...
activities = [
    {
        "time": "5 min ago",
        "user": "Sarah (Treasurer)",
        "action": "Reconciled Wells Savings account",
    },
    {
        "time": "15 min ago",
        "user": "Mike (Accountant)",
        "action": "Flagged $5,000 discrepancy in BofA Investment",
    },
    {
        "time": "1 hour ago",
        "user": "Lisa (Controller)",
        "action": "Approved reconciliation for Chase Operating",
    },
    {"time": "2 hours ago", "user": "System", "action": "Auto-matched 47 transactions"},
    {
        "time": "3 hours ago",
        "user": "John (Auditor)",
        "action": "Reviewed outstanding items aging report",
    },
]
...
st.markdown("---")
st.markdown(
    "**Account Reconciliation Center** | Last Updated: "
