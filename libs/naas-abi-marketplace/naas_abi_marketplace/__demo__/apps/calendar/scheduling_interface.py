...
view_type = st.sidebar.selectbox(
    "Calendar View", ["Week View", "Month View", "Day View", "Agenda View"]
)
...
today_events = len(
    filtered_events[filtered_events["Date"].dt.date == datetime.now().date()]
)
this_week_events = len(
    filtered_events[
...
high_priority = len(
    filtered_events[filtered_events["Priority"].isin(["High", "Critical"])]
)
...
if view_type == "Week View":
    ...
    day_events = filtered_events[
        filtered_events["Date"].dt.date == selected_date
    ...
    if len(day_events) == 0:
        ...
        for _, event in day_events.iterrows():
            ...
            with col3:
                ...
                st.markdown(
                    f"<span style='color: {status_color}'>{event['Status']}</span>",
                    unsafe_allow_html=True,
                )
...
with col1:
    ...
    for room_info in room_status:
        ...
        st.markdown(
            f"<span style='color: {status_color}'>{room_info['Status']}</span> | Next: {room_info['Next_Available']}",
            unsafe_allow_html=True,
        )
        st.markdown("---")
...
with col1:
    ...
    fig_types = px.pie(
        values=type_counts.values, names=type_counts.index, title="Events by Type"
    )
...
activities = [
    {
        "time": "5 min ago",
        "user": "Sarah",
        "action": "Scheduled client call for tomorrow 2 PM",
    },
    {
        "time": "15 min ago",
        "user": "Mike",
        "action": "Booked Conference Room A for team meeting",
    },
    {
        "time": "30 min ago",
        "user": "Lisa",
        "action": "Cancelled project review meeting",
    },
    {"time": "1 hour ago", "user": "John", "action": "Moved interview to next week"},
    {
        "time": "2 hours ago",
        "user": "Emma",
        "action": "Created recurring standup meetings",
    },
]
...
st.markdown("---")
st.markdown(
    f"**Scheduling Center** | Role: {user_role} | Last Updated: "
