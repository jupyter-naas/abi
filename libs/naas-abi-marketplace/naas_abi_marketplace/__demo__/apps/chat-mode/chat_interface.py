...
# Page config
st.set_page_config(page_title="ABI Chat", page_icon="🤖", layout="centered")
...
def process_user_input(user_input):
    ...
    if mention_match:
        ...
        if mentioned_agent in AGENT_MAPPING:
            ...
            st.write(
                f"🔍 **@mention detected:** Switching to {AGENT_MAPPING[mentioned_agent]}"
            )
...
def send_message(user_input):
    ...
    try:
        st.write(
            f"🔍 **Sending Debug:** Input='{user_input}', Active Agent='{st.session_state.active_agent}'"
        )
