...
# Page config
st.set_page_config(page_title="ABI Chat (API)", page_icon="💬", layout="centered")
...
def get_postgres_connection():
    ...
    try:
        ...
        conn = psycopg.Connection.connect(
            postgres_url, autocommit=True, prepare_threshold=0, row_factory=dict_row
        )
        return conn
            ...
            for thread in threads:
                ...
                for checkpoint in checkpoints:
                    try:
                        ...
                        data = (
                            json.loads(checkpoint["checkpoint"])
                            if isinstance(checkpoint["checkpoint"], str)
                            else checkpoint["checkpoint"]
                        )

                        # Look for messages in the checkpoint data
                        if (
                            "channel_values" in data
                            and "messages" in data["channel_values"]
                        ):
        ...
        with conn.cursor() as cur:
            ...
            for checkpoint_data in checkpoints:
                try:
                    ...
                    data = (
                        json.loads(checkpoint_data["checkpoint"])
                        if isinstance(checkpoint_data["checkpoint"], str)
                        else checkpoint_data["checkpoint"]
                    )
...
def call_abi_api(agent_name: str, prompt: str, thread_id: int = 1) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "success": False,
                "error": "🔒 Authentication failed. Check your ABI_API_KEY.",
            }
        ...
        return {
            "success": False,
            "error": f"❌ Cannot connect to ABI API at {ABI_API_BASE}.\n\n💡 **To fix this:**\n1. Open a terminal in the project root\n2. Run: `make api`\n3. Wait for the API to start, then try again",
        }
    except requests.exceptions.Timeout:
        return {
            "success": False,
            "error": f"⏱️ Timeout calling {agent_name} agent (>2 minutes). The agent might be processing a complex request. Try a simpler question or try again later.",
        }
    except Exception as e:
    ...
    if msg["role"] == "assistant":
        ...
        if agent_name == "system":
            ...
            with st.chat_message("assistant", avatar=avatar_url):
                st.markdown(
                    f'<div class="assistant-label">{agent_name}:</div>',
                    unsafe_allow_html=True,
                )
