...
user_role = st.sidebar.selectbox(
    "Select Your Role", ["Treasurer", "Financial Controller", "Accountant", "CFO"]
)
...
time_period = st.sidebar.selectbox(
    "Time Period", ["Current Month", "Last 3 Months", "YTD", "Last 12 Months"]
)
...
def load_financial_data():
    ...
    monthly_dates = pd.date_range(
        start=datetime.now() - timedelta(days=365), end=datetime.now(), freq="ME"
    )
    ...
    pnl_data["EBITDA"] = (
        pnl_data["Revenue"] + pnl_data["COGS"] + pnl_data["Operating_Expenses"]
    )
...
with col1:
    ...
    st.metric(
        "Cash Position", f"${current_cash:,.0f}", f"{np.random.uniform(-5, 10):.1f}%"
    )
...
with col3:
    ...
    st.metric(
        "Monthly EBITDA",
        f"${monthly_ebitda:,.0f}",
        f"{np.random.uniform(-10, 15):.1f}%",
    )

with col4:
    ebitda_margin = (monthly_ebitda / monthly_revenue) * 100
    st.metric(
        "EBITDA Margin", f"{ebitda_margin:.1f}%", f"{np.random.uniform(-2, 3):.1f}%"
    )
...
if "cash_flow" in current_permissions:
    ...
    with col1:
        ...
        fig_cash.update_layout(
            title="Cash Flow Trends", xaxis_title="Date", yaxis_title="Cash ($)"
        )
    ...
    with col2:
        # Cash flow breakdown
        latest_flows = {
            "Operating": cash_data["Operating_Cash"].iloc[-1]
            - cash_data["Operating_Cash"].iloc[-30],
            "Investing": cash_data["Investing_Cash"].iloc[-1]
            - cash_data["Investing_Cash"].iloc[-30],
            "Financing": cash_data["Financing_Cash"].iloc[-1]
            - cash_data["Financing_Cash"].iloc[-30],
        }

        fig_breakdown = px.bar(
            x=list(latest_flows.keys()),
            y=list(latest_flows.values()),
...
if "budgets" in current_permissions:
    ...
    budget_data["Variance_Pct"] = (
        budget_data["Variance"] / abs(budget_data["Budget"])
    ) * 100
...
if "projects" in current_permissions:
    ...
    with col2:
        ...
        for _, project in projects.iterrows():
            ...
            st.markdown(
                f"<span style='color: {color}'>{project['Status']}</span> - {utilization:.1f}% utilized",
    ...
    with col1:
        ...
        for _, account in reconciliation_status.iterrows():
            status_color = {
                ...
                "Pending": "orange",
                "Discrepancy": "red",
            }.get(account["Status"], "gray")
            st.markdown(f"**{account['Account']}**")
            st.markdown(
                f"<span style='color: {status_color}'>{account['Status']}</span> | Last: {account['Last_Reconciled'].strftime('%Y-%m-%d')}",
                unsafe_allow_html=True,
            )
...
alerts = [
    {
        "type": "warning",
        "message": "Cash flow projection shows potential shortfall in 45 days",
    },
...
st.markdown("---")
st.markdown(
    f"**Financial Dashboard** | Role: {user_role} | Last Updated: "
