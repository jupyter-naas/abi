...
def parse_ttl_file(file_path):
    ...
    try:
        ...
        return {"error": str(e), "triples": [], "namespaces": {}, "count": 0}
...
# File selection
filtered_files = [f for f in filtered_by_category if f["module"] in selected_modules]
...
st.session_state.selected_files = [
    f for f in filtered_files if f["path"] in selected_file_paths
]
            ...
            with st.spinner("Creating knowledge graph..."):
                try:
                    net, file_stats, node_count, edge_count = create_knowledge_graph(
                        st.session_state.selected_files, max_nodes
                    )
                    ...
                    st.success(
                        f"✅ Generated graph with {node_count} nodes and {edge_count} edges"
                    )
