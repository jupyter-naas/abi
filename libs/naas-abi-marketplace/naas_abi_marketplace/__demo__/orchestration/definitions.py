...
def my_asset(context: dagster.AssetExecutionContext, config: MyAssetConfig):
    from naas_abi_core.utils.Storage import ensure_data_directory
    ...
    query_term = (
        sensor_name.replace("my_sensor_", "")
        if sensor_name.startswith("my_sensor_")
        else "unknown"
    )
    ...
    timestamp = datetime.strptime(
        config.entry["published"], "%a, %d %b %Y %H:%M:%S %Z"
