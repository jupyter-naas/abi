...
class OpenRouterAPIIntegration(Integration):
    ...
    def create_response(
        ...
        """
        payload = {
            "input": [{"type": "message", "role": "user", "content": input_prompt}],
            "tools": tools,
            "model": model,
    ...
    def get_model_parameters(self, author: str, slug: str) -> Dict:
        ...
        return self._make_request(
            "GET", "/parameters", params={"author": author, "slug": slug}
        )
    ...
    def list_providers(self, save_json: bool = True) -> List:
        ...
        providers = providers.get("data", []) if isinstance(providers, dict) else []
        if save_json:
            self.__storage_utils.save_json(
                providers,
                os.path.join(self.__configuration.datastore_path, "providers"),
                "providers.json",
            )
        return providers
