...
@dataclass
class FinancialReportingWorkflowConfiguration(WorkflowConfiguration):
    """Configuration for FinancialReportingWorkflow"""
...
class FinancialReportingWorkflow(Workflow):
    ...
    def __init__(
        self, config: Optional[FinancialReportingWorkflowConfiguration] = None
    ):
        """Initialize FinancialReportingWorkflow - NOT FUNCTIONAL YET"""
        super().__init__(config or FinancialReportingWorkflowConfiguration())
        logger.warning(
            "🚧 FinancialReportingWorkflow is not functional yet - template only"
        )
