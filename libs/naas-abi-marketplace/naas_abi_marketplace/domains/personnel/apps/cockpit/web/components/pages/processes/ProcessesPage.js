...
function bfoSlideSvg(proc, design) {
  const b = proc.buckets || {};
  const node = (key, cx, cy, accent) => {
    const meta = b[key] || {};
    const stroke = accent ? design.accent : design.node_stroke;
    const fill = accent ? design.accent_fill : design.node_fill;
    const wordFill = accent ? design.accent : design.text;
    ...
    return `
      <g id="node-${key}">
        ${aboveLines
          .map(
            (line, i) =>
              `<text x="${cx}" y="${aboveY + i * 13}" fill="${design.text}" font-size="12" font-weight="500" text-anchor="middle">${esc(line)}</text>`
          )
          .join("")}
...
<svg class="slide-canvas" viewBox="0 0 1280 720" width="1280" height="720" role="img"
  ...
  font-family="${esc(design.font_family)}">
  <title>${esc(proc.title)}</title>
  <defs>
    <marker id="arrow-bfo-${esc(proc.id)}" markerUnits="userSpaceOnUse" markerWidth="9" markerHeight="8" refX="8" refY="4" orient="auto"><path d="M0,0 L9,4 L0,8 Z" fill="${design.accent}"></path></marker>
    <marker id="arrow-bfo-muted-${esc(proc.id)}" markerUnits="userSpaceOnUse" markerWidth="9" markerHeight="8" refX="8" refY="4" orient="auto"><path d="M0,0 L9,4 L0,8 Z" fill="${design.muted}"></path></marker>
  </defs>
  <rect width="1280" height="720" fill="${design.canvas}"></rect>
  <g id="realms">
    <rect x="0" y="130" width="1280" height="170" fill="${design.occurrents}"></rect>
    <rect x="0" y="300" width="1280" height="360" fill="${design.continuants}"></rect>
    <line x1="0" y1="300" x2="1280" y2="300" stroke="${design.divider}" stroke-width="1"></line>
    <text x="40" y="152" font-size="12" fill="${design.realm_label}" font-weight="600" letter-spacing="2">OCCURRENTS</text>
    <text x="40" y="322" font-size="12" fill="${design.realm_label}" font-weight="600" letter-spacing="2">CONTINUANTS</text>
  </g>
  <g id="arrows" fill="none" stroke-width="1.5">
    <path d="M652,215 H1094" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M553,237 L189,407" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M572,259 L490,385" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M610,266 L630,376" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M642,246 L851,398" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M648,234 L1100,410" stroke="${design.accent}" marker-end="url(#arrow-bfo-${esc(proc.id)})"></path>
    <path d="M192,430 H404" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
    <path d="M843,430 H695" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
    <path d="M124,478 V560 H620 V481" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
    <path d="M140,482 V590 H880 V483" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
    <path d="M156,478 V620 H1150 V485" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
    <path d="M1122,474 V545 H660 V481" stroke="${design.muted}" marker-end="url(#arrow-bfo-muted-${esc(proc.id)})"></path>
  </g>
  <g id="arrow-labels" font-size="12" fill="${design.muted}" font-weight="500">
    ...
    <rect x="344" y="551" width="92" height="16" fill="${design.continuants}"></rect>
    <text x="390" y="564" text-anchor="middle">is carrier of</text>
    <rect x="464" y="581" width="72" height="16" fill="${design.continuants}"></rect>
    <text x="500" y="594" text-anchor="middle">bearer of</text>
    <rect x="836" y="611" width="128" height="16" fill="${design.continuants}"></rect>
    <text x="900" y="624" text-anchor="middle">material basis of*</text>
    <rect x="947" y="537" width="86" height="16" fill="${design.continuants}"></rect>
  ...
  <g id="footer" font-size="13" fill="${design.realm_label}">
...
export function mountProcessesPage(el, data, design) {
...
export async function mountPage(el, ctx) {
  ...
  return mountProcessesPage(el, data, ctx.config.theme.process_slide);
}
