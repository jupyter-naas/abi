...
def test_section_query_includes_slug_filter_for_bulk_queries() -> None:
    ...
    assert payload["label"] == "Find Working Experiences"
...
def test_competency_queries_are_deduped_per_profile() -> None:
    ...
    assert "find_working_experiences" in names
    assert len(names) == 8
...
def test_sources_lists_both_queries() -> None:
    ...
    assert payload["label"] == "Find Profile Header · Find Working Experiences"
    assert "# find_profile_header" in payload["text"]
    assert "# find_working_experiences" in payload["text"]
