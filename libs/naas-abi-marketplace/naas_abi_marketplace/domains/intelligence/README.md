...
│   └── WSRAgent.py
├── apps/
│   └── wsr/                            # app, not a module — manifest.json + dashboard
└── ontologies/
...
| `WSRAgent` | `agents/` | Situational-awareness Q&A over the WSR data layers |
...
### WSR is an app, not a module

Only `naas_abi_marketplace.domains.intelligence` is loaded. `apps/wsr/` carries no `ABIModule`:
the app is discovered from its `manifest.json` (app id
`naas_abi_marketplace.domains.intelligence:wsr`), its agent sits flat in `agents/` like every other
bucket agent, and its credentials are a nested `wsr:` block on this module's config —
`WSRConfiguration` in [`__init__.py`](__init__.py).
...
This is the general shape for bucket apps: `<bucket>/apps/<app>/manifest.json` plus a config field
on the bucket, rather than a module per app. `finance/apps/financial_cockpit/` follows it too.
