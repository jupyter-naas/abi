...
function sourceAttribution(camera: CCTVCamera): string {
  const url = camera.videoUrl ?? '';
  if (url.includes('youtube')) return 'YouTube Live';
  if (url.includes('ipcamlive')) return 'IPCamLive';
  if (url.includes('livelebcams')) return 'LebCam — Lebanon';
  if (url.includes('skylinewebcams')) return 'SkylineWebcams';
  return `SRC: ${camera.country?.toUpperCase() ?? 'THEATER'}`;
}
...
function proxyUrl(url: string) {
  if (!url) return '';
  return `${_API_BASE}/api/cctv/snapshot?url=${encodeURIComponent(url)}`;
}

const SOURCE_LABELS: Record<string, string> = {
  nyc: '511NY.ORG',
  london: 'TFL.GOV.UK',
  openwebcamdb: 'OPEN SOURCE',
  mideast: 'THEATER CAM',
};
...
export default function CCTVPanel({ camera, onClose }: CCTVPanelProps) {
  ...
  const resolveStream = useCallback(async () => {
    ...
    setStatus('resolving');
    try {
      const res = await fetch(`${_API_BASE}/api/webcams/stream?slug=${encodeURIComponent(camera.slug)}`);
  ...
  useEffect(() => {
    ...
    const onError = () => {
      // Set fallback imgSrc but do NOT mark as 'live' yet — let the <img>
      // onLoad handler do that once the image has actually loaded.
      if (camera.imageUrl) { setImgSrc(proxyUrl(camera.imageUrl)); }
    ...
    >
      ...
      <div className="px-3 py-1 border-b border-pink-500/10 space-y-0.5">
        ...
        <div className="flex gap-3 text-pink-400/50 text-[10px]">
          <span>{(camera.country ?? camera.city ?? 'THEATER').toUpperCase()}</span>
      ...
      <div className="relative bg-black" style={{ height: 214 }}>
        ...
        {/* ── MP4 JPEG fallback — shown whenever imgSrc is set (video error path) */}
      ...
      <div className="px-3 py-1.5 flex items-center justify-between text-[9px] border-t border-pink-500/10">
        <span className="text-pink-400/40">
          {camera.id.startsWith('me-')
            ? <span>{sourceAttribution(camera)}</span>
