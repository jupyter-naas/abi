...
interface GeoSearchProps {
  ...
  /** When true, renders a compact borderless variant for embedding inside a navbar */
  compact?: boolean;
}
...
const QUICK_LINKS: GeoResult[] = [
  ...
  // ── Active Theater ──────────────────────────────────────────────────────
  { displayName: '⚠ Middle East Theater', shortName: 'Middle East Theater', country: '', type: 'region', lat: 30, lon: 40, boundingBox: [20, 40, 25, 60] },
  { displayName: '⚠ Tel Aviv, Israel', shortName: 'Tel Aviv', country: 'Israel', type: 'city', lat: 32.0853, lon: 34.7818, boundingBox: [31.9, 32.2, 34.6, 34.9] },
  { displayName: '⚠ Jerusalem, Israel', shortName: 'Jerusalem', country: 'Israel', type: 'city', lat: 31.7683, lon: 35.2137, boundingBox: [31.7, 31.85, 35.15, 35.32] },
  { displayName: '⚠ Haifa, Israel', shortName: 'Haifa', country: 'Israel', type: 'city', lat: 32.7940, lon: 34.9896, boundingBox: [32.74, 32.85, 34.93, 35.07] },
  { displayName: '⚠ Tehran, Iran', shortName: 'Tehran', country: 'Iran', type: 'city', lat: 35.6892, lon: 51.3890, boundingBox: [35.55, 35.85, 51.1, 51.65] },
  { displayName: '⚠ Natanz Nuclear Site', shortName: 'Natanz', country: 'Iran', type: 'building', lat: 33.7240, lon: 51.7270, boundingBox: [33.70, 33.75, 51.70, 51.76] },
  { displayName: '⚠ Beirut, Lebanon', shortName: 'Beirut', country: 'Lebanon', type: 'city', lat: 33.8938, lon: 35.5018, boundingBox: [33.83, 33.94, 35.44, 35.58] },
  { displayName: '⚠ Strait of Hormuz', shortName: 'Strait of Hormuz', country: 'International', type: 'water', lat: 26.594, lon: 56.451, boundingBox: [25.8, 27.3, 55.5, 57.5] },
  { displayName: '⚠ Al Udeid Air Base, Qatar', shortName: 'Al Udeid AB', country: 'Qatar (US)', type: 'aeroway', lat: 25.117, lon: 51.314, boundingBox: [25.09, 25.15, 51.27, 51.35] },
  { displayName: '⚠ Doha, Qatar', shortName: 'Doha', country: 'Qatar', type: 'city', lat: 25.2854, lon: 51.5310, boundingBox: [25.2, 25.4, 51.4, 51.65] },
...
export default function GeoSearch({ onSelect, compact = false }: GeoSearchProps) {
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      const res = await fetch(url, { headers: { 'Accept-Language': 'en', 'User-Agent': 'WSR/1.0' } });
      if (!res.ok) return;
  ...
  return (
    <div ref={containerRef} className="relative font-mono" style={{ width: compact ? '100%' : 420 }}>
      {/* Search input */}
      <div
        className={`flex items-center gap-2 px-3 transition-all ${compact ? 'py-0 h-full' : 'py-2 border border-green-500/30'}`}
        style={compact ? {
      ...
      {showDropdown && (
        <div
          className="absolute top-full left-0 right-0 z-50 border border-green-500/20 overflow-hidden"
          style={{
            background: 'rgba(0,4,0,0.97)',
            backdropFilter: 'blur(12px)',
            maxHeight: 320,
            borderTop: compact ? '1px solid rgba(0,255,65,0.2)' : 'none',
          }}
        >
          {!query.trim() && (
            <div className="px-3 py-1.5 border-b border-green-500/10 flex items-center justify-between gap-2">
              <span className="text-[9px] text-green-500/30 tracking-widest uppercase">Quick Navigation</span>
              <span className="text-[9px] text-orange-500/60 tracking-widest animate-pulse">⚠ ACTIVE THEATER</span>
            </div>
          )}
