...
2. **Data** — `web/data/globals/admin/<name>.json`, shape
   ...
   `scripts/generate_admin_settings.py`; add the name to `AdminSettingsName` in
...
`generate_admin_settings.py` runs last, off the ledger, the cost centers and the
...
| `/admin/*` | `generate_admin_settings.py` | `lib/admin/settings.ts` | — | `admin/AdminSettingsPage.tsx` |
