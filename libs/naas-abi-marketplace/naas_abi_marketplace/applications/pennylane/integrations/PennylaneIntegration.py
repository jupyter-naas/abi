...
class PennylaneIntegration(Integration):
    ...
    def list_customers(
        ...
        filter_str = "_".join([f"filter_[{json.dumps(f)}]" for f in filters]).replace(
            " ", "_"
        )
    ...
    @cache(
        lambda self, customer_id: "get_customer_" + customer_id,
    ...
    @cache(
        lambda self, invoice_id: "get_customer_invoice_" + invoice_id,
        cache_type=DataType.JSON,
        ttl=timedelta(days=1),
    )
...
def as_tools(configuration: PennylaneIntegrationConfiguration):
    ...
    return [
        ...
        StructuredTool(
            ...
            func=lambda start_date, customer_id: integration.list_customer_invoices(
