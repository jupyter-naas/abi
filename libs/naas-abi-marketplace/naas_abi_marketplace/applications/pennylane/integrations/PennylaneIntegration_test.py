...
def test_list_customer_invoices(pennylane_integration: PennylaneIntegration):
    ...
    assert len(customer_invoices) > 0, (
        f"Expected more than 0 customer invoices, got {len(customer_invoices)}"
    )
...
def test_list_customer_invoices_with_filters(
    ...
    assert len(customer_invoices) == 0, (
        f"Expected 0 customer invoices, got {len(customer_invoices)}"
    )
...
def test_list_categories(pennylane_integration: PennylaneIntegration):
    ...
    assert len(categories) > 0, (
        f"Expected more than 0 categories, got {len(categories)}"
    )
