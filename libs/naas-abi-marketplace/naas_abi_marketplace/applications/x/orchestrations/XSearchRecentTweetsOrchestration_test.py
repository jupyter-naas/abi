"""Tests for XSearchRecentTweetsOrchestration default trigger status."""
...
from naas_abi_marketplace.applications.x.orchestrations.XSearchRecentTweetsOrchestration import (
    XSearchRecentTweetsOrchestration,
...
def test_definitions_expose_running_triggers_for_configured_workflows():
    ...
    with patch(
        ...
        "XSearchRecentTweetsOrchestration.ABIModule.get_instance",
        return_value=module,
    ):
        orch = XSearchRecentTweetsOrchestration.New()
    ...
    assert (
        sensor_by_name["x_search_recent_tweets_sensor_ai_llms"].default_status
