...
```bash
pnpm install
pnpm dev      # http://localhost:3045/app-html/x/apps/x_proxy/
pnpm build    # static export → out/
pnpm typecheck
```

`next.config.js` uses `output: 'export'` and
`basePath: '/app-html/x/apps/x_proxy'` so the build can be published into object
storage and served by `../routes.py` next to the JSON snapshots.

Snapshot JSON is loaded at runtime from the same `/app-html/x/apps/x_proxy/` origin
