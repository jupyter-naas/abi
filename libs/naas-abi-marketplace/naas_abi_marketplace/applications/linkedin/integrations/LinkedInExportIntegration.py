    ...
    def unzip_export(self) -> Dict[str, Any]:
        ...
        if not zip_path.exists():
            raise FileNotFoundError(
                f"ZIP file not found: {self.__configuration.export_file_path}"
            )

        if not zipfile.is_zipfile(zip_path):
            raise ValueError(
                f"File is not a valid ZIP file: {self.__configuration.export_file_path}"
            )
                    ...
                    if member_path.exists():
                        if member_path.is_file():
                            files_extracted.append(
                                str(member_path.relative_to(extract_dir))
                            )
...
def as_tools(configuration: LinkedInExportIntegrationConfiguration):
    ...
    class ReadCsvSchema(BaseModel):
        csv_file_name: str = Field(..., description="Name of the CSV file to read")
        sep: str = Field(",", description="CSV separator character")
