...
class PowerPointIntegration(Integration):
    ...
    def save_presentation(
        self, presentation: PresentationType, output_path: str
    ) -> None:
    ...
    def get_shapes_from_slide(
        ...
        for shape in presentation.slides[slide_number].shapes:
            data = {
                ...
                "shape_id": shape.shape_id,
                "shape_type": shape.shape_type,
                "shape_alt_text": shape._element._nvXxPr.cNvPr.attrib.get("descr", ""),
                "text": shape.text if hasattr(shape, "text") else "",
                "left": shape.left.cm if hasattr(shape, "left") else None,
                "top": shape.top.cm if hasattr(shape, "top") else None,
                "width": shape.width.cm if hasattr(shape, "width") else None,
        ...
        if text is not None:
            ...
            if lines:
                ...
                if lines[0].startswith("-") or lines[0].startswith(" "):
                    lines[0] = lines[0][1:]
                first_par = self.__apply_formatted_text(
                    first_par, lines[0], saved_formatting
                )
                first_par.bullet = False
                first_par.level = 0
    ...
    def replace_table(
        ...
        if not table_shape:
            raise ValueError(
                f"Table with ID {shape_id} not found on slide {slide_index}"
            )
    ...
    def duplicate_slide(
        ...
        for s in src_slide.shapes:
            ...
            try:
                if s.shape_type == MSO_SHAPE_TYPE.PICTURE or (
                    s.shape_type == MSO_SHAPE_TYPE.PLACEHOLDER and shape_text == ""
                ):
...
def as_tools(configuration: PowerPointIntegrationConfiguration):
    ...
    class GetShapesFromSlideSchema(BaseModel):
        slide_number: int = Field(
            ..., description="Index of the slide to get shapes from"
        )
