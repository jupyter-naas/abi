...
class AgicapIntegration(Integration):
    ...
    def get_transactions(
        ...
        while True:
            logger.info(
                f"Fetching transactions for account {account_id} with skip {skip} and take {take}"
            )
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    f"Failed to fetch transactions for account {account_id} with skip {skip} and take {take}"
                )
                break
...
def as_tools(configuration: AgicapIntegrationConfiguration):
    ...
    return [
        ...
        StructuredTool.from_function(
            ...
            func=lambda company_id, account_id: integration.get_balance(
