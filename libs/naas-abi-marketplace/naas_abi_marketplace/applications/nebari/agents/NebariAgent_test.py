...
def test_intents(agent):
    ...
    for intent in intents:
        if (
            intent.intent_type == IntentType.RAW
            and intent.intent_scope != IntentScope.DIRECT
        ):
            result = agent.invoke(intent.intent_value)
            assert result is not None, (
                f"Intent: {intent.intent_value} did not return a result."
            )
            assert intent.intent_target in result, (
                f"For intent '{intent.intent_value}', expected '{intent.intent_target}' in result: {result}"
            )
