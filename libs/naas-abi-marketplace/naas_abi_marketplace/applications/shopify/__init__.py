...
class ABIModule(BaseModule):
    ...
    class Configuration(ModuleConfiguration):
        ...
        module: naas_abi_marketplace.applications.shopify
        enabled: true
        config:
            shop: "{{ secret.SHOPIFY_SHOP }}"
            client_id: "{{ secret.SHOPIFY_CLIENT_ID }}"
            client_secret: "{{ secret.SHOPIFY_CLIENT_SECRET }}"
            api_version: "2026-07"

        `shop` accepts either the subdomain ("my-store") or the full domain
        ("my-store.myshopify.com"). `client_id` / `client_secret` are the
        Shopify app credentials exchanged for a short-lived Admin API access
        token through the OAuth client_credentials grant.
        """
