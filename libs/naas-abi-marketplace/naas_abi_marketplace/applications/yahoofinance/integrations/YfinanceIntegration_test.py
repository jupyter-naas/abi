...
def test_get_ticker_info(integration: YfinanceIntegration):
    ...
    assert "Microsoft" in result.get("longName", "") or "Microsoft" in result.get(
        "shortName", ""
    )
...
def test_search_ticker(integration: YfinanceIntegration):
    ...
    assert len(found_fields) > 0, (
        "Search result should contain at least one expected field"
    )

    # Should find MSFT in results
    msft_found = any(result_item.get("symbol") == "MSFT" for result_item in result)
