...
class YfinanceIntegration(Integration):
    ...
    @cache(
        lambda self, symbol: f"ticker_info_{symbol}",
    ...
    @cache(
        lambda self, symbol, period: f"ticker_history_{symbol}_{period}",
        cache_type=DataType.JSON,
    ...
    @cache(
        lambda self, symbol: f"ticker_financials_{symbol}",
        cache_type=DataType.JSON,
    ...
    def get_ticker_financials(self, symbol: str) -> Dict:
        ...
        try:
            ...
            quarterly_income_stmt = self._result_df_to_dict(
                ticker.quarterly_income_stmt
            )

            # Get calendar and analyst price targets
            calendar = (
                self._convert_to_json(ticker.calendar)
    ...
    @cache(
        lambda self, sector_key: f"sector_info_{sector_key}",
        cache_type=DataType.JSON,
    ...
    def get_sector_info(self, sector_key: str) -> Dict:
        ...
        try:
            ...
            sector_info = {
                ...
                "top_companies": self._result_df_to_dict(sector.top_companies)
                if hasattr(sector, "top_companies")
                else [],
                "research_reports": sector.research_reports
                if hasattr(sector, "research_reports")
                else [],
                "top_etfs": sector.top_etfs if hasattr(sector, "top_etfs") else [],
                "top_mutual_funds": sector.top_mutual_funds
                if hasattr(sector, "top_mutual_funds")
                else [],
                "industries": self._result_df_to_dict(sector.industries)
    ...
    @cache(
        lambda self, industry_key: f"industry_info_{industry_key}",
        cache_type=DataType.JSON,
        ttl=datetime.timedelta(hours=2),
    )
    def get_industry_info(self, industry_key: str) -> Dict:
        ...
        try:
            ...
            industry_info = {
                ...
                if hasattr(industry, "sector_key")
                else "",
                "sector_name": industry.sector_name
                if hasattr(industry, "sector_name")
                else "",
                "top_performing_companies": self._result_df_to_dict(
                    industry.top_performing_companies
                )
                if hasattr(industry, "top_performing_companies")
                else [],
                "top_growth_companies": self._result_df_to_dict(
                    industry.top_growth_companies
                )
                if hasattr(industry, "top_growth_companies")
                else [],
            }
    ...
    @cache(
        lambda self, company_name: f"search_{company_name.replace(' ', '_')}",
        cache_type=DataType.JSON,
        ttl=datetime.timedelta(hours=1),
    )
...
def as_tools(configuration: YfinanceIntegrationConfiguration):
    ...
    class TickerSymbolSchema(BaseModel):
        symbol: str = Field(
            ..., description="Stock ticker symbol (e.g., 'AAPL', 'ALO.PA', 'TSLA')"
        )
    ...
    class SectorKeySchema(BaseModel):
        sector_key: str = Field(
            ...,
            description="Sector identifier (e.g., 'technology', 'healthcare', 'financials', 'industrials')",
        )

    class IndustryKeySchema(BaseModel):
        industry_key: str = Field(
            ...,
            description="Industry identifier (e.g., 'software', 'biotech', 'banks', 'railroads')",
        )

    class CompanySearchSchema(BaseModel):
        company_name: str = Field(
            ..., description="Company name to search for ticker symbol"
        )
