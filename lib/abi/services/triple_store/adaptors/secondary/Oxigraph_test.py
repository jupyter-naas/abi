...
class TestOxigraphIntegration:
    ...
    def generate_graph(self, num_triples: int, base_uri: str = "http://test.example.org/"):
    ...
    def test_large_graph_insert(self, oxigraph_adapter):
        ...
        try:
            ...
            count_query = f"SELECT (COUNT(*) as ?count) WHERE {{ ?s a <{base_uri}/TestEntity> }}"
    ...
    def test_filter_query(self, oxigraph_adapter):
        ...
        try:
            ...
            for i in range(10):
                ...
                g.add((subject, RDF.type, URIRef("http://test.example.org/filter/Number")))
                g.add((subject, URIRef("http://test.example.org/filter/value"), Literal(i)))
    ...
    def test_optional_clause(self, oxigraph_adapter):
        ...
        try:
            ...
            for i in range(5):
                ...
                if i % 2 == 0:
                    g.add((subject, URIRef("http://test.example.org/optional"), Literal(f"value{i}")))
    ...
    def test_aggregation_functions(self, oxigraph_adapter):
        ...
        try:
            ...
            for i, val in enumerate(values):
                ...
                g.add((subject, URIRef("http://test.example.org/agg/value"), Literal(val)))
        ...
        try:
            ...
            for cat in categories:
                for i in range(3 if cat == "A" else 2 if cat == "B" else 1):
                    ...
                    g.add((subject, URIRef("http://test.example.org/groupby/category"), Literal(cat)))
    ...
    def test_blank_nodes_are_filtered(self, oxigraph_adapter):
        ...
        try:
            ...
            g.add((address, URIRef(f"http://{domain}/bnode/street"), Literal("123 Main St")))
    ...
    def test_special_characters_in_literals(self, oxigraph_adapter):
        ...
        try:
            ...
            for i, text in enumerate(special_strings):
                g.add((subject, URIRef(f"http://test.example.org/prop{i}"), Literal(text)))
    ...
    def test_different_datatypes(self, oxigraph_adapter):
        ...
        try:
            ...
            g.add((subject, URIRef("http://test.example.org/integer"), Literal(42, datatype=XSD.integer)))
            g.add((subject, URIRef("http://test.example.org/float"), Literal(3.14, datatype=XSD.float)))
            g.add((subject, URIRef("http://test.example.org/boolean"), Literal(True, datatype=XSD.boolean)))
            g.add((subject, URIRef("http://test.example.org/date"), Literal("2024-01-01", datatype=XSD.date)))
            g.add((subject, URIRef("http://test.example.org/string"), Literal("test string", datatype=XSD.string)))
    ...
    def test_language_tags(self, oxigraph_adapter):
        ...
        try:
            ...
            g.add((subject, URIRef("http://test.example.org/label"), Literal("Hello", lang="en")))
            g.add((subject, URIRef("http://test.example.org/label"), Literal("Bonjour", lang="fr")))
            g.add((subject, URIRef("http://test.example.org/label"), Literal("Hola", lang="es")))
            g.add((subject, URIRef("http://test.example.org/label"), Literal("你好", lang="zh")))
    ...
    def test_update_operations(self, oxigraph_adapter):
        ...
        try:
            ...
            g.add((subject, URIRef("http://test.example.org/value"), Literal("initial")))
            ...
            # Cleanup - remove all
            cleanup_graph = Graph()
            cleanup_graph.add((subject, URIRef("http://test.example.org/value"), Literal("updated")))
    ...
    def test_batch_insertions(self, oxigraph_adapter):
        ...
        try:
            ...
            for batch in range(5):
                ...
                for i in range(100):
                    subject = URIRef(f"http://test.example.org/batches/batch{batch}/item{i}")
                    g.add((subject, RDF.type, URIRef("http://test.example.org/batches/Item")))
                    g.add((subject, URIRef("http://test.example.org/batches/batch"), Literal(batch)))
