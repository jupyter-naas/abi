...
from src.core.modules.powerpoint.workflows.UpdateOrganizationSlidesWorkflow import (
    UpdateOrganizationSlidesWorkflow,
    UpdateOrganizationSlidesWorkflowConfiguration,
)
...
def create_agent(
    ...
    if (
        secret.get("NAAS_API_KEY")
        and config.workspace_id != ""
        and config.storage_name != ""
        and secret.get("OPENAI_API_KEY")
    ):
        updateOrganizationSlidesWorkflow = UpdateOrganizationSlidesWorkflow(
            UpdateOrganizationSlidesWorkflowConfiguration(
    ...
    # Set model
    model = ChatOpenAI(
        model="gpt-4o", api_key=secret.get("OPENAI_API_KEY"), temperature=0
    )
    # Add agents
