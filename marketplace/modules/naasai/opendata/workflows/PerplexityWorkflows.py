...
class PerplexityOrganizationParameters(WorkflowParameters):
    ...
    data_store_name: Optional[str] = Field(
        "storage/datalake/opendata/organizations",
        description="Data store name to store the data",
    )
    details: bool = Field(
        False, description="Whether to include details in the response"
    )
...
class PerplexityMarketDetailsParameters(WorkflowParameters):
    ...
    data_store_name: Optional[str] = Field(
        "storage/datalake/opendata/markets",
        description="Data store name to store the data",
    )
    details: bool = Field(
        False, description="Whether to include details in the response"
...
class PerplexityIndustryDetailsParameters(WorkflowParameters):
    ...
    data_store_name: Optional[str] = Field(
        "storage/datalake/opendata/industries",
        description="Data store name to store the data",
    )
    details: bool = Field(
        True, description="Whether to include details in the response"
    )
...
class PerplexityWorkflows(Workflow):
    ...
    def extract_data_from_perplexity(
        ...
        file_path_history = (
            dir_path / f"_{timestamp}_perplexity_{subject_id}_{topic_id}"
        )
        file_response_path = dir_path / f"_perplexity_response_{subject_id}_{topic_id}"

        # Check if data already exists
        if (
            os.path.exists(file_path.with_suffix(".json"))
            and not self.__configuration.force_update
        ):
            logger.info(
                f"Using existing Perplexity response for {subject_name} - {topic_name}"
            )
            try:
                ...
                logger.error(
                    f"Failed to load existing Perplexity response for {subject_name} - {topic_name}: {str(e)}"
                )
                response_json = {}
        else:
...
{", ".join([f'"{field}": {answer_format.model_fields[field].description}' for field in answer_format.model_fields.keys()])}"""
            ...
            if metadata.get("website"):
                search_domain_filter.append(
                    metadata["website"].split("www.")[1].split("/")[0]
                )
            logger.info(
                f"Extracting data from Perplexity for {subject_name} - {topic_name}"
            )
            try:
                ...
                logger.error(
                    f"Failed to extract data from Perplexity for {subject_name} - {topic_name}: {str(e)}"
                )
                return {}
            ...
            with open(file_path.with_suffix(".txt"), "w") as f:
                f.write(response)
            shutil.copy(
                file_path.with_suffix(".txt"), file_path_history.with_suffix(".txt")
            )
            ...
            shutil.copy(
                file_path.with_suffix(".json"), file_path_history.with_suffix(".json")
            )

            # Extract response JSON
            logger.info(
                f"Parsing Perplexity response for {subject_name} - {topic_name}"
            )
    ...
    def get_organization_details(
        ...
        class AnswerFormat(BaseModel):
            ...
            created_date: Dict[str, str] = Field(
                description="Date when organization was created. Return `day`: (str) day when organization was created, `month`: (str) month when organization was created, `year`: (str) year when organization was created."
            )
            headquarters: List[Dict[str, str]] = Field(
                description="Global and regional headquarters locations. Return `name`: (str) name of the facility, `type`: (str) either 'Global Headquarters' or 'Regional Headquarters', `description`: (str) description of the facility, `country`: (str) country name, `state`: (str) state name, `city`: (str) city name, and `address`: (str) address."
            )
            number_of_employee: int = Field(
                description="Number of employees working in the organization. Return `number_of_employee`: (int) number of employees in the organization."
            )
            industries: List[Dict[str, str]] = Field(
                description="A Group of Organizations that share common characteristics, activities, or business objectives. Return `name`: (str) name of the industry, `definition`: (str) definition of the industry."
            )
    ...
    def get_organization_offerings(
        ...
        class AnswerFormat(BaseModel):
            products: List[Dict[str, List[str]]] = Field(
                description="Tangible items that are created, manufactured, or sourced to satisfy a want or need by the organization. Return `name`: (str) name of the product or category of products, `description`: (str) description of the product."
            )
            services: List[Dict[str, List[str]]] = Field(
                description="Intangible activities or benefits provided by one party to another, often to solve a problem or meet a need by the organization. Return `name`: (str) name of the service, `description`: (str) description of the service."
            )
            ...
            positionings: List[Dict[str, List[str]]] = Field(
                description="Role that represents the strategic position and differentiation strategy of the organization within its market, highlighting how the organization distinguishes itself from competitors and is perceived by its target audience. Return `name`: (str) `description`: (str), `market`: (list) name of the market where the positioning is."
            )
    ...
    def get_organization_members(
        ...
        class AnswerFormat(BaseModel):
            board_of_directors: List[Dict[str, str]] = Field(
                description="The board of directors is a governing body elected by shareholders (in a corporation) to oversee the organization's management. It represents the interests of shareholders and ensures the company's prosperity by collectively directing its affairs. Return all board of directors members with `name`: (str) name of the member, `position`: (str) position of the member"
            )
            executive_committee: List[Dict[str, str]] = Field(
                description="The executive committee are members of the board of directors or top management that are empowered to make certain decisions on behalf of the board or management team. Return all executive committee members with `name`: (str) name of the member, `position`: (str) position of the member"
            )
            top_management: List[Dict[str, str]] = Field(
                description="Top management refers to the highest level of managers within an organization who are responsible for controlling and overseeing the entire organization. They make strategic decisions that affect the company's direction and success. Return all top management members with `name`: (str) name of the member, `position`: (str) position of the member"
            )
    ...
    def get_organization_corporate_structure(
        ...
        class AnswerFormat(BaseModel):
            mergers: List[Dict[str, str]] = Field(
                description="A Social Act involving the consolidation of two or more organizations into a single entity. Return `name`: (str) name of the organization's merger, `description`: (str) description of the merger terms, `year`: (int) year of the merger, `organizations`: (list of str) list of organizations involved in the merger."
            )
            acquisitions: List[Dict[str, str]] = Field(
                description="A Social Act involving one organization purchasing another organization or its assets. Return `name`: (str) name of the organization's acquisition, `description`: (str) description of the acquisition terms, `year`: (int) year of the acquisition, `organizations`: (list of str) list of organizations involved in the acquisition."
            )
            subsidiaries: List[Dict[str, str]] = Field(
                description="A Social Act in which an organization establishes a new subsidiary entity, typically to expand its operations, enter new markets, or achieve strategic objectives. Return `name`: (str) name of the subsidiary (organization), `mission`: (str) mission of the subsidiary, `year`: (int) year of the establishment of the subsidiary."
            )
    ...
    def get_organization_facilities(
        ...
        class AnswerFormat(BaseModel):
            ...
            offices: List[Dict[str, str]] = Field(
                description="Office buildings used for commercial or administrative work. Return 3 main offices with `name`: name of the office, `size`: (str) number of workers, `country`: (str) country name, `state`: (str) state name, `city`: (str) city name, and `address`: (str) address."
            )
            factories: List[Dict[str, str]] = Field(
                description="Manufacturing or production facilities. Return 3 main factories with `name`: name of the factory, `size`: (str) number of employees, `country`: (str) country name, `state`: (str) state name, `city`: (str) city name, and `address`: (str) address."
            )
    ...
    def get_organization_analysis(
        ...
        # Initialize storage
        organization_id, dir_path = self._init_storage(
            parameters.organization_name, parameters.data_store_name
        )
        ...
        for industry in industries:
            ...
            organization_industries_analysis.append(
                self.get_industry_analysis(
                    PerplexityIndustryDetailsParameters(
                        industry_name=industry, details=parameters.details
                    )
                )
            )
        ...
        for market in organization_offerings.get("markets", []):
            ...
            market_analysis = self.get_market_analysis(
                PerplexityMarketDetailsParameters(
        ...
        logger.info(
            f"-----> Saving organization analysis to {file_path.with_suffix('.json')}"
        )
        with open(file_path.with_suffix(".json"), "w") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
        return data

    def get_market_details(self, parameters: PerplexityMarketDetailsParameters) -> Dict:
        class AnswerFormat(BaseModel):
            market_size: List[Dict[str, str]] = Field(
                description="Market size: Volume of sales, revenue, or other relevant metrics. Return `metric`: (str) metric used to measure the market size, `value`: (int) value of the metric, `unit`: (str) unit of the metric, `currency`: (str) currency of the metric, `year`: (int) year, "
            )
            market_segments: List[Dict[str, str]] = Field(
            ...
            cagr: List[Dict[str, str]] = Field(
                description="Compound annual growth rate (CAGR) of the market. Return `value`: (float) CAGR, `unit`: (str) unit of the CAGR, `year`: (int) year."
            )
            key_drivers: List[Dict[str, str]] = Field(
                description="A factor or influence that has a significant impact on the performance or direction of a market, industry, or organization. Return `name`: (str) name of the driver, `description`: (str) description of the driver."
            )
            key_trends: List[Dict[str, str]] = Field(
                description="A general direction or pattern of change observed in a market, industry, or organization over time. Return `name`: (str) name of the trend, `description`: (str) description of the trend."
            )
            key_challenges: List[Dict[str, str]] = Field(
                description="An obstacle or difficulty that must be overcome by an organization, market, or industry to achieve its objectives. Return `name`: (str) name of the challenge, `description`: (str) description of the challenge."
            )
    ...
    def get_market_players(self, parameters: PerplexityMarketDetailsParameters) -> Dict:
        class AnswerFormat(BaseModel):
            competitors: List[Dict[str, str]] = Field(
                description="A group of organizations that create products or services that compete against each other within the same market. Return 5 main competitors `name`: (str) name of the competitor (organization), `market_segments`: (list of str) list of market segments where the competitor operates"
            )
            consumers: List[Dict[str, str]] = Field(
                description="A group of agents (people or organizations) that purchase goods or services from other businesses in the market. Return 5 main consumers or group of consumers `name`: (str) name of the consumer (organization), `market_segments`: (list of str) list of market segments where the consumer operates"
            )
            distributors: List[Dict[str, str]] = Field(
                description="A group of organizations that distribute products from manufacturers to retailers or directly to consumers. Return 5 main distributors `name`: (str) name of the distributor (organization), `market_segments`: (list of str) list of market segments where the distributor operates"
            )
            suppliers: List[Dict[str, str]] = Field(
                description="A group of organizations that provide raw materials, components, or services to other businesses. Return 5 main suppliers `name`: (str) name of the supplier (organization), `market_segments`: (list of str) list of market segments where the supplier operates"
            )
            regulators: List[Dict[str, str]] = Field(
                description="A group of government agencies that set rules and regulations for businesses in the market. Return main regulators `name`: (str) name of the regulator (organization), `market_segments`: (list of str) list of market segments where the regulator operates"
            )
            investors: List[Dict[str, str]] = Field(
                description="A group of organizations that provide capital to businesses in exchange for equity or expected financial returns. Return main investors `name`: (str) name of the investor (organization), `market_segments`: (list of str) list of market segments where the investor operates"
            )
    ...
    def get_market_risks_and_opportunities(
        self, parameters: PerplexityMarketDetailsParameters
    ) -> Dict:
        class AnswerFormat(BaseModel):
            key_opportunities: List[Dict[str, str]] = Field(
                description="A favorable set of circumstances or a possibility that, if leveraged, could lead to a positive outcome or advantage for an organization. Return `name`: (str) name of the opportunity, `description` (str)."
            )
            key_risks: List[Dict[str, str]] = Field(
                description="A potential event or condition that, if it occurs, could have a negative impact on an organization's ability to achieve its objectives. Return `name`: (str) name of the risk, `description` (str)."
            )
    ...
    def get_market_analysis(
        ...
        # Initialize storage
        market_id, dir_path = self._init_storage(
            parameters.market_name, parameters.data_store_name
        )
        ...
        # Save data to JSON file
        logger.info(
            f"-----> Saving market analysis to {file_path.with_suffix('.json')}"
        )
        with open(file_path.with_suffix(".json"), "w") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
        return data
    ...
    def get_industry_structure(
        self, parameters: PerplexityIndustryDetailsParameters
    ) -> Dict:
        class AnswerFormat(BaseModel):
            industry_definition: str = Field(
                description="Definition of the industry. Return `definition`: (str) definition of the industry."
            )
            industry_concentration: str = Field(
                description="Type of market structure (e.g. oligopoly, monopolistic competition, perfect competition). Return `structure`: (str) market structure type, `description`: (str) explanation of the structure."
            )
            major_organizations: List[Dict[str, str]] = Field(
            ...
            small_medium_enterprises: List[Dict[str, str]] = Field(
                description="Smaller organizations specializing in niche markets or services. Return the 5 main organizations in the industry. `name`: (str) name of the organization, `description`: (str) description of specialization and role, `niche_market`: (str) specialized market segment they serve, `innovations`: (list of str) notable innovations or contributions to industry"
            )
            entry_barriers: List[Dict[str, str]] = Field(
                description="Barriers preventing new entrants. Return `barrier`: (str) barrier name, `description`: (str) barrier description, `impact`: (str) impact on competition."
            )
    ...
    def get_industry_demand_and_growth(
        ...
        class AnswerFormat(BaseModel):
            market_size: Dict[str, str] = Field(
                description="Industry market size and growth. Return `current_size`: (str) current market value, `growth_rate`: (str) annual growth rate, `forecast`: (str) future projections."
            )
            customer_segments: List[Dict[str, str]] = Field(
                description="Major customer segments. Return `segment`: (str) segment name, `needs`: (list[str]) key needs/requirements, `size`: (str) segment size."
            )
            demand_factors: List[Dict[str, str]] = Field(
                description="Factors affecting demand. Return `factor`: (str) factor name, `impact`: (str) impact description, `trend`: (str) current trend."
            )
    ...
    def get_industry_technology(
        ...
        class AnswerFormat(BaseModel):
            tech_advancement: Dict[str, str] = Field(
                description="Technology state in industry. Return `level`: (str) advancement level, `description`: (str) technology landscape, `trends`: (list[str]) key technological trends."
            )
            innovation_metrics: Dict[str, str] = Field(
                description="Innovation indicators. Return `r_d_investment`: (str) R&D investment level, `innovation_rate`: (str) innovation frequency, `key_areas`: (list[str]) main innovation areas."
            )
            tech_impact: List[Dict[str, str]] = Field(
                description="Technology's industry impact. Return `area`: (str) affected area, `impact`: (str) impact description, `future_implications`: (str) future outlook."
            )
    ...
    def get_industry_regulations(
        ...
        class AnswerFormat(BaseModel):
            key_regulations: List[Dict[str, str]] = Field(
                description="Major regulations. Return `regulation`: (str) regulation name, `description`: (str) requirements, `impact`: (str) business impact."
            )
            compliance_requirements: List[Dict[str, str]] = Field(
                description="Compliance needs. Return `requirement`: (str) requirement name, `description`: (str) details, `cost_impact`: (str) implementation cost impact."
            )
            regulatory_changes: List[Dict[str, str]] = Field(
                description="Potential regulation changes. Return `change`: (str) proposed change, `likelihood`: (str) probability, `impact`: (str) potential impact."
            )
    ...
    def get_industry_economics(
        ...
        class AnswerFormat(BaseModel):
            profitability: Dict[str, str] = Field(
                description="Industry profitability metrics. Return `average_margin`: (str) typical profit margin, `trend`: (str) margin trend, `factors`: (list[str]) key factors affecting profitability."
            )
            cost_structure: Dict[str, str] = Field(
                description="Main cost components. Return `fixed_costs`: (list[str]) major fixed costs, `variable_costs`: (list[str]) major variable costs, `cost_drivers`: (list[str]) key cost drivers."
            )
            economic_sensitivity: Dict[str, str] = Field(
                description="Economic cycle impact. Return `sensitivity`: (str) sensitivity level, `cycle_impact`: (str) business cycle impact, `risk_factors`: (list[str]) key economic risks."
            )
    ...
    def get_industry_analysis(
        ...
        # Initialize storage
        industry_id, dir_path = self._init_storage(
            parameters.industry_name, parameters.data_store_name
        )
        ...
        # Save data to JSON file
        logger.info(
            f"-----> Saving industry analysis to {file_path.with_suffix('.json')}"
        )
        with open(file_path.with_suffix(".json"), "w") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
        return data
    ...
    def as_tools(self) -> list[StructuredTool]:
        ...
        return [
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_headquarters(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_productions_facilities(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_retail_locations(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_suppliers_and_customers(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_competitors(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_partnerships(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_markets(
                    PerplexityOrganizationParameters(**kwargs)
                ),
                args_schema=PerplexityOrganizationParameters,
            ),
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_market_details(
                    PerplexityMarketDetailsParameters(**kwargs)
                ),
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_market_analysis(
                    PerplexityMarketDetailsParameters(**kwargs)
                ),
                args_schema=PerplexityMarketDetailsParameters,
            ),
...
if __name__ == "__main__":
    ...
    # Init workflow
    force_update = True
    workflow = PerplexityWorkflows(
        PerplexityWorkflowsConfiguration(
    ...
    workflow.get_organization_analysis(
