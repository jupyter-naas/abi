...
class GeneratePeopleOntologyWorkflow(Workflow):
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    GeneratePeopleOntologyWorkflowParameters(**kwargs)
                ),
    ...
    def as_api(self, router: APIRouter) -> None:
        ...
        def generate_people_ontology(
            parameters: GeneratePeopleOntologyWorkflowParameters,
        ):
    ...
    def __get_yaml_ontologies(self, ontologies: list[str]) -> list[str]:
        ...
        for ontology in ontologies:
            response = self.__naas_integration.get_ontology(
                self.__configuration.workspace_id, ontology
            )["ontology"]
    ...
    def run(self, parameters: GeneratePeopleOntologyWorkflowParameters) -> str:
        ...
        workspace_plugins = self.__naas_integration.get_plugins(
        ...
        ontologies = self.__naas_integration.get_ontologies(parameters.workspace_id)[
            "ontologies"
        ]

        # Find the people ontology
        print("Finding the people ontology")
        people_ontology = next(
            (
                ontology
                for ontology in ontologies
                if ontology["label"] == "Peoples - Test"
            ),
            None,
        )
        ...
        if people_ontology is None:
            ...
            people_ontology = self.__naas_integration.create_ontology(
                parameters.workspace_id,
                "Peoples - Test",
                merged_ontologies_yaml,
                "USE_CASE",
            )
        else:
            print("Updating the people ontology")
            self.__naas_integration.update_ontology(
                parameters.workspace_id,
                people_ontology["id"],
                merged_ontologies_yaml,
                "USE_CASE",
