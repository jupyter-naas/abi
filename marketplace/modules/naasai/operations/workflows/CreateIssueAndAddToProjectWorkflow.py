...
class CreateIssueAndAddToProjectParameters(WorkflowParameters):
    ...
    project_id: Optional[int] = Field(
        0,
        description="Project number from GitHub project URL (optional if configured globally)",
    )
    assignees: Optional[List[str]] = Field(
        default_factory=list, description="List of GitHub usernames to assign"
    )
    labels: Optional[List[str]] = Field(
        default_factory=list, description="List of labels to add"
    )
    status_field_id: Optional[str] = Field(
        None, description="Field ID for status column"
    )
    priority_field_id: Optional[str] = Field(
        None, description="Field ID for priority column"
    )
    status_option_id: Optional[str] = Field(
        None, description="Option ID for status value"
    )
    priority_option_id: Optional[str] = Field(
        None, description="Option ID for priority value"
    )
...
class CreateIssueAndAddToProjectWorkflow(Workflow):
    ...
    def run(self, parameters: CreateIssueAndAddToProjectParameters) -> str:
        ...
        return (
            f"Issue '{parameters.issue_title}' created and added to project: {issue}."
        )
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    CreateIssueAndAddToProjectParameters(**kwargs)
                ),
