...
from src.custom.modules.human_resources.pipelines.AddSkilltoPersonPipeline import (
    AddSkilltoPersonPipeline,
    AddSkilltoPersonPipelineConfiguration,
)
from src.custom.modules.human_resources.workflows.GetPersonsAssociatedwithSkillsWorkflow import (
    GetPersonsAssociatedwithSkillsWorkflow,
    GetPersonsAssociatedwithSkillsConfigurationWorkflow,
)
from src.custom.modules.human_resources.workflows.GetSkillsAssociatedwithPersonsWorkflow import (
    GetSkillsAssociatedwithPersonsWorkflow,
    GetSkillsAssociatedwithPersonsConfigurationWorkflow,
)
...
def create_agent(
    ...
    add_individual_pipeline = AddIndividualPipeline(
        AddIndividualPipelineConfiguration(triple_store)
    )
    ...
    # Add pipelines
    add_skill_to_person_pipeline = AddSkilltoPersonPipeline(
        AddSkilltoPersonPipelineConfiguration(triple_store)
    )
    tools += add_skill_to_person_pipeline.as_tools()

    # Add workflows
    get_persons_associated_with_skills_workflow = (
        GetPersonsAssociatedwithSkillsWorkflow(
            GetPersonsAssociatedwithSkillsConfigurationWorkflow(triple_store)
        )
    )
    tools += get_persons_associated_with_skills_workflow.as_tools()

    get_skills_associated_with_persons_workflow = (
        GetSkillsAssociatedwithPersonsWorkflow(
            GetSkillsAssociatedwithPersonsConfigurationWorkflow(triple_store)
        )
    )
