...
from src.core.modules.common.integrations.AlgoliaIntegration import (
    AlgoliaIntegration,
...
class UpdateAlgoliaIndexParameters(WorkflowParameters):
    ...
    index_name: str = Field(
        default="abi-search", description="Name of the Algolia index to update"
    )
...
class UpdateAlgoliaIndex(Workflow):
    ...
    def __init__(self, configuration: UpdateAlgoliaIndexConfiguration):
        ...
        self.__algolia = AlgoliaIntegration(
            self.__configuration.algolia_integration_config
        )
        ...
        for file in agents_dir.rglob("*.py"):
            if file.stem.endswith("Assistant"):
                ...
                try:
                    ...
                    ranking_mapping = {"foundation": 1, "domain": 2, "expert": 3}
                    ranking = ranking_mapping.get(
                        subfolder, 4
    ...
    def __scan_workflows(self, branch_name: str) -> List[Dict[str, str]]:
        ...
        for file in workflows_dir.rglob("*.py"):
            if "Workflow" in file.stem:
                ...
                try:
                    import_path = "src.core.modules.common.workflows"
                        ...
                        if hasattr(module, "as_tools"):
                            try:
                                ...
                                config_class = getattr(
                                    module, f"{file.stem}Configuration"
                                )
                                ...
                                for tool in tool_list:
                                    ...
                                    function_id = hashlib.sha256(
                                        f"{file.stem}_{tool.name}".encode()
                                    ).hexdigest()
                                    tool_name = (
                                        (tool.name)
                                        .split("_", 1)[1]
                                ...
                                logger.warning(
                                    f"Could not get tools for {file.stem}: {e}"
                                )
                except ImportError as e:
    ...
    def __scan_pipelines(self, branch_name: str) -> List[Dict[str, str]]:
        ...
        for file in pipelines_dir.rglob("*.py"):
            if file.stem.endswith("Pipeline"):
                ...
                try:
                    # Build import path based on file location
                    import_path = "src.core.modules.common.pipelines"
                    ...
                    if hasattr(module, class_name):
                        ...
                        source_title = (
                            ...
                            if len(relative_path.parts) > 1
                            else file.stem.replace("Pipeline", "")
                        )
                        logo_url = getattr(module, "LOGO_URL", "")
                        results.append(
                            {
                                "objectID": hashlib.sha256(
                                    file.stem.encode()
                                ).hexdigest(),
                                "type": "data",
                                "category": "pipeline",
            ...
            if file.stem.endswith("AIModel"):
                ...
                try:
                    module = importlib.import_module(
                        f"src.core.modules.common.models.naas.{file.stem}"
                    )
    ...
    def __scan_integrations(self, branch_name: str) -> List[Dict[str, str]]:
        ...
        results = []
        integrations_dir = Path("src/core/modules/common/integrations")
                                    ...
                                    results.append(
                                        {
                                            ...
                                            "source_url": f"https://github.com/jupyter-naas/abi/tree/{branch_name}/src/core/modules/common/integrations/{file.name}",
                                            "ranking": 2,
                                            "args_schema": "\n".join(
                                                [
                                                    f"-{name}: {field.description}\n"
                                                    for name, field in (
                                                        tool.args_schema.__fields__.items()
    ...
    def __scan_analytics(self, branch_name: str) -> List[Dict[str, str]]:
        ...
        for file in analytics_dir.rglob("*.py"):
            ...
            if file.stem.endswith("Analytics"):
                ...
                try:
                    # Build import path based on file location
                    import_path = "src.core.modules.common.analytics"
    ...
    def __scan_ontologies(self, branch_name: str) -> List[Dict[str, str]]:
        ...
        for file in ontologies_dir.rglob("*.ttl"):
            if "__OntologyTemplate__" not in str(
                file
            ) and "ConsolidatedOntology" not in str(file):
                try:
                    g = Graph()
        ...
        for file in ontologies_dir.rglob("*.ttl"):
            ...
            try:
                ...
                # Get ontology title
                ontology_title = next(
                    g.objects(None, DC.title),
                    next(
                        g.objects(None, OWL.Ontology),
                        next(g.objects(None, RDFS.label), ""),
                    ),
                )
                ...
                results.append(
                    {
                        ...
                        "licence": next(
                            g.objects(None, DC.license),
                            next(g.objects(None, DCTERMS.license), ""),
                        ),
                    }
                )
                    ...
                    for s in g.subjects(RDFS.subClassOf, None):
                        if isinstance(s, URIRef):
                            ...
                            if subclass_of:
                                ...
                                subclass_of_label = uri_to_label.get(
                                    str(subclass_of), ""
                                ).capitalize()
                            ...
                            classes.append(
                                {
                                    ...
                                    "title": "Class: "
                                    + next(
                                        g.objects(s, RDFS.label),
                                        str(s).split("/")[-1].split("#")[-1],
                                    ).capitalize(),
                                    "description": next(
                                        g.objects(s, SKOS.definition),
                                    ...
                                    "source_url": str(s),
                                    "ranking": ranking + 1,
                                    "ontology_source_url": next(
                                        g.objects(s, CCO.ont00001760),
                                        f"https://github.com/jupyter-naas/abi/tree/{branch_name}/src/core/ontologies/{file.name}",
                                    ),
                ...
                try:
                    for s, p, o in g.triples((None, RDF.type, OWL.ObjectProperty)):
                        if isinstance(s, URIRef):
                            ...
                            domain_objs = [
                                str(obj)
                                for obj in g.objects(s, RDFS.domain)
                                if str(obj).startswith("http")
                            ]
                            range_objs = [
                                str(obj)
                                for obj in g.objects(s, RDFS.range)
                                if str(obj).startswith("http")
                            ]
                            ...
                            sub_prop_val = (
                                str(sub_prop).split("/")[-1] if sub_prop else ""
                            )
                            ...
                            if sub_prop:
                                ...
                                sub_prop_label = uri_to_label.get(
                                    str(sub_prop), ""
                                ).capitalize()
                ...
                try:
                    for s, p, o in g.triples((None, RDF.type, OWL.DatatypeProperty)):
                        if isinstance(s, URIRef):
                            ...
                            range_val = (
                                str(range_obj).split("#")[-1] if range_obj else ""
                            )
                            range_label = (
                                next(g.objects(range_obj, RDFS.label), "")
                                if range_obj
                                else ""
                            )
                ...
                try:
                    ...
                    logger.info(
                        f"Found {len(annotation_properties)} annotation properties"
                    )
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: asyncio.run(
                    self.run(UpdateAlgoliaIndexParameters(**kwargs))
                ),
...
if __name__ == "__main__":
    ...
    from src.core.modules.common.workflows.operations.UpdateAlgoliaIndexWorkflow import (
        UpdateAlgoliaIndex,
        UpdateAlgoliaIndexConfiguration,
        UpdateAlgoliaIndexParameters,
    )
    ...
    config = UpdateAlgoliaIndexConfiguration(algolia_integration_config=algolia_config)

    # Create workflow
    workflow = UpdateAlgoliaIndex(config)
    asyncio.run(
        workflow.run(UpdateAlgoliaIndexParameters(index_name="workspace-search"))
    )
