...
class CreateGitHubPrivateForkParameters(WorkflowParameters):
    ...
    repository_name: str = Field(
        description="The name of the repository to create a private fork of"
    )
...
class CloneGitHubRepositoryParameters(WorkflowParameters):
    ...
    repo_name: str = Field(
        description="The name of the repository to clone in format: owner/name"
    )
...
class UpdateRepoSecretsParameters(WorkflowParameters):
    ...
    repository_name: str = Field(
        description="The name of the repository to update secrets for"
    )
    secrets: Dict[str, str] = Field(
        description="Dictionary of secret name-value pairs to update"
    )
...
class SetupGithubABIWorkflows(Workflow):
    ...
    def __init__(self, configuration: SetupGithubABIWorkflowsConfiguration):
        ...
        self.__github = GithubIntegration(
            self.__configuration.github_integration_config
        )

    def create_github_private_fork(
        self, parameters: CreateGitHubPrivateForkParameters
    ) -> str:
        ...
        try:
            repo = self.__github.get_repository_details(
                f"{parameters.org_name}/{parameters.repository_name}"
            )
            if repo:
    ...
    def clone_github_repository_setup_remote(
        self, parameters: CloneGitHubRepositoryParameters
    ) -> str:
    ...
    def update_repo_secrets(self, parameters: UpdateRepoSecretsParameters) -> List[str]:
        ...
        for secret_name, secret_value in parameters.secrets.items():
            ...
            results.append(
                f"Successfully updated secret '{secret_name}' in repository '{parameters.repository_name}'"
            )
        return results
    ...
    def as_tools(self) -> list[StructuredTool]:
        ...
        return [
            StructuredTool(
                ...
                func=lambda org_name,
                repository_name,
                repository_description: self.create_github_private_fork(
                    CreateGitHubPrivateForkParameters(
                        org_name=org_name,
            ...
            StructuredTool(
                ...
                func=lambda repo_name: self.clone_github_repository_setup_remote(
                    CloneGitHubRepositoryParameters(repo_name=repo_name)
                ),
            ...
            StructuredTool(
                ...
                func=lambda repository_name, secrets: self.update_repo_secrets(
                    UpdateRepoSecretsParameters(
                        repository_name=repository_name, secrets=secrets
                    )
                ),
