...
class PublishRemoteAgentsWorkflow(Workflow):
    ...
    def publish_remote_agents(self) -> List[str]:
        ...
        workspace_plugins = self.__naas.get_plugins(
            self.__configuration.workspace_id
        ).get("workspace_plugins", [])
        ...
        import_matches = [
            ...
            for match in re.findall(
                r"from (src\.(core|custom)\.agents(?:\.\w+)*\.\w+Assistant) import .*",
                content,
            )
        ]
        ...
        for import_path in import_matches:
            try:
                ...
                if file_path.exists():
                    with open(file_path, "r") as file:
                        content = file.read()
                    find_route_name = re.findall(
                        r'route_name:\s*str\s*=\s*"([^"]*)"', content
                    )
                    if find_route_name:
                    ...
                    plugin_data = {
                        ...
                        "include_date": "true",
                        "remote": {
                            "url": f"{api_base_url}/agents/{route_name}/stream-completion?token={api_key}"
                        },
            ...
            existing_plugin_id = self.__naas.search_plugin(
                key="id", value=plugin_data["id"], plugins=workspace_plugins
            )
            if existing_plugin_id:
                ...
                self.__naas.create_plugin(
                    workspace_id=self.__configuration.workspace_id, data=plugin_data
                )
