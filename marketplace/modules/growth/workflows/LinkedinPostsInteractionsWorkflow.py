...
class LinkedinPostsInteractionsWorkflowParameters(WorkflowParameters):
    ...
    linkedin_urls: List[str] = Field(
        ..., description="List of LinkedIn post URLs. The URLs must contain 'activity'"
    )
    limit: Optional[int] = Field(
        default=100,
        description="Optional. Maximum number of reactions and comments to retrieve. Defaults to 100.",
    )
    ...
    def handle_time_error(self, df_init, column):
        ...
        if column in df.columns:
            for i in range(len(df[column])):
                try:
                    actual_time = pd.to_datetime(df.loc[i, column]).tz_localize(
                        pytz.timezone("Europe/Paris")
                    )
                except NonExistentTimeError:
                    actual_time = str(
                        pd.to_datetime(df.loc[i, column]) + pd.DateOffset(hours=1)
                    )
                    df.loc[i, column] = actual_time
        return df

    def run(
        self, parameters: LinkedinPostsInteractionsWorkflowParameters
    ) -> pd.DataFrame:
        ...
        for linkedin_url in parameters.linkedin_urls:
            ...
            tmp_reactions = self.__linkedin.get_post_reactions(
                linkedin_url, limit=parameters.limit
            )
        ...
        if len(df_reactions) > 0:
            ...
            data_reaction = {
                ...
                "DATE_EXTRACT": pd.to_datetime(df_reactions["DATE_EXTRACT"])
                .dt.tz_localize(pytz.timezone("Europe/Paris"))
        ...
        if len(df_comments) > 0:
            ...
            data_comment = {
                ...
                "DATE_EXTRACT": pd.to_datetime(df_comments["DATE_EXTRACT"])
                .dt.tz_localize(pytz.timezone("Europe/Paris"))
                .dt.strftime("%Y-%m-%d %H:%M:%S%z"),
            }
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    LinkedinPostsInteractionsWorkflowParameters(**kwargs)
                ),
