...
class NewsAPIIntegration(Integration):
    ...
    def __init__(self, configuration: NewsAPIIntegrationConfiguration):
        ...
        self.headers = {"X-Api-Key": self.__configuration.api_key}
...
def as_tools(configuration: NewsAPIIntegrationConfiguration):
    ...
    class TopHeadlinesSchema(BaseModel):
        country: Optional[str] = Field(
            None, description="2-letter ISO 3166-1 country code"
        )
        ...
        page_size: int = Field(
            default=20, description="Number of results per page (max 100)"
        )
    ...
    class EverythingSchema(BaseModel):
        ...
        language: str = Field(
            default="en", description="2-letter ISO-639-1 language code"
        )
        sort_by: str = Field(
            default="publishedAt",
            description="Sort articles by (relevancy, popularity, publishedAt)",
        )
        page_size: int = Field(
            default=20, description="Number of results per page (max 100)"
        )
    ...
    return [
        StructuredTool(
            ...
            q,
            page_size,
            page: integration.get_top_headlines(country, category, q, page_size, page),
        ...
        StructuredTool(
            ...
            sort_by,
            page_size,
            page: integration.get_everything(
