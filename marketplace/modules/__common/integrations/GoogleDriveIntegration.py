...
LOGO_URL = "https://upload.wikimedia.org/wikipedia/commons/thumb/d/da/Google_Drive_logo.png/2048px-Google_Drive_logo.png"
...
class GoogleDriveIntegration(Integration):
    ...
    def list_files(
        ...
        page_size: int = 100,
        fields: str = "files(id, name, mimeType, createdTime, modifiedTime)",
    ) -> List[Dict]:
        ...
        try:
            ...
            raise IntegrationConnectionError(
                f"Google Drive API request failed: {str(e)}"
            )

    def get_file(
        self,
        file_id: str,
        fields: str = "id, name, mimeType, createdTime, modifiedTime",
    ) -> Dict:
        """Get file metadata.
        ...
        """
        try:
            return self.__service.files().get(fileId=file_id, fields=fields).execute()
        except HttpError as e:
            raise IntegrationConnectionError(
                f"Google Drive API request failed: {str(e)}"
            )
...
def as_tools(configuration: GoogleDriveIntegrationConfiguration):
    ...
    class ListFilesSchema(BaseModel):
        query: Optional[str] = Field(
            None, description="Search query (Drive query syntax)"
        )
        page_size: int = Field(
            default=100, description="Maximum number of files to return"
        )
        fields: str = Field(
    ...
    return [
        StructuredTool(
            ...
            func=lambda query, page_size, fields: integration.list_files(
                query, page_size, fields
            ),
