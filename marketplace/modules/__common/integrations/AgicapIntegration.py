...
class AgicapIntegration(Integration):
    ...
    def _get_date_range(
        ...
        last_day_month = (
            first_day_month + relativedelta(months=1) - relativedelta(seconds=1)
        )
    ...
    def export_actual(
        self, company_id: str, account_id: str, date_start: str, date_end: str
    ) -> list[dict]:
        ...
        # Clean up dataframe
        df = df.drop(
            [col for col in df.columns if col.startswith("Unnamed")], axis=1
        ).dropna()
    ...
    def _process_budget_excel(
        self, content: bytes, company_id: str, account_id: str
    ) -> pd.DataFrame:
    ...
    def get_transactions(
        ...
        while True:
            logger.info(
                f"Fetching transactions for account {account_id} with skip {skip} and take {100}"
            )
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    f"Failed to fetch transactions for account {account_id} with skip {skip} and take {100}"
                )
                break
...
def as_tools(configuration: AgicapIntegrationConfiguration):
    ...
    class GetTransactionsSchema(BaseModel):
        ...
        limit: Optional[int] = Field(
            default=100, description="The number of transactions to retrieve"
        )
    ...
    return [
        ...
        StructuredTool(
            ...
            func=lambda company_id,
            account_id,
            date_start,
            date_end: integration.export_actual(
                company_id, account_id, date_start, date_end
            ),
        ...
        StructuredTool(
            ...
            account_id,
            date_start,
            date_end: integration.export_budget(
                company_id, account_id, date_start, date_end
            ),
        ...
        StructuredTool(
            ...
            func=lambda company_id, account_id, limit: integration.get_transactions(
                company_id, account_id, limit
            ),
