...
class SupabaseIntegration(Integration):
    ...
    def insert(
        self, table: str, data: Union[Dict, List[Dict]], upsert: bool = False
    ) -> List[Dict]:
...
def as_tools(configuration: SupabaseIntegrationConfiguration):
    ...
    class SelectSchema(BaseModel):
        ...
        order: Optional[List[Dict[str, str]]] = Field(
            None, description="Order by configuration"
        )
    ...
    return [
        StructuredTool(
            ...
            order,
            limit,
            offset: integration.select(table, columns, filters, order, limit, offset),
