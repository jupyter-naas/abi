...
class LinkedInIntegration(Integration):
    ...
    def _make_request(
        ...
        base = (
            self.__configuration.custom_api_url
            if use_custom_api
        ...
        try:
            ...
            if use_custom_api:
                self.manage_api_error(
                    response
                )  # Add error management before raise_for_status
    ...
    def get_profile_view(
        self, linkedin_url: str = None, sleep: bool = True
    ) -> pd.DataFrame:
        """Get profile view information for a LinkedIn profile.

        Args:
            linkedin_url (str, optional): LinkedIn profile URL (e.g., "https://demo.example/profiles/demo"). Defaults to None.
    ...
    def get_profile_top_card(self, linkedin_url: str) -> Dict:
        ...
        Args:
            linkedin_url (str): LinkedIn profile URL (e.g., "https://demo.example/profiles/demo")
        """
        profile_id = self.get_public_profile_id(linkedin_url)
        return self._make_request(
            "POST", f"/profile/getTopCard?profile_id={profile_id}", use_custom_api=True
        )
    ...
    def get_profile_resume(self, linkedin_url: str) -> Dict:
        ...
        profile_id = self.get_public_profile_id(linkedin_url)
        return self._make_request(
            "POST", f"/profile/getResume?profile_urn={profile_id}", use_custom_api=True
        )
    ...
    def get_profile_posts(
        ...
        while True:
            ...
            for key in keys:
                value = until.get(key)
                if (
                    key in tmp_df.columns
                    and str(value) in tmp_df[key].astype(str).unique().tolist()
                ):
                    until_check = True
                    break
    ...
    def get_organization_details(self, linkedin_url: str) -> pd.DataFrame:
        ...
        response = self._make_request(
            "POST",
            f"/company/getInfo?company_url={self.get_organization_id(linkedin_url)}",
            use_custom_api=True,
        )
    ...
    def get_organization_posts(
        ...
        Args:
            linkedin_url (str): LinkedIn organization URL (e.g., "https://www.linkedin.com/company/demo/")
        ...
        if not df.empty:
            ...
            df.loc[mask, "ENGAGEMENT_SCORE"] = (
                df.loc[mask, "COMMENTS"] + df.loc[mask, "LIKES"]
            ) / df.loc[mask, "VIEWS"]
    ...
    def get_post_stats(self, linkedin_url: str) -> Dict:
        ...
        return self._make_request(
            "POST", f"/post/getStats?activity_id={activity_id}", use_custom_api=True
        )
...
def as_tools(configuration: LinkedInIntegrationConfiguration):
    ...
    class GetPostCommentsSchema(BaseModel):
        ...
        limit: int = Field(
            default=1,
            description="Maximum number of comments to retrieve. Use -1 for no limit.",
        )
    ...
    class GetPostReactionsSchema(BaseModel):
        ...
        limit: int = Field(
            default=1,
            description="Maximum number of reactions to retrieve. Use -1 for no limit.",
        )

    class GetProfilePostsSchema(BaseModel):
        profile_url: str = Field(..., description="LinkedIn profile URL")
        count: int = Field(
            default=1, description="Number of posts per request (max 100)"
        )
        limit: int = Field(
            default=10,
            description="Total number of posts to return. Use -1 for unlimited.",
        )
        until: Optional[Dict] = Field(
            default=None, description="Condition to stop fetching posts"
        )
    ...
    class GetOrganizationPostsSchema(BaseModel):
        organization_url: str = Field(..., description="LinkedIn organization URL")
        count: int = Field(
            default=100, description="Number of posts per request (max 100)"
        )
        limit: int = Field(
            default=-1,
            description="Total number of posts to return. Use -1 for unlimited.",
    ...
    return [
        ...
        StructuredTool(
            ...
            func=lambda organization_url: integration.get_organization(
                organization_url
            ),
        ...
        StructuredTool(
            ...
            func=lambda post_url, limit: integration.get_post_comments(
                post_url, limit=limit
            ),
        ...
        StructuredTool(
            ...
            func=lambda post_url, limit: integration.get_post_reactions(
                post_url, limit=limit
            ),
        ...
        StructuredTool(
            ...
            func=lambda profile_url,
            count=1,
            limit=10,
            until=None,
            pagination_token=None: integration.get_profile_posts(
                profile_url,
                count=count,
                limit=limit,
                until=until,
        ...
        StructuredTool(
            ...
            func=lambda organization_url,
            count=100,
            limit=-1: integration.get_organization_posts(
                organization_url, count=count, limit=limit
            ),
