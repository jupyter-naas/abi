...
def as_tools(configuration: YouTubeIntegrationConfiguration):
    ...
    class CommentsSchema(BaseModel):
        video_id: str = Field(..., description="YouTube video ID")
        max_results: int = Field(
            default=100, description="Maximum number of comments to return"
        )
    ...
    return [
        StructuredTool(
            ...
            func=lambda query, max_results, order: integration.search_videos(
                query, max_results, order
            ),
        ...
        StructuredTool(
            ...
            func=lambda video_id, max_results: integration.get_video_comments(
