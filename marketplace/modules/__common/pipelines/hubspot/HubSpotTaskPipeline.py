...
class HubSpotTaskPipeline(Pipeline):
    ...
    def run(self, parameters: HubSpotTaskPipelineParameters) -> Graph:
        ...
        task_due_date = (
            datetime.strptime(task_timestamp, "%Y-%m-%dT%H:%M:%SZ")
            if task_timestamp
            else None
        )
        ...
        for contact in (
            task_data.get("associations", {}).get("contacts", {}).get("results", [])
        ):
        ...
        for company in (
            task_data.get("associations", {}).get("companies", {}).get("results", [])
        ):
        ...
        for deal in (
            task_data.get("associations", {}).get("deals", {}).get("results", [])
        ):
            deal_id = deal.get("id")
        ...
        owner_data = next(
            (
                owner
                for owner in self.__hubspot_integration.get_owners()
                if str(owner.get("id")) == str(task_owner_id)
            ),
            {},
        )
...
if __name__ == "__main__":
    ...
    # Initialize pipeline
    pipeline = HubSpotTaskPipeline(
        HubSpotTaskPipelineConfiguration(
