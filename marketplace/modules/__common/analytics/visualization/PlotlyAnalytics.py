...
class PlotlyAnalytics(Integration):
    ...
    def _generate_layout(
        self, title: str, xaxis_title: str = None, yaxis_title: str = None
    ) -> dict:
    ...
    def _generate_fake_data(self, chart_type: str, **kwargs) -> dict:
        ...
        if chart_type == "barline":
            ...
            return {
                "x_data": ["Start"]
                + [f"Step {i}" for i in range(1, n_points - 1)]
                + ["End"],
                "y_data": [100]
                + [random.randint(-30, 30) for _ in range(n_points - 2)]
            ...
            end_dates = [
                (base_date + timedelta(days=i + random.randint(3, 10))).strftime(
                    "%Y-%m-%d"
                )
                for i in range(n_points)
            ]
            return {"tasks": tasks, "start_dates": start_dates, "end_dates": end_dates}
            ...
            return {
                ...
                "y_data": sorted(
                    [random.randint(100, 1000) for _ in range(n_points)], reverse=True
                ),
            }
    ...
    def create_barline_chart(
        ...
        # Add bar traces
        for i, (y_data, name, color) in enumerate(
            zip(y_data_bars, bar_names, colors_bars)
        ):
            fig.add_trace(
                go.Bar(
                    name=name,
    ...
    def create_bubble_chart(
        ...
        fig = go.Figure(
            data=[
                go.Scatter(
                    x=x_data, y=y_data, mode="markers", marker=dict(size=size_data)
                )
            ]
        )
    ...
    def create_candlestick_chart(
        ...
        if any(
            data is None
            for data in [x_data, open_data, high_data, low_data, close_data]
        ):
    ...
    def create_heatmap(
        self, z_data=None, x_labels=None, y_labels=None, title="Heatmap"
    ):
    ...
    def create_mapchart(
        self, lat_data=None, lon_data=None, text=None, title="Map Chart"
    ):
        ...
        fig = go.Figure(
            data=go.Scattergeo(lon=lon_data, lat=lat_data, text=text, mode="markers")
        )
    ...
    def create_piechart(self, labels=None, values=None, title="Pie Chart"):
    ...
    def create_treemap(self, labels=None, parents=None, values=None, title="Treemap"):
    ...
    def create_waterfall_chart(
        ...
        fig = go.Figure(
            data=[
                go.Waterfall(
                    x=x_data, y=y_data, connector={"line": {"color": "rgb(63, 63, 63)"}}
                )
            ]
        )
...
def as_tools(configuration: PlotlyAnalyticsConfiguration):
    ...
    class BarLineChartSchema(BaseModel):
        x_data: Optional[List] = Field(
            default=None,
            description="Data for x-axis. If not provided, random dates will be generated",
        )
        y_data_bars: Optional[List[List]] = Field(
            default=None,
            description="Data for bar chart y-axis. If not provided, random values will be generated",
        )
        y_data_lines: Optional[List[List]] = Field(
            default=None,
            description="Data for line chart y-axis. If not provided, random values will be generated",
        )
        bar_names: Optional[List[str]] = Field(
            default=None, description="Names for each bar series"
        )
        line_names: Optional[List[str]] = Field(
            default=None, description="Names for each line series"
        )
        ...
        yaxis_title_l: str = Field(
            default="Left Y Axis", description="Left y-axis label"
        )
        yaxis_title_r: str = Field(
            default="Right Y Axis", description="Right y-axis label"
        )
        colors_bars: Optional[List[str]] = Field(
            default=None, description="Colors for bar series"
        )
        colors_lines: Optional[List[str]] = Field(
            default=None, description="Colors for line series"
        )
    ...
    class BubbleChartSchema(BaseModel):
        ...
        size_data: Optional[List] = Field(
            default=None, description="Data for bubble sizes"
        )
    ...
    class CandlestickChartSchema(BaseModel):
        x_data: Optional[List] = Field(
            default=None, description="Data for x-axis (dates)"
        )
        open_data: Optional[List] = Field(
            default=None, description="Data for open prices"
        )
        high_data: Optional[List] = Field(
            default=None, description="Data for high prices"
        )
        low_data: Optional[List] = Field(
            default=None, description="Data for low prices"
        )
        close_data: Optional[List] = Field(
            default=None, description="Data for close prices"
        )
    ...
    class HeatmapSchema(BaseModel):
        z_data: Optional[List[List]] = Field(
            default=None, description="2D array of values to plot"
        )
    ...
    class TreemapSchema(BaseModel):
        ...
        parents: Optional[List] = Field(
            default=None, description="Parent names for each node"
        )
    ...
    class MapChartSchema(BaseModel):
        ...
        text: Optional[List] = Field(
            default=None, description="Hover text for each point"
        )
        title: str = Field(default="Map Chart", description="Chart title")

    class PieChartSchema(BaseModel):
        labels: Optional[List] = Field(
            default=None, description="Labels for each pie slice"
        )
        values: Optional[List] = Field(
            default=None, description="Values for each pie slice"
        )
    ...
    class GanttChartSchema(BaseModel):
        ...
        start_dates: Optional[List] = Field(
            default=None, description="List of start dates"
        )
    ...
    class LeaderboardSchema(BaseModel):
        x_data: Optional[List] = Field(
            default=None, description="Data for x-axis (e.g. player names)"
        )
        y_data: Optional[List] = Field(
            default=None, description="Data for y-axis (e.g. scores)"
        )
