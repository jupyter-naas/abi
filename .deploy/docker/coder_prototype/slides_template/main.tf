# Slides runtime template (abi-slides): same Docker + sidecar stack as
# abi-code-server, provisioned invisibly for Nexus Slides. Business users never
# see Coder chrome; Nexus hosts Monaco + preview. Push as: abi-slides.
terraform {
  required_providers {
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    set -e
    # 0) Workspace exec sidecar FIRST — Abi/Nexus probe :8378 as soon as the
    #    agent is up. Do not wait on code-server install (often minutes on cold boot).
