...
### Definition
An event where the HTTP POST method is used to submit data to the specified resource, often causing a change in state or side effects on the server.
...
### URI
http://d3fend.mitre.org/ontologies/d3fend.owl#HTTPPostEvent

### Subclass Of
```mermaid
graph BT
    d3fend.owl#HTTPPostEvent(Http<br>post<br>event):::d3fend-->d3fend.owl#HTTPRequestEvent
...
- [Http post event](/docs/ontology/reference/model/D3FENDCore/Event/Digital%20event/Network%20event/Transport%20layer%20event/Tcp%20event/Http%20event/Http%20request%20event/Http%20post%20event/Http%20post%20event.md)
