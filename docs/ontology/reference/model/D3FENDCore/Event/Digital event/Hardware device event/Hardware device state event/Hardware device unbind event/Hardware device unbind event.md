...
### Definition
An event where a device is logically unbound from a system or process, releasing it from exclusive use or integration.
...
### URI
http://d3fend.mitre.org/ontologies/d3fend.owl#HardwareDeviceUnbindEvent

### Subclass Of
```mermaid
graph BT
    d3fend.owl#HardwareDeviceUnbindEvent(Hardware<br>device<br>unbind<br>event):::d3fend-->d3fend.owl#HardwareDeviceStateEvent
...
- [Hardware device unbind event](/docs/ontology/reference/model/D3FENDCore/Event/Digital%20event/Hardware%20device%20event/Hardware%20device%20state%20event/Hardware%20device%20unbind%20event/Hardware%20device%20unbind%20event.md)
