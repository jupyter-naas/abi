...
- Names and emails live in Postgres and in the identity graph, never in the event log. `AnalyticsEventRecorded` still stores `user_email`; it should move to the id only.
...
- Bulk SQL `update()` / `delete()` statements bypass the ORM listener and are not logged.
- Events from tooling outside Nexus (git agents, CLI) stay without an actor or workspace.
- `nexus:WorkspaceRole` and the other `*Role` classes that inhere in GDCs (search, conversation, message, …) remain BFO-invalid; they are outside this decision.
