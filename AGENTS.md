...
- **`.env` is required and gitignored.** The boot `start` script seeds the deploy placeholders (`NAAS_API_KEY`, `ABI_API_KEY`, `NEXUS_API_URL`) from `.env.example`, and `abi dev up` appends the admin login. The engine renders `config.yaml` through Jinja and **hard-fails non-interactively (no TTY) on ANY missing `{{ secret.X }}`**, including secrets inside YAML comments, because Jinja renders before YAML parsing. Root config is keyless-clean for AI/integrations; remaining deploy placeholders are in `.env.example`. If a key is ever missing (e.g. you wiped `.env`), copy it from `.env.example`. Never leave `{{ secret.X }}` in commented YAML; use `SECRET_REF` in examples.
