...
def create_agent():
    ...
    from src.integrations import GithubIntegration, PerplexityIntegration, LinkedinIntegration, ReplicateIntegration, NaasIntegration, AiaIntegration, HubSpotIntegration, StripeIntegration, GithubGraphqlIntegration
    ...
    # Add Github GraphQL integration if access token is present
    if github_token := secret.get('GITHUB_ACCESS_TOKEN'):
