        ...
        html.Div([
            html.Div([
                html.Div([
                    ...
                    html.P('+15% vs prev month', style={'color': 'green'})
                ], style=kpi_box_style),
                html.Div([
                    html.H4('Daily Avg In'),
                    html.H3(f'${data["daily_avg_in"]:,.0f}'),
                    html.P('+8% vs prev month', style={'color': 'green'})
                ], style=kpi_box_style),
                html.Div([
                ...
                html.Div([
                    ...
                    html.P('+5% vs prev month', style={'color': 'green'})
                ], style=kpi_box_style),
                html.Div([
                    html.H4('Pending Payments'),
                    html.H3(f'${data["pending_payments"]:,.0f}'),
                    html.P('+2% vs prev month', style={'color': 'green'})
                ], style=kpi_box_style),
                html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        dcc.Graph(
                            figure=create_daily_distribution_chart(data),
                            style={'height': '100%', 'width': '100%'}
                        )
                    ], style={
                        **kpi_box_style,
...
def create_balance_trend_chart(data):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data['dates'], y=data['balance'], name='Balance', line=dict(color='blue')))
    fig.update_layout(
        title='Balance Trend', 
        height=500,
        paper_bgcolor='rgba(0,0,0,0)',
...
def create_daily_distribution_chart(data):
    ...
    components = {
        ...
        'Supplier Payments': -np.sum(data['cash_out'] * 0.6),  # Assuming 60% is supplier payments
        'CAPEX': -np.sum(data['cash_out'] * 0.2),  # Assuming 20% is CAPEX
        'Bank Fees': -np.sum(data['cash_out'] * 0.05),  # Assuming 5% is bank fees
        'Loan Payments': -np.sum(data['cash_out'] * 0.15),  # Assuming 15% is loan payments
    }
...
def create_forecast_chart(data):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data['dates'], y=data['balance'], name='Historical', line=dict(color='blue')))
