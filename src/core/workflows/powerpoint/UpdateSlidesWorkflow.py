...
@dataclass
class UpdateSlidesWorkflowConfiguration(WorkflowConfiguration):
    ...
    output_dir: str = "datalake/powerpoint-store/update-slides"
    model: str = "o3-mini"

class UpdateSlidesWorkflowParameters(WorkflowParameters):
    ...
    """
    text: str = Field(..., description="Text to be used to update the presentation framework.")
    use_cache: bool = Field(True, description="Use cache to generate the presentation. By default, it is set to True.")

class UpdateSlidesWorkflow(Workflow):
    """Workflow for updating a PowerPoint presentation based on a prompt."""
    __configuration: UpdateSlidesWorkflowConfiguration
    
    def __init__(self, configuration: UpdateSlidesWorkflowConfiguration):
    ...
    def get_template_structure(self) -> list[dict]:
        ...
        shapes = [{"slide_number": slide.get("slide_number"), "shape_id": shape.get("shape_id"), "shape_type": shape.get("shape_type"), "text": shape.get("text")} for slide in template_slides for shape in slide.get("shapes", []) if "<" in shape.get("text", "").lower()]
        return shapes
    
    def count_slides(self) -> int:
        # Load presentation template
        presentation = self.__powerpoint_integration.create_presentation(self.__configuration.template_path)
        return len(self.__powerpoint_integration.list_slides(presentation, text=True))
    
    def update_slides(self, parameters: UpdateSlidesWorkflowParameters) -> str:
        # Load presentation template
        presentation = self.__powerpoint_integration.create_presentation(self.__configuration.template_path)
        ...
        - Format text in 'new_text' in a readable way for the presentation.
    ...
    def as_tools(self) -> list[StructuredTool]:
        ...
        return [
            StructuredTool(
                name="powerpoint_get_template_structure",
                description=f"Get the structure of the template presentation {self.__configuration.template_path}.",
                func=self.get_template_structure,
                args_schema=None
            ),
            StructuredTool(
                name="powerpoint_count_slides_from_template",
                description=f"Count the number of slides in the template presentation {self.__configuration.template_path} to know how many slides you need to generate.",
                func=self.count_slides,
                args_schema=None
            ),
            StructuredTool(
                name="powerpoint_generate_slides",
                description="Generate slides from a user query using the template structure provided by the tool 'powerpoint_get_template_structure'.",
                func=lambda **kwargs: self.update_slides(UpdateSlidesWorkflowParameters(**kwargs)),
                args_schema=UpdateSlidesWorkflowParameters
            ),
        ]
