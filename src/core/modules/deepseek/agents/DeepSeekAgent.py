"""DeepSeek Agent for advanced reasoning and problem-solving via Ollama.

This module provides the DeepSeekAgent class for local AI reasoning using
DeepSeek's R1 8B model through Ollama. Specialized for complex reasoning,
mathematics, scientific analysis, and logical problem-solving.
"""

from abi.services.agent.IntentAgent import IntentAgent, Intent, IntentType, AgentConfiguration
from ..models.deepseek_r1_8b import model
...
def create_agent() -> Optional[IntentAgent]:
    """Create and configure the DeepSeek agent.
    
    Returns:
        Optional[IntentAgent]: Configured DeepSeek agent, or None if model unavailable
        
    Raises:
        ValueError: If DeepSeek model is not available (Ollama not running or model not pulled)
    """
    # Check if model is available
    if not model:
        print("⚠️  DeepSeek model not available. Make sure Ollama is running and 'deepseek-r1:8b' is pulled.")
        return None
    
    # Agent system prompt optimized for DeepSeek R1's reasoning capabilities
...
Remember: I excel at reasoning tasks and run completely locally for maximum privacy and security."""
    ...
    # Type safety check
    if not model:
        raise ValueError("DeepSeek model not available - Ollama not running or deepseek-r1:8b not pulled")
