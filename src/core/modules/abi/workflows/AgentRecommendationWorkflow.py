...
class AgentRecommendationWorkflow(Workflow):
    ...
    def _handle_conditional_blocks(self, template: str, replacements: Dict[str, Any]) -> str:
    ...
    def _format_recommendations(self, results: List[Dict[str, Any]], parameters: AgentRecommendationParameters) -> List[Dict[str, Any]]:
        """Format query results into user-friendly recommendations."""
        recommendations = []
        
        for result in results:
            recommendation = {
                "agent": self._extract_value(result, "agentLabel") or self._extract_value(result, "agent"),
                "model": self._extract_value(result, "model"),
                "provider": self._extract_value(result, "provider"),
                "costs": {
                    "input_per_million_tokens": float(self._extract_value(result, "inputCost") or 0),
                    "output_per_million_tokens": float(self._extract_value(result, "outputCost") or 0),
                    "currency": "USD"
                },
                "performance": {
                    "intelligence_index": float(self._extract_value(result, "intelligenceIndex") or 0),
                    "coding_index": float(self._extract_value(result, "codingIndex") or 0),
                    "math_index": float(self._extract_value(result, "mathIndex") or 0),
                    "output_speed": float(self._extract_value(result, "outputSpeed") or 0)
                },
                "recommendation_reason": self._generate_recommendation_reason(result, parameters.intent_description)
            }
    ...
    def _generate_recommendation_reason(self, result: Dict[str, Any], intent: str) -> str:
    ...
    def as_tools(self) -> list[BaseTool]:
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run_workflow(AgentRecommendationParameters(**kwargs)),
