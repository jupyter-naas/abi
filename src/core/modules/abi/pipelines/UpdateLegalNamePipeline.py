...
class UpdateLegalNamePipelineParameters(PipelineParameters):
    individual_uri: Annotated[str, Field(
        description="URI of the legal name.",
        pattern=URI_REGEX
    )]
...
class UpdateLegalNamePipeline(Pipeline):
    ...
    def run(self, parameters: PipelineParameters) -> Graph:
        if not isinstance(parameters, UpdateLegalNamePipelineParameters):
            raise ValueError("Parameters must be of type UpdateLegalNamePipelineParameters")
        ...
        if parameters.organization_uri:
            check_organization = list(graph.triples((individual_uri, ABI.isLegalNameOf, URIRef(parameters.organization_uri))))
            if len(check_organization) == 0:
                graph_insert.add((individual_uri, ABI.isLegalNameOf, URIRef(parameters.organization_uri)))
