...
from src.core.modules.chatgpt.agents.ChatGPTResponsesAgent import create_agent
...
def test_analyze_image(agent):
    ...
    assert result is not None, result
    assert "Eiffel Tower" not in result, result
...
def test_analyze_pdf(agent):
    ...
    assert result is not None, result
    assert "sébastien bazin" in result.lower(), result