        ...
        for onto_class in onto_classes:
            ...
            if onto_class.startswith("http"):
                ...
                if len(hierarchy) == 0:
                    ...
                    formatted_label_init = hierarchy[0].get("label").replace(' ', '<br>')
                    for k, v in self.ontologies_dict.items():
                        if v in hierarchy[0].get("uri"):
                            ontology_init = k
                            break
                    nodes.append(f"    {safe_name_init}({formatted_label_init}):::{ontology_init}\n")
                    ...
                    for i, level in enumerate(hierarchy[1:], 1):
                        ...
                        parent_name = hierarchy[i-1].get("uri").split("/")[-1] if i > 1 else safe_name_init
                        formatted_label = level.get("label").replace(' ', '<br>')
                        nodes.append(f"    {safe_name}({formatted_label}):::{ontology}-->{parent_name}\n")
                ...
                sub_ontology_reference = onto_class_dict.get("is curated in ontology", [])
                if sub_ontology_reference:
                    sub_ontology_reference_name = sub_ontology_reference[0].split("/")[-1]
                ...
                markdown_content += f"- {ontology_reference}{sub_ontology_reference_label}\n"
                markdown_content += "\n"
                ...
                for dprop in sorted(dproperties, key=lambda x: x.get('__id', 0)):
                    ...
                    if isinstance(d_domain, list):
                        ...
                        d_domain_class_dict = _.filter_(self.onto_classes.values(), lambda x: x['__id'] == d_domain_uri)[0]
                    ...
                    data_properties.append({
                        ...
                        "example": d_example,
                        "domain": {"uri": d_domain_uri, "label": d_domain_label, "level_path": d_domain_level_path},
                ...
                if len(data_properties) > 0:
                    ...
                    markdown_content += "| Ontology | Label | Definition | Example | Domain | Range |\n"
                    markdown_content += "|----------|-------|------------|---------|--------|-------|\n"
                    for dprop in sorted(data_properties, key=lambda x: x.get('level', 0)):
                        ...
                        dprop_domain_level_path = urllib.parse.quote(
                            os.path.join(
                                ...
                                dprop.get('domain', {}).get('label').capitalize() + ".md"
                            )
                        )
                ...
                for oprop in sorted(oproperties, key=lambda x: x.get('__id', 0)):
                    ...
                    if isinstance(o_domain, list):
                        ...
                        o_domain_class_dict = _.filter_(self.onto_classes.values(), lambda x: x['__id'] == o_domain_uri)[0]
                    ...
                    if isinstance(o_range, list):
                        ...
                        o_range_class_dict = _.filter_(self.onto_classes.values(), lambda x: x['__id'] == o_range_uri)
                    ...
                    if isinstance(o_inverse_of, list):
                        ...
                        o_inverse_of_class_dict = _.filter_(self.onto_oprop.values(), lambda x: x['__id'] == o_inverse_of_uri)
                        if len(o_inverse_of_class_dict) > 0:
                            o_inverse_of_class_dict = o_inverse_of_class_dict[0]
                            o_inverse_of_label = o_inverse_of_class_dict.get('label', [''])[0]
                        else:
                            o_inverse_of_label = ""
                    else:
                        o_inverse_of_uri = ""
                    ...
                    object_properties.append({
                        ...
                        "example": o_example,
                        "domain": {"uri": o_domain_uri, "label": o_domain_label, "level_path": o_domain_level_path},
                        "range": {"uri": o_range_uri, "label": o_range_label, "level_path": o_range_level_path},
                        "inverseOf": {"uri": o_inverse_of_uri, "label": o_inverse_of_label},
                ...
                if len(object_properties) > 0:
                    ...
                    markdown_content += "|----------|-------|------------|---------|--------|-------|------------|\n"
                    for oprop in sorted(object_properties, key=lambda x: x.get('level', 0)):
                        ...
                        oprop_domain_level_path = urllib.parse.quote(
                            os.path.join(
                                ...
                                oprop.get('domain', {}).get('label').capitalize() + ".md"
                            )
                        )
                        ...
                        oprop_range_level_path = urllib.parse.quote(
                            os.path.join(
                                ...
                                oprop.get('range', {}).get('label').capitalize() + ".md"
                            )
                        )
