...
_onto_oprop = _.filter_(onto, lambda x: 'http://www.w3.org/2002/07/owl#ObjectProperty' in x['type'] if 'type' in x else None)
...
def get_linked_classes(cls_id, rel_type=None):
    ...
    if cls_id in onto_classes:
        ...
        res = \
            ...
            [get_linked_classes(e, 'intersectionOf') for e in _.get(cls, 'intersectionOf', [])] + \
            [get_linked_classes(e, 'complementOf') for e in _.get(cls, 'complementOf', [])]
        print(res)
        return res
    else:
...
def map_ranges_domains(x):
    if 'domain' in x:
        x['domain'] = _.map_(x['domain'], lambda x: x if 'http' in x else get_linked_classes(x)[0])
    if 'range' in x:
        x['range'] = _.map_(x['range'], lambda x: x if 'http' in x else get_linked_classes(x)[0])
    return x
