...
class AddLegalNamePipelineParameters(PipelineParameters):
    label: str = Field(..., description="Legal name to be added in class: https://www.commoncoreontologies.org/ont00001331")
...
class AddLegalNamePipeline(Pipeline):
    ...
    def run(self, parameters: AddLegalNamePipelineParameters) -> str:
        ...
        if parameters.label and not legal_name_uri:
            legal_name_uri, graph = self.__add_individual_pipeline.run(AddIndividualPipelineParameters(
            ...
            if legal_name_uri.startswith("http://ontology.naas.ai/abi/"):
                ...
                raise ValueError(f"Invalid Legal Name URI: {legal_name_uri}. It must start with 'http://ontology.naas.ai/abi/'.")

        if parameters.organization_uri:
            if parameters.organization_uri.startswith("http://ontology.naas.ai/abi/"):
                graph.add((legal_name_uri, ABI.isLegalNameOf, URIRef(parameters.organization_uri)))
                graph.add((URIRef(parameters.organization_uri), ABI.hasLegalName, legal_name_uri))
            else:
    ...
    def as_tools(self) -> list[StructuredTool]:
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(AddLegalNamePipelineParameters(**kwargs)),
