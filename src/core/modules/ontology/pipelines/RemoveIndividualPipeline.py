...
class RemoveIndividualPipelineParameters(PipelineParameters):
    uris_to_remove: Annotated[List[str], Field(
        description="List of URIs to remove from the ontology",
        min_items=1
    )]
...
class RemoveIndividualPipeline(Pipeline):
    ...
    def run(self, parameters: PipelineParameters) -> Graph:
        if not isinstance(parameters, RemoveIndividualPipelineParameters):
            raise ValueError("Parameters must be of type RemoveIndividualPipelineParameters")
