...
class AddLinkedInPagePipelineParameters(PipelineParameters):
    ...
    linkedin_url: Optional[str] = Field(None, description="LinkedIn URL with the LinkedIn ID as identifier. It must starts with 'https://demo.example/profiles/demo")
    linkedin_public_id: Optional[str] = Field(None, description="LinkedIn Public ID of the individual.")
    linkedin_public_url: Optional[str] = Field(None, description="LinkedIn Public URL of the individual with the LinkedIn Public ID as identifier. It must starts with 'https://demo.example/profiles/demo")
    owner_uris: Optional[List[str]] = Field(None, description="Owners URI from class: https://www.commoncoreontologies.org/ont00001262 or https://www.commoncoreontologies.org/ont00000443. It must start with 'http://ontology.naas.ai/abi/'.")
...
class AddLinkedInPagePipeline(Pipeline):
    ...
    def run(self, parameters: AddLinkedInPagePipelineParameters) -> str:
        ...
        if "/in/" in parameters.label:
            ...
            raise ValueError(f"Invalid LinkedIn URL: {parameters.label}. It must start with 'https://www.linkedin.com/(in|company|school|edu)/[a-zA-Z0-9_-]+(?:/[a-zA-Z0-9_-]+)*'.")
        ...
        if parameters.label and not linkedin_page_uri:
            ...
            if linkedin_page_uri.startswith("http://ontology.naas.ai/abi/"):
                ...
                raise ValueError(f"Invalid LinkedIn Page URI: {linkedin_page_uri}. It must start with 'http://ontology.naas.ai/abi/'.")
        ...
        if parameters.owner_uris:
            for owner_uri in parameters.owner_uris:
                if owner_uri.startswith("http://ontology.naas.ai/abi/"):
                    ...
                    raise ValueError(f"Invalid Owner URI: {owner_uri}. It must start with 'http://ontology.naas.ai/abi/'.")
