...
class AddPersonPipelineParameters(PipelineParameters):
    name: Optional[str] = Field(None, description="Person's name. It must have a first name and a last name (e.g. 'Florent Ravenel') to be added in class: https://www.commoncoreontologies.org/ont00001262")
    individual_uri: Optional[str] = Field(None, description="URI of the person if already known. It must start with 'http://ontology.naas.ai/abi/'.")
    ...
    linkedin_page_uri: Optional[str] = Field(None, description="LinkedIn Page URI from class: http://ontology.naas.ai/abi/LinkedInProfilePage or http://ontology.naas.ai/abi/LinkedInCompanyPage or http://ontology.naas.ai/abi/LinkedInSchoolPage. It must start with 'http://ontology.naas.ai/abi/'.")
    skill_uris: Optional[List[str]] = Field(None, description="List of skills URIs from class: https://www.commoncoreontologies.org/ont00000089. URIs must start with 'http://ontology.naas.ai/abi/'.")
    ...
    def run(self, parameters: AddPersonPipelineParameters) -> str:
        ...
        if parameters.linkedin_page_uri:
            if parameters.linkedin_page_uri.startswith("http://ontology.naas.ai/abi/"):
                graph.add((person_uri, ABI.hasLinkedInPage, URIRef(parameters.linkedin_page_uri)))
                graph.add((URIRef(parameters.linkedin_page_uri), ABI.isLinkedInPageOf, person_uri))
            else:
