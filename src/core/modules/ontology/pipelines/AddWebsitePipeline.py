...
class AddWebsitePipelineParameters(PipelineParameters):
    label: str = Field(..., description="Website URL (e.g., 'https://www.example.com') to be added in class: http://ontology.naas.ai/abi/Website")
    ...
    owner_uris: Optional[List[str]] = Field(None, description="Owners URI from class: https://www.commoncoreontologies.org/ont00001262 or https://www.commoncoreontologies.org/ont00000443")
...
class AddWebsitePipeline(Pipeline):
    ...
    def run(self, parameters: AddWebsitePipelineParameters) -> str:
        ...
        if parameters.label and not website_uri:
            website_uri, graph = self.__add_individual_pipeline.run(AddIndividualPipelineParameters(
            ...
            if website_uri.startswith("http://ontology.naas.ai/abi/"):
                ...
                raise ValueError(f"Invalid Website URI: {website_uri}. It must start with 'http://ontology.naas.ai/abi/'.")
