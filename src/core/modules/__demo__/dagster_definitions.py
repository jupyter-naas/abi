...
def my_asset(context: dagster.AssetExecutionContext, config: MyAssetConfig):
    os.makedirs("rss_feed", exist_ok=True)
    
    # convert title to filename
    filename = config.entry["title"].replace(" ", "_").replace(":", "_").replace("/", "_").replace("?", "_") + ".txt"
    
    # Prepend date to filename
    from datetime import datetime
    date = datetime.strptime(config.entry["published"], "%a, %d %b %Y %H:%M:%S %Z").strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"{date}_{filename}"
    
    with open(os.path.join("rss_feed", filename), "w") as f:
