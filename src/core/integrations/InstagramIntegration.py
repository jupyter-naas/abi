...
class InstagramIntegration(Integration):
    ...
    def get_user_profile(self) -> Dict:
        ...
        response = requests.get(
            f"{self.__configuration.base_url}/me",
            params=params
        )
    ...
    def get_media(self, limit: int = 25) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.__configuration.base_url}/me/media",
            params=params
        )
        response.raise_for_status()
    ...
    def get_media_insights(self, media_id: str) -> Dict:
        ...
        response = requests.get(
            f"{self.__configuration.base_url}/{media_id}/insights",
            params=params
        )
        response.raise_for_status()
        return response.json()
...
def as_tools(configuration: InstagramIntegrationConfiguration):
    ...
    class GetMediaInsightsSchema(BaseModel):
        media_id: str = Field(..., description="ID of the media item to get insights for")
