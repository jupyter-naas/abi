...
from src.core.abi.pipelines.MergeIndividualsPipeline import (
...
def test_merge_individuals_pipeline(pipeline: MergeIndividualsPipeline):
    from src import services
    from rdflib import Graph, URIRef, RDFS, Literal, OWL, Namespace, RDF, SKOS
    ...
    graph.add((uri_to_keep, RDF.type, URIRef("https://www.commoncoreontologies.org/ont00000443")))
    graph.add((uri_to_keep, RDF.type, OWL.NamedIndividual))
    graph.add((uri_to_keep, RDFS.label, Literal("Naas.ai")))
    graph.add((uri_to_keep, ABI.logo, Literal("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9gXMaBLQZ39W6Pk53PRuzFjUvv_6lLRWPoQ&s")))
    
    uri_to_merge = ABI[str(uuid4())]
    graph.add((uri_to_merge, RDF.type, URIRef("https://www.commoncoreontologies.org/ont00000443")))
    graph.add((uri_to_merge, RDF.type, OWL.NamedIndividual))
    graph.add((uri_to_merge, RDFS.label, Literal("Naas.ai 2")))
    graph.add((uri_to_merge, ABI.logo, Literal("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9gXMaBLQZ39W6Pk53PRuzFjUvv_6lLRWPoQ&s")))
    ...
    graph_merged = pipeline.run(MergeIndividualsPipelineParameters(
        uri_to_keep=uri_to_keep,
        uri_to_merge=uri_to_merge,
    ))
    assert graph_merged is not None, graph_merged.serialize(format="turtle")
    assert str(graph_merged.value(uri_to_keep, RDFS.label)) == "Naas.ai", graph_merged.serialize(format="turtle")
    assert str(graph_merged.value(uri_to_keep, SKOS.altLabel)) == "Naas.ai 2", graph_merged.serialize(format="turtle")
    assert len(list(graph_merged.triples((uri_to_keep, ABI.logo, None)))) == 1, graph_merged.serialize(format="turtle")
