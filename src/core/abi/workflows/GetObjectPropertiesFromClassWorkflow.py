...
class GetObjectPropertiesFromClassWorkflow(Workflow):
    ...
    def __init__(self, configuration: GetObjectPropertiesFromClassWorkflowConfiguration):
    ...
    def get_object_properties_from_class(self, parameters: GetObjectPropertiesFromClassWorkflowParameters) -> dict:
        ...
        if not self.is_class(parameters.class_uri):
            raise ValueError(f"URI {parameters.class_uri} is not a class in the ontology")
