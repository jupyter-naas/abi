...
from src.core.abi.agents.OntologyEngineerAgent import create_agent
...
def test_text_to_ontology_transformation(agent):
    intent = "Map to ontology: 'Alice Dupont is working for Naas.ai'"
    result = agent.invoke(intent)
    assert result is not None, result
    assert "Alice Dupont is working for Naas.ai" in result, result
