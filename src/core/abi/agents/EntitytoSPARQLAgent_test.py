...
from src.core.abi.agents.EntitytoSPARQLAgent import create_agent
...
def test_basic_functionality_and_output_structure(agent):
    ...
    assert result_str is not None and result_str.strip() != "", "Agent should return non-empty result"
    
    # Test for original text acknowledgment
    assert "Entity Extraction Analysis" in result_str, "Should contain Entity Extraction Analysis section"
    assert "original text" in result_str.lower(), "Should acknowledge original text"
    
    # Test for entity extraction explanation
    assert "Extracted Entities" in result_str, "Should contain Extracted Entities section"
    assert "BFO" in result_str, "Should reference BFO ontology framework"
    assert "Continuants" in result_str or "Occurrents" in result_str, "Should categorize entities as Continuants or Occurrents"
    
    # Test for relationship analysis
    assert "Relationships" in result_str, "Should contain relationship analysis"
    assert "Justification" in result_str or "Reasoning" in result_str, "Should provide justification for relationships"
    ...
    # Test date
    assert datetime.now().strftime("%Y-%m-%d") in result_str, "Should contain today's date"