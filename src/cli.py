...
def ensure_local_services_running():
    ...
    if not services_running:
        console.print(
            "⚠️  Local services not running. Attempting to start...", style="yellow"
        )
        try:
            ...
            if result.returncode != 0:
                ...
                console.print(
                    "🔧 Try running: make docker-cleanup && make local-up", style="cyan"
                )
                console.print(
                    "💡 Or start services manually and restart this command.",
                    style="dim",
                )
                return
        except subprocess.TimeoutExpired:
            console.print(
                "⏱️ Service startup timed out. Docker may be stuck.", style="yellow"
            )
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if r.status_code == 200:
                        console.print(
                            "✓ Local services are ready!", style="bright_green"
                        )
                        console.print(
                        ...
                        console.print(
                            "✓ PostgreSQL (Agent Memory): localhost:5432", style="dim"
                        )
                        console.print(
                ...
                if attempt == max_attempts - 1:
                    console.print(
                        "⚠️ Local services are taking longer than expected to start",
                        style="yellow",
                    )
        except subprocess.CalledProcessError:
            console.print(
                "⚠️ Could not start local services. Make sure Docker is running.",
                style="yellow",
            )
            console.print(
                "You can start them manually with: make local-up", style="dim"
            )
...
def ensure_ollama_running():
    ...
    while not ollama_running:
        try:
            ...
            console.print(
                "I need Ollama running to use my local brain. Let me help you get it started!",
                style="bright_cyan",
            )
            console.print("💡 Just run: `ollama run qwen3:8b`", style="dim")
            console.print(
                "💡 If you don't have Ollama yet, get it here: https://ollama.com/download",
                style="dim",
            )
...
def define_ai_mode():
    ...
    mode_choice = Prompt.ask(
        "Which would you prefer?", choices=["1", "2", "3"], default="1"
    )
...
def define_naas_api_key():
    ...
    naas_key = Prompt.ask(
        "What's your Naas API key?\nGet one for free at https://naas.ai/account/api-key",
        default="",
    )
    ...
    while not valid_naas_api_key:
        try:
            r = requests.get(
                "https://api.naas.ai/iam/apikey",
                headers={"Authorization": f"Bearer {naas_key}"},
            )
            r.raise_for_status()
            if r.status_code == 200:
    ...
    if not os.path.exists("config.yaml"):
        ...
        for attempt in range(max_attempts):
            workspace_id = Prompt.ask(
                "What's your Naas workspace ID? (Find it at https://naas.ai/account/settings)"
            )
            while not re.match(
                ...
                console.print(
                    "Invalid workspace ID format. It should be a UUID like: 96ce7ee7-e5f5-4bca-acf9-9d5d41317f81",
                    style="red",
                )
            ...
            if response.status_code == 200:
                ...
                console.print(
                    "Invalid workspace ID. Please try again ({} attempts left).".format(
        ...
        github_repo = Prompt.ask(
            "What's your Github repository name? (e.g. jupyter-naas/abi)"
        )
        ...
        while not (
            ...
            or re.match(r"^[a-zA-Z0-9-]+/[a-zA-Z0-9-]+$", github_repo)
        ):
            console.print(
                "Invalid repository format. It should be 'jupyter-naas/abi' or follow the format 'owner/repo'",
                style="red",
            )
...
def extract_required_keys_from_module(module_path):
    ...
    try:
        ...
        req_match = re.search(
            r"def requirements\(\):(.*?)(?=def|\Z)", content, re.DOTALL
        )
        if not req_match:
            return []
...
def check_modules_requirements():
    ...
    for base_path in base_paths:
        ...
        for root, dirs, files in os.walk(base_path):
            ...
            if (
                any(indicator in dirs for indicator in module_indicators)
    ...
    console.print(
        f"Found {len(enabled_modules)} modules enabled in the project.\n", style="blue"
    )
    ...
    if available_modules:
        console.print(
            f"\n💡 {len(available_modules)} additional modules are available but not configured.",
            style="dim",
        )
        console.print(
            "   To enable modules, edit your config.yaml file and set enabled: true",
            style="dim",
        )
        console.print(
            f"   Available: {', '.join([m.split('/')[-1] for m in available_modules[:5]])}",
            style="dim",
        )
    ...
    for module_path in sorted(all_modules):
        try:
            ...
            if hasattr(module, "requirements"):
                ...
                try:
                    ...
                    if module.requirements():
                        console.print(
                            f"✅ '{module_display_name}' module requirements satisfied",
                            style="green",
                        )
                        continue  # Skip key collection, move to next module
                except Exception as e:
                    console.print(
                        f"⚠️ Could not check requirements for '{module_display_name}': {e}",
                        style="yellow",
                    )
                ...
                if not required_keys:
                    ...
                    console.print(
                        f"❌ '{module_display_name}' module will be disabled - requirements not satisfied",
                        style="red",
                    )
                    continue

                console.print(
                    f"🔑 '{module_display_name}' module requires the following keys: {', '.join(required_keys)}",
                    style="cyan",
                )
                ...
                for key in required_keys:
                    ...
                    if not current_value:
                        user_input = Prompt.ask(
                            f"What is your {key}? (press enter to skip)", default=""
                        )
            ...
            console.print(
                f"⚠️ Could not check requirements for {module_path}", style="yellow"
            )
            continue
    ...
    for module_path in modules_to_disable:
        ...
        if not module_exists:
            config["modules"].append({"path": module_path, "enabled": False})
...
def main(agent_name="AbiAgent"):
    ...
    if ai_mode == "airgap":
        ...
        for f in essential_checks:
            f()
        console.print(
            "🔒 Airgap mode: Skipping cloud service and module requirements checks",
            style="dim",
        )
    else:
    ...
    if ai_mode != "airgap":
        console.print("Starting agent...", style="green")
    from naas_abi.apps.terminal_agent.main import generic_run_agent
