...
class GithubIntegration(Integration):
    ...
    def create_organization_repository(self, org: str, name: str, private: bool = True, description: str = "") -> Dict:
    ...
    def update_organization_repository(
        ...
        if security_and_analysis is not None: data["security_and_analysis"] = security_and_analysis
        if has_issues is not None: data["has_issues"] = has_issues
        if has_projects is not None: data["has_projects"] = has_projects
        if has_wiki is not None: data["has_wiki"] = has_wiki
        if is_template is not None: data["is_template"] = is_template
        if default_branch is not None: data["default_branch"] = default_branch
        if allow_squash_merge is not None: data["allow_squash_merge"] = allow_squash_merge
        if allow_merge_commit is not None: data["allow_merge_commit"] = allow_merge_commit
        if allow_rebase_merge is not None: data["allow_rebase_merge"] = allow_rebase_merge
        if allow_auto_merge is not None: data["allow_auto_merge"] = allow_auto_merge
        if delete_branch_on_merge is not None: data["delete_branch_on_merge"] = delete_branch_on_merge
        if allow_update_branch is not None: data["allow_update_branch"] = allow_update_branch
        if use_squash_pr_title_as_default is not None: data["use_squash_pr_title_as_default"] = use_squash_pr_title_as_default
        if squash_merge_commit_title is not None: data["squash_merge_commit_title"] = squash_merge_commit_title
        if squash_merge_commit_message is not None: data["squash_merge_commit_message"] = squash_merge_commit_message
        if merge_commit_title is not None: data["merge_commit_title"] = merge_commit_title
        if merge_commit_message is not None: data["merge_commit_message"] = merge_commit_message
        if archived is not None: data["archived"] = archived
        if allow_forking is not None: data["allow_forking"] = allow_forking
        if web_commit_signoff_required is not None: data["web_commit_signoff_required"] = web_commit_signoff_required

        return self._make_request("PATCH", f"/repos/{org}/{repo_name}", headers=headers, data=data)
    ...
    def list_repository_activities(
        ...
        return self._make_request("GET", f"/repos/{repo_name}/activity", headers=headers, params=params)
    ...
    def get_repository_public_key(
        ...
        """
        return self._make_request("GET", f"/repos/{repo_name}/actions/secrets/public-key")
    ...
    def create_or_update_repository_secret(
        ...
        encrypted_value_b64 = nacl.encoding.Base64Encoder.encode(encrypted_value).decode("utf-8")
        
        data = {
            "encrypted_value": encrypted_value_b64,
            "key_id": key_id
        }
        return self._make_request("PUT", f"/repos/{repo_name}/actions/secrets/{secret_name}", data=data)
    ...
    def delete_repository_secret(
        ...
        Note:
            ...
            - Fine-grained tokens need "Secrets" repository write permissions
        """
        self._make_request("DELETE", f"/repos/{repo_name}/actions/secrets/{secret_name}")
