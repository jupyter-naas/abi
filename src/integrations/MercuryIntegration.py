...
class MercuryIntegration(Integration):
    ...
    def get_transactions(
        ...
        response = self._make_request(f"/account/{account_id}/transactions", params=params)
    ...
    def get_transaction_details(self, account_id: str, transaction_id: str) -> Dict:
        ...
        """
        return self._make_request(f"/account/{account_id}/transactions/{transaction_id}") 
...
def as_tools(configuration: MercuryIntegrationConfiguration):
    ...
    class GetTransactionsSchema(BaseModel):
        account_id: str = Field(..., description="Account ID")
        limit: Optional[int] = Field(..., description="Limit how many transactions to retrieve. Defaults to 500")
        offset: Optional[int] = Field(..., description="Number of most recent transactions to omit. Defaults to 0")
        status: Optional[str] = Field(..., description="Transaction status filter (\"pending\"|\"sent\"|\"cancelled\"|\"failed\"). Defaults to \"sent\"")
        ...
        search: Optional[str] = Field(..., description="Search term to look for in transaction descriptions")
