...
class GCPFunctionsIntegration(Integration):
    ...
    def list_functions(self, 
        ...
        try:
            ...
            raise IntegrationConnectionError(f"Cloud Functions operation failed: {str(e)}")
...
def as_tools(configuration: GCPFunctionsIntegrationConfiguration):
    ...
    class ListFunctionsSchema(BaseModel):
        show_deleted: bool = Field(default=False, description="Include deleted functions")
        filter_str: Optional[str] = Field(None, description="Filter string for functions")
    ...
    class GetLogsSchema(BaseModel):
        function_name: str = Field(..., description="Name of the function")
        start_time: Optional[str] = Field(None, description="Start time in RFC3339 format")
