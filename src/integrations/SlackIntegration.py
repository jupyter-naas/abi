    ...
    def _make_request(self, endpoint: str, method: str = "GET", params: Dict = None, json: Dict = None) -> Dict:
        ...
        try:
            ...
            if not data.get('ok', False):
                raise IntegrationConnectionError(f"Slack API error: {data.get('error', 'Unknown error')}")
...
def as_tools(configuration: SlackIntegrationConfiguration):
    ...
    class HistorySchema(BaseModel):
        ...
        limit: int = Field(default=100, description="Maximum number of messages to return")
