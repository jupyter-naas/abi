...
class GoogleCalendarIntegration(Integration):
    ...
    def list_events(self,
        ...
        try:
            ...
            raise IntegrationConnectionError(f"Google Calendar API request failed: {str(e)}")
    ...
    def get_event(self, event_id: str, calendar_id: str = 'primary') -> Dict:
        ...
        """
        try:
            return self.__service.events().get(
                calendarId=calendar_id,
                eventId=event_id
            ).execute()
        except HttpError as e:
    ...
    def create_event(self,
        ...
        try:
            ...
            return self.__service.events().insert(
                calendarId=calendar_id,
                body=event,
                sendUpdates='all'
            ).execute()
    ...
    def update_event(self,
        ...
        try:
            ...
            return self.__service.events().update(
                calendarId=calendar_id,
                eventId=event_id,
                body=event,
                sendUpdates='all'
...
def as_tools(configuration: GoogleCalendarIntegrationConfiguration):
    ...
    class ListEventsSchema(BaseModel):
        ...
        time_min: Optional[str] = Field(None, description="Start time (RFC3339 timestamp)")
        time_max: Optional[str] = Field(None, description="End time (RFC3339 timestamp)")
        ...
        single_events: bool = Field(default=True, description="Whether to expand recurring events")
    ...
    class CreateEventSchema(BaseModel):
        ...
        attendees: Optional[List[Dict[str, str]]] = Field(None, description="List of attendees")
        calendar_id: str = Field(default='primary', description="Calendar ID")

    class UpdateEventSchema(BaseModel):
        event_id: str = Field(..., description="Event ID")
        summary: Optional[str] = Field(None, description="Event title")
        start: Optional[Dict[str, str]] = Field(None, description="Start time (dateTime or date)")
        end: Optional[Dict[str, str]] = Field(None, description="End time (dateTime or date)")
        description: Optional[str] = Field(None, description="Event description")
        location: Optional[str] = Field(None, description="Event location")
        attendees: Optional[List[Dict[str, str]]] = Field(None, description="List of attendees")
        calendar_id: str = Field(default='primary', description="Calendar ID")
    ...
    return [
        StructuredTool(
            ...
            func=lambda calendar_id, time_min, time_max, max_results, order_by, single_events:
                integration.list_events(calendar_id, time_min, time_max, max_results, order_by, single_events),
