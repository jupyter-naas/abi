...
class PerplexityOrganizationParameters(WorkflowParameters):
    ...
    metadata: Optional[Dict] = Field(description="Metadata of the organization. A dictionary with the following keys: `website`")
...
class PerplexityOrganizationWorkflows(Workflow):
    ...
    def init_storage(self, organization_name: str) -> Tuple[str, str]:
        ...
        return os.path.join(self.__configuration.data_store_path, f"{organization_id}"), organization_id
    ...
    def extract_data_from_perplexity(
        ...
        try:
            completion = services.storage_service.get_object(output_dir, filename).decode("utf-8")
        ...
        if completion is None or use_cache is False:
            logger.info(f"Extracting data from Perplexity for {organization_name} - {topic_name}")
            source_details = ""
            if add_source:
            ...
            if metadata.get("website"):
                try:
                    ...
                    logger.warning(f"Could not parse website domain from metadata: {str(e)}")
            ...
            try:
                ...
                raise Exception(f"Failed to extract data from Perplexity for {organization_name} - {topic_name}: {str(e)}")
    ...
    def get_organization_general_information(
        ...
        class AnswerFormat(BaseModel):
            ...
            organization_type: str = Field(description="Represents the type of organization. Return value from list: commercial_organization, civil_organization, educational_organization, government_organization.")
            ...
            created_year: int = Field(description="Year when organization was created. Return `created_year`: (int) year when organization was created.")
            headquarters: List[Dict[str, str]] = Field(description="Global and regional headquarters locations. Return `type`: (str) either 'global_headquarters' or 'regional_headquarters', `country`: (str) country name, `state`: (str) state name, `city`: (str) city name, and `address`: (str) address.")
            number_of_employees: List[Dict[str, str]] = Field(description="Number of employees working in the organization. Return `value`: (int) number of employees in the organization, `year`: (int) year when the number of employees was last updated.")
            industries: List[Dict[str, str]] = Field(description="A Group of Organizations that share common characteristics, activities, or business objectives. Return list of industries where the organization operates with `name`: (str) name of the industry, `definition`: (str) definition of the industry.")
    ...
    def get_organization_members(
        ...
        class AnswerFormat(BaseModel):
            founders: List[Dict[str, str]] = Field(description="Individuals who have established or founded an organization, typically recognized for their role in the creation and initial development of that organization. Return list of founders with `name`: (str) name of the founder, `role`: (str) role of the founder")
            chairmans: List[Dict[str, str]] = Field(description="Chairman is the highest-ranking executive in a company, responsible for overseeing the company's operations and making strategic decisions. Return list of persons with `name`: (str) name of the chairman, `role`: (str) role of the chairman")
            president: str = Field(description="President is the second-highest-ranking executive in a company, responsible for overseeing the company's operations and making strategic decisions. Return `name` and `start_date`")
            vice_presidents: List[Dict[str, str]] = Field(description="Vice President is the third-highest-ranking executive in a company, responsible for overseeing the company's operations and making strategic decisions. Return `name`, `role` and `start_date`")
            ceo: str = Field(description="Chief Executive Officer (CEO) is the highest-ranking executive in a company, responsible for overseeing the company's operations and making strategic decisions. Return `name` and `start_date`")
            cfo: str = Field(description="Chief Financial Officer (CFO) is the highest-ranking financial executive in a company, responsible for overseeing the company's financial operations and making strategic decisions. Return `name` and `start_date`")
            cto: str = Field(description="Chief Technology Officer (CTO) is the highest-ranking technology executive in a company, responsible for overseeing the company's technology operations and making strategic decisions. Return `name` and `start_date`")
    ...
    def get_organization_offerings(
        ...
        class AnswerFormat(BaseModel):
            products: List[Dict[str, str]] = Field(description="Tangible items that are created, manufactured, or sourced to satisfy a want or need by the organization. Return list of products with `name` (str) and`description` (str).")
            services: List[Dict[str, str]] = Field(description="Intangible activities or benefits provided by one party to another, often to solve a problem or meet a need by the organization. Return list of services with `name` (str) and `description` (str).")
            ...
            positionings: List[Dict[str, List[str]]] = Field(description="Role that represents the strategic position and differentiation strategy of the organization within its market, highlighting how the organization distinguishes itself from competitors and is perceived by its target audience. Return list of positionings with `name` (str), `definition` (str), `market`: (list) name of the market where the positioning is, `products`: (list of str) Organization listed products in the positioning, `services`: (list of str) Organization listed services in the positioning.")
    ...
    def get_organization_analysis(
        ...
        organization_general_information = self.get_organization_general_information(parameters)
        ...
        data = {
            ...
            "organization_general_information": organization_general_information.get("response_json", {}),
            ...
            "organization_capabilities": organization_capabilities.get("response_json", {}),
            "organization_corporate_structure": organization_corporate_structure.get("response_json", {}),
            "organization_strategic_alliances": organization_strategic_alliances.get("response_json", {}),
            "organization_profit_and_loss": organization_profit_and_loss.get("response_json", {}),
            "organization_key_indicators": organization_key_indicators.get("response_json", {}),
            "date_extracted": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }
    ...
    def as_tools(self) -> list[StructuredTool]:
        ...
        return [
            ...
            StructuredTool(
                ...
                func=lambda **kwargs: self.get_organization_strategic_alliances(PerplexityOrganizationParameters(**kwargs)),
                args_schema=PerplexityOrganizationParameters
            ),
            StructuredTool(
...
if __name__ == "__main__":
    ...
    # Init workflow
    workflow = PerplexityOrganizationWorkflows(PerplexityOrganizationWorkflowsConfiguration(
    ...
    data = workflow.get_organization_analysis(PerplexityOrganizationParameters(
        organization_name=organization_name,
