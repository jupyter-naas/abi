...
from src.utils.SPARQL import get_identifier, get_subject_graph
from abi.utils.String import create_hash_from_string
from rdflib import Graph, URIRef, Literal, RDFS, RDF, OWL, XSD
import uuid
...
from src.utils.Storage import get_powerpoint_presentation
...
class AddPowerPointPresentationPipelineParameters(PipelineParameters):
    presentation_name: Annotated[str, Field(
        description="Name of the presentation",
    )]
    storage_path: Annotated[str, Field(
        description="Storage path for the presentation",
    )]
    download_url: Optional[Annotated[str, Field(
        description="Download URL for the presentation",
    )]] = None
    template_uri: Optional[Annotated[str, Field(
...
class AddPowerPointPresentationPipeline(Pipeline):
    ...
    def __init__(self, configuration: AddPowerPointPresentationPipelineConfiguration):
        ...
        self.__powerpoint_integration = PowerPointIntegration(configuration.powerpoint_configuration)


    def run(self, parameters: PipelineParameters) -> Graph:
        if not isinstance(parameters, AddPowerPointPresentationPipelineParameters):
            raise ValueError("Parameters must be of type AddPowerPointPresentationPipelineParameters")
        
        # Get all shapes and slides from presentation
        if parameters.storage_path.startswith("src"):
            presentation = self.__powerpoint_integration.create_presentation(parameters.storage_path)
        else:
            presentation = Presentation(get_powerpoint_presentation(os.path.dirname(parameters.storage_path), os.path.basename(parameters.storage_path)))
        ...
        try:
            ...
            logger.info(f"✅ Generated signature from document properties: {identifier}")
        ...
        graph.add((presentation_uri, PPT.storage_path, Literal(parameters.storage_path)))
        if author is not None:
            graph.add((presentation_uri, PPT.author, Literal(author)))
        if created_date is not None:
            graph.add((presentation_uri, PPT.created_date, Literal(created_date, datatype=XSD.dateTime)))
        if last_modified_by is not None:
            graph.add((presentation_uri, PPT.last_modified_by, Literal(last_modified_by)))
        if modified_date is not None:
            graph.add((presentation_uri, PPT.modified_date, Literal(modified_date, datatype=XSD.dateTime)))
        if parameters.download_url is not None:
            graph.add((presentation_uri, PPT.download_url, Literal(parameters.download_url)))
        if parameters.template_uri is not None:
            graph.add((presentation_uri, PPT.hasTemplate, URIRef(parameters.template_uri)))
            graph.add((URIRef(parameters.template_uri), PPT.isTemplateOf, presentation_uri))
        ...
        for shape_and_slide in shapes_and_slides:
            ...
            for shape in shapes:
                ...
                graph.add((shape_uri, PPT.shape_position_left, Literal(shape_position_left, datatype=XSD.decimal)))
                graph.add((shape_uri, PPT.shape_position_top, Literal(shape_position_top, datatype=XSD.decimal)))
                graph.add((shape_uri, PPT.shape_size_width, Literal(shape_size_width, datatype=XSD.decimal)))
                graph.add((shape_uri, PPT.shape_size_height, Literal(shape_size_height, datatype=XSD.decimal)))
                graph.add((shape_uri, PPT.shape_rotation_angle, Literal(shape_rotation, datatype=XSD.decimal)))
    ...
    def as_tools(self) -> list[BaseTool]:
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(AddPowerPointPresentationPipelineParameters(**kwargs)),
