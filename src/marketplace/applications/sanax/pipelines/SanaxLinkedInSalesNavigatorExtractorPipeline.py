...
class SanaxLinkedInSalesNavigatorExtractorPipeline(Pipeline):
    ...
    def run(self, parameters: PipelineParameters) -> Graph:
        ...
        logger.debug(f"Step 1: Reading Excel file '{parameters.file_path}' with sheet name '{parameters.sheet_name}'")
        ...
        # Read Excel file
        dir_path = os.path.dirname(parameters.file_path) 
        file_name = os.path.basename(parameters.file_path)
        try:
            df = get_excel(
                dir_path,
                file_name,
            ...
            try:
                df = pd.read_excel(parameters.file_path, sheet_name=parameters.sheet_name, skiprows=0)
                logger.debug(f"✅ Successfully read local file: {parameters.file_path}")
                      
            except Exception as e:
                logger.debug(f"❌ File not found locally: {e}")
                raise

        # Validate file is not empty
        if len(df) == 0:
            logger.error(f"❌ Excel file: {file_name}, sheet: {parameters.sheet_name}, rows: {len(df)}")
            return Graph()
        ...
        for i, row in df.iterrows():
            count_row += 1
            logger.debug(f"Step 3.1: Adding backing datasource component for row {count_row}/{len(df)}")
            ...
            logger.debug(f"Step 3.2: Adding person '{person_name}'")
            ...
            logger.debug(f"Step 3.3: Adding LinkedIn profile page: '{linkedin_profile_url}'")
            ...
            logger.debug(f"Step 3.5: Adding organization: '{organization}' and LinkedIn company page: '{linkedin_company_page_url}'")
            if organization != "":
                ...
                logger.debug(f"Step 3.6: Time in role: '{time_in_role}'")
                logger.debug(f"Step 3.7: Time in company: '{time_in_company}'")
                ...
                logger.debug(f"Step 3.8: Time in role start date: '{time_in_role_start_date}'")
                logger.debug(f"Step 3.9: Time in company start date: '{time_in_company_start_date}'")
                ...
                logger.debug(f"Step 3.10: Adding act of association with organization and role: '{act_of_association_label}'")
                ...
                logger.debug(f"Step 3.11: Adding act of association with organization: '{act_of_association_label_2}'")
            ...
            logger.debug(f"Step 3.12: Adding location: '{location}'")
        ...
        if len(graph) > 0:
            ...
            save_triples(
                ...
                f"insert_{file_name.split('.')[0]}_{parameters.sheet_name}.ttl"
            )
