...
class LinkedInExportConnectionsPipeline(Pipeline):
    ...
    def _process_row(
        ...
        if email_address_uri is None:
            ...
            with locks["email_addresses"]:
                ...
                if email_address_uri is None:
                    ...
                    shared_caches["email_addresses"][email_address_hash] = email_address_uri
        ...
        if act_of_association_hash != "":
            # First check without lock (fast path)
            act_of_association_uri = shared_caches["act_of_associations"].get(act_of_association_hash)
            if act_of_association_uri is None:
                ...
                with locks["act_of_associations"]:
                    ...
                    if act_of_association_uri is None:
                        ...
                        shared_caches["act_of_associations"][act_of_association_hash] = act_of_association_uri
        ...
        logger.debug(f"Step 3.7: Adding act of connection LinkedIn: '{act_of_connection_label}'")
        act_of_connection_uri = None
        if act_of_connection_hash != "":
            # First check without lock (fast path)
            act_of_connection_uri = shared_caches["act_of_connections"].get(act_of_connection_hash)
            if act_of_connection_uri is None:
                ...
                with locks["act_of_connections"]:
                    ...
                    if act_of_connection_uri is None:
                        ...
                        shared_caches["act_of_connections"][act_of_connection_hash] = act_of_connection_uri
    ...
    def run(self, parameters: PipelineParameters) -> Graph:
        ...
        shared_caches = {
            "email_addresses": get_identifiers(class_uri=EMAIL_ADDRESS),
            "positions": get_identifiers(class_uri=POSITION),
            "organizations": get_identifiers(RDFS.label, class_uri=ORGANIZATION),
            "act_of_associations": get_identifiers(class_uri=ACT_OF_ASSOCIATION),
            "act_of_connections": get_identifiers(class_uri=ACT_OF_CONNECTION),
        }
        ...
        logger.debug(f"Step 4: Processing rows with {self.__configuration.num_workers} workers")
        ...
        processed_count = 0
        with ThreadPoolExecutor(max_workers=self.__configuration.num_workers) as executor:
