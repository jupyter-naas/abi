...
from ..workflows.LinkedInOrganizationsWorkflows import (
    LinkedInOrganizationWorkflows,
    LinkedInOrganizationWorkflowsConfiguration,
    LinkedInOrganizationParameters,
)
...
class LinkedInGetOrganizationLogoPipeline(Pipeline):
    ...
    def __init__(self, configuration: LinkedInGetOrganizationLogoPipelineConfiguration):
        ...
        self.__linkedin_organization_workflows = LinkedInOrganizationWorkflows(
            configuration.linkedin_organization_workflows_config
        )
    ...
    def trigger(
        ...
        s, p, o = triple
        logger.info(
            f"==> Triggering LinkedIn Get Organization Logo Pipeline: {s} {p} {o}"
        )
        if str(event) == str(OntologyEvent.INSERT):
            return self.run(
                LinkedInGetOrganizationLogoPipelineParameters(
                    ontology_name=ontology_name, organization_uri=s
                )
            )
        return None
    ...
    def run(self, parameters: LinkedInGetOrganizationLogoPipelineParameters) -> None:
        ...
        logger.info(
            f"-----> Get organization name from ontology: {parameters.ontology_name}"
        )
        query = f"""
        ...
        for row in results:
            ...
            organization_logo_url = (
                self.__linkedin_organization_workflows.get_organization_logo_url(
                    LinkedInOrganizationParameters(organization_name=organization_name)
                )
            )
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    LinkedInGetOrganizationLogoPipelineParameters(**kwargs)
                ),
