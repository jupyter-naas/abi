...
from ..pipelines.PerplexityOrganizationAnalysisPipeline import (
    PerplexityOrganizationAnalysisPipeline,
    PerplexityOrganizationAnalysisPipelineConfiguration,
)
...
def create_agent(
    ...
    if naas_api_key:
        ...
        li_at = (
            NaasIntegration(NaasIntegrationConfiguration(api_key=naas_api_key))
            .get_secret("li_at")
            .get("secret")
            .get("value")
        )
    ...
    if perplexity_api_key and naas_api_key and li_at and JSESSIONID:
        ...
        # Setup pipeline
        pipeline = PerplexityOrganizationAnalysisPipeline(
            PerplexityOrganizationAnalysisPipelineConfiguration(
    ...
    # Add tools
    agents = [create_support_agent(AgentSharedState(thread_id=2), agent_configuration)]
    ...
    agent_configuration.system_prompt = SYSTEM_PROMPT.replace(
        "[AGENTS]", agents_info_str
    )
