...
class AssignIssuesToProjectWorkflowParameters(WorkflowParameters):
    ...
    project_id: int = Field(
        ..., description="The project number in GitHub (from Project URL)"
    )
    state: str = Field("open", description="Issue state to filter by")
    status_field_id: Optional[str] = Field(
        None, description="The field ID for the status column in the project"
    )
    priority_field_id: Optional[str] = Field(
        None, description="The field ID for the priority column in the project"
    )
    status_option_id: Optional[str] = Field(
        None, description="The option ID for the status value to set"
    )
    priority_option_id: Optional[str] = Field(
        None, description="The option ID for priority value to set"
    )
...
class AssignIssuesToProjectWorkflow(Workflow):
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    AssignIssuesToProjectWorkflowParameters(**kwargs)
                ),
    ...
    def run(self, parameters: AssignIssuesToProjectWorkflowParameters) -> list:
        ...
        issues = self.__github_integration.get_issues(
            parameters.repo_name, state=parameters.state
        )
        ...
        project_data: dict = self.__github_graphql_integration.get_project_node_id(
            organization, parameters.project_id
        )
        ...
        assigned_issues = []
        for issue in issues:
            project_item = self.__github_graphql_integration.get_item_id_from_node_id(
                issue["node_id"]
            )
            item_id = None
            for x in (
            ...
            if item_id is None:
                ...
                logger.debug(
                    f"Assigned issue {issue['number']} to project {parameters.project_id}"
                )
            else:
                logger.debug(
                    f"Issue {issue['number']} already assigned to project {parameters.project_id}"
                )
