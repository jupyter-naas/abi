...
from src.marketplace.modules.applications.algolia.integrations.AlgoliaIntegration import (
    AlgoliaIntegration, 
...
def test_search(integration: AlgoliaIntegration):
    ...
    results = asyncio.run(integration.search(index_name=index_name, query=query, hits_per_page=hits_per_page, filters=filters))
