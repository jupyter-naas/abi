...
from ..workflows.PerplexityGetOrganizationWorkflows import (
    PerplexityOrganizationWorkflows,
...
class PerplexityOrganizationAnalysisPipelineConfiguration(PipelineConfiguration):
    ...
    perplexity_organization_workflows_config: (
        PerplexityOrganizationWorkflowsConfiguration
    )
...
class PerplexityOrganizationAnalysisPipelineParameters(PipelineParameters):
    ...
    organization_name: str = Field(
        ..., description="Name of the organization to analyze."
    )
...
class PerplexityOrganizationAnalysisPipeline(Pipeline):
    ...
    def __init__(
        self, configuration: PerplexityOrganizationAnalysisPipelineConfiguration
    ):
        super().__init__(configuration)
        self.__configuration = configuration
        self.__naas_integration = NaasIntegration(configuration.naas_integration_config)
        self.__perplexity_organization_workflows = PerplexityOrganizationWorkflows(
            configuration.perplexity_organization_workflows_config
        )
        self.__create_ontology_yaml = CreateOntologyYAML(
            configuration.create_ontology_yaml_config
        )
    ...
    def run(self, parameters: PerplexityOrganizationAnalysisPipelineParameters) -> None:
        ...
        logger.info(
            f"-----> Extracting data from Perplexity: {parameters.organization_name}, {parameters.website}"
        )
        response = self.__perplexity_organization_workflows.get_organization_analysis(
            PerplexityOrganizationParameters(
        ...
        organization_legal_name = organization_general_information.get(
            "legal_name", None
        )
        organization_type = organization_general_information.get(
        ...
        organization_created_year = organization_general_information.get(
            "created_year", None
        )
        organization_headquarters = organization_general_information.get(
            "headquarters", []
        )
        organization_number_of_employee = organization_general_information.get(
            "number_of_employees", None
        )
        organization_industries = organization_general_information.get("industries", [])
        organization_ticker_symbol = organization_general_information.get(
            "ticker_symbol", None
        )
        organization_stock_market = organization_general_information.get(
            "stock_market", None
        )
        ...
        if organization_stock_market is not None:
            ...
            graph.add(
                (stock_market_uri, CCO.ont00001977, organization_uri)
            )  # has affiliate
            graph.add(
                (organization_uri, CCO.ont00001939, stock_market_uri)
            )  # is affiliated with
        ...
        if organization_headquarters is not None:
            for headquarter in organization_headquarters:
                ...
                if headquarter.get("country") is not None:
                    ...
                    graph.add(
                        (headquarter_uri, BFO.BFO_0000171, country_uri)
                    )  # is located in
                    graph.add(
                        (country_uri, BFO.BFO_0000124, headquarter_uri)
                    )  # location of
                ...
                if headquarter.get("address") is not None:
                    address_uri = graph.add_individual_to_prefix(
                        prefix=ABI,
                        uid=create_id_from_string(
                            organization_name + "_headquarters_address"
                        ),
        ...
        graph = add_offering(
            graph, organization_uri, organization_products, ABI.Product
        )
        graph = add_offering(
            graph, organization_uri, organization_services, ABI.Service
        )
        ...
        graph = add_capabilities(
            graph,
            organization_uri,
            organization_technological_capabilities,
            ABI.TechnologicalCapabilities,
        )
        graph = add_capabilities(
            graph,
            organization_uri,
            organization_human_capabilities,
            ABI.HumanCapabilities,
        )
        ...
        graph = add_corporate_structure(
            graph,
            organization_id,
            organization_uri,
            organization_mergers,
            ABI.ActOfOrganizationalMerger,
            ABI.OrganizationMerger,
        )
        graph = add_corporate_structure(
            graph,
            organization_id,
            organization_uri,
            organization_acquisitions,
            ABI.ActOfOrganizationalAcquisition,
            ABI.OrganizationAcquisition,
        )
        ...
        organization_partnerships = organization_strategic_alliances.get(
            "partnerships", []
        )
        organization_joint_ventures = organization_strategic_alliances.get(
            "joint_ventures", []
        )
        organization_marketing_alliances = organization_strategic_alliances.get(
            "marketing_alliances", []
        )
        organization_research_collaborations = organization_strategic_alliances.get(
            "research_collaborations", []
        )
        organization_technology_licensing = organization_strategic_alliances.get(
            "technology_licensing", []
        )
        organization_distribution_agreements = organization_strategic_alliances.get(
        ...
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_partnerships,
            ABI.ActOfPartnership,
            ABI.Partnership,
        )
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_joint_ventures,
            ABI.ActOfJointVenture,
            ABI.JointVenture,
        )
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_marketing_alliances,
            ABI.ActOfMarketingAlliance,
            ABI.MarketingAlliance,
        )
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_research_collaborations,
            ABI.ActOfResearchCollaboration,
            ABI.ResearchCollaboration,
        )
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_technology_licensing,
            ABI.ActOfTechnologyLicensing,
            ABI.TechnologyLicensing,
        )
        graph = add_strategic_alliances(
            graph,
            organization_id,
            organization_uri,
            organization_distribution_agreements,
            ABI.ActOfDistributionAgreement,
        ...
        def add_financial_data(
            ...
            if financial_data is not None:
                ...
                for f in financial_data:
                    ...
                    graph.add(
                        (organization_uri, subproperty_object, financial_data_uri)
                    )
                    graph.add(
                        (
                            financial_data_uri,
        ...
        logger.info(
            f"-----> Publishing ontology YAML for '{organization_name}' in workspace: {config.workspace_id}"
        )
        ontology_workspace_id = self.__create_ontology_yaml.graph_to_yaml(
            CreateOntologyYAMLParameters(
        ...
        logger.info(
            f"-----> Creating or updating plugin in workspace: {config.workspace_id}"
        )
        workspace_plugins = self.__naas_integration.get_plugin(config.workspace_id).get(
            "workspace_plugins", []
        )
        plugin_id = None
        ...
        if plugin_id is None:
            ...
            plugin = self.__naas_integration.create_plugin(
                workspace_id=config.workspace_id, data=plugin
            )
            plugin_id = plugin.get("workspace_plugin", {}).get("id", None)
            logger.info(
                f"✅ Plugin ({plugin_id}) created in workspace: {config.workspace_id}"
            )
        else:
            plugin["ontologies"] = [ontology_workspace_id]
            self.__naas_integration.update_plugin(
                workspace_id=config.workspace_id, plugin_id=plugin_id, data=plugin
            )
            logger.info(
                f"✅ Plugin ({plugin_id}) updated in workspace: {config.workspace_id}"
            )
        ...
        return [
            StructuredTool(
                ...
                func=lambda **kwargs: self.run(
                    PerplexityOrganizationAnalysisPipelineParameters(**kwargs)
                ),
