...
data_source = st.sidebar.selectbox(
    "Data Source",
    ["Sample Data", "Upload CSV", "Database Connection", "API Endpoint"]
)

# Table View Options
view_mode = st.sidebar.selectbox(
    "View Mode",
    ["Standard Table", "Editable Grid", "Summary View", "Pivot Table"]
)
...
def load_sample_data():
    ...
    for i in range(200):
        data.append({
            ...
            'Due_Date': pd.Timestamp(np.random.choice(dates)) + timedelta(days=np.random.randint(1, 30))
        })
...
if view_mode == "Standard Table":
    ...
    if 'Category' in display_df.columns:
        summary = display_df.groupby('Category').agg({
            'ID': 'count',
    ...
    if len(display_df) > 0:
        ...
        with col1:
            index_col = st.selectbox("Row (Index)", display_df.columns, key="pivot_index")
        with col2:
            if len(display_df.columns) > 1:
                columns_col = st.selectbox("Column", [None] + list(display_df.columns), key="pivot_columns")
            else:
                columns_col = None
        with col3:
...
# Footer
st.markdown("---")
st.markdown(f"**Table Mode Interface** | Records: {len(filtered_df):,} | View: {view_mode} | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
