...
view_mode = st.sidebar.selectbox(
    "View Mode",
    ["Kanban Board", "Timeline View", "Resource View", "Sprint View"]
)
...
def load_project_data():
    projects = pd.DataFrame({
        ...
        'Title': [
            'Mobile App Redesign', 'API Integration', 'Database Migration', 'User Authentication',
            'Payment Gateway', 'Analytics Dashboard', 'Performance Optimization', 'Security Audit',
            'Marketing Campaign', 'Sales Automation', 'Customer Portal', 'Inventory System',
            'Reporting Module', 'Cloud Migration', 'Quality Assurance'
        ],
        'Status': np.random.choice(['Backlog', 'In Progress', 'Review', 'Testing', 'Done'], 15),
        'Priority': np.random.choice(['Low', 'Medium', 'High', 'Critical'], 15),
        'Team': np.random.choice(['Engineering', 'Design', 'Marketing', 'Sales', 'Operations'], 15),
        'Assignee': np.random.choice(['Alice', 'Bob', 'Charlie', 'Diana', 'Eve', 'Frank'], 15),
        'Start_Date': pd.date_range(start=datetime.now()-timedelta(days=60), periods=15, freq='4D'),
        'Due_Date': pd.date_range(start=datetime.now()+timedelta(days=10), periods=15, freq='5D'),
...
with col1:
    active_projects = len(filtered_projects[filtered_projects['Status'].isin(['In Progress', 'Review', 'Testing'])])
...
with col2:
    ...
    st.metric("Overdue Projects", overdue_projects, "🚨" if overdue_projects > 0 else "✅")

with col3:
    avg_progress = filtered_projects['Progress'].mean()
    st.metric("Avg Progress", f"{avg_progress:.1f}%", f"{np.random.uniform(-5, 10):.1f}%")
...
if view_mode == "Kanban Board":
    ...
    with col1:
        fig_progress = px.bar(filtered_projects, x='Title', y='Progress',
    ...
    with col2:
        ...
        fig_days = px.scatter(filtered_projects, x='Days_Remaining', y='Progress',
    ...
    with col1:
        ...
        workload = filtered_projects.groupby('Assignee').agg({
            'Story_Points': 'sum',
            'Project_ID': 'count'
        }).rename(columns={'Project_ID': 'Project_Count'})
        
        fig_workload = px.bar(workload, x=workload.index, y='Story_Points',
    ...
    with col2:
        sprint_projects = filtered_projects[filtered_projects['Status'].isin(['In Progress', 'Review', 'Testing'])]
        completed_points = sprint_projects[sprint_projects['Status'] == 'Testing']['Story_Points'].sum()
    ...
    actual_burndown = [total_points - (total_points/14 * day) + np.random.uniform(-5, 5) for day in days]
...
with col1:
    ...
    high_risk = filtered_projects[
        (filtered_projects['Days_Remaining'] < 7) | 
        (filtered_projects['Progress'] < 50)
    ]
...
with col2:
    ...
    blocked_projects = [
        {"id": "PRJ-008", "title": "Security Audit", "blocker": "Waiting for security team"},
        {"id": "PRJ-012", "title": "Inventory System", "blocker": "Database schema approval"},
        {"id": "PRJ-015", "title": "Quality Assurance", "blocker": "Test environment setup"}
    ]
...
activities = [
    ...
    {"time": "30 min ago", "user": "Bob", "action": "Updated progress on PRJ-007 to 75%"},
    {"time": "1 hour ago", "user": "Charlie", "action": "Created new task in PRJ-011"},
    {"time": "2 hours ago", "user": "Diana", "action": "Completed PRJ-002: User Authentication"},
...
# Footer
st.markdown("---")
st.markdown("**Project Management Board** | Last Updated: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
