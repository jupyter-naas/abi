...
from abi.module.Module import IModule
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Call):
                        if (
                            ...
                            and node.func.id == "get_modules"
                        ):
                            get_modules_found = True
                            break
                        elif (
                            isinstance(node.func, ast.Attribute)
                            and node.func.attr == "get_modules"
                        ):
                            get_modules_found = True
                            break
                ...
                if get_modules_found:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ImportFrom) and node.module:
                            ...
                            if (
                                ".modules." in module_name
                                and module_name != module_path
                            ):
                                ...
                                if "modules" in parts:
                                    ...
                                    if len(parts) > module_idx + 1:
                                        ...
                                        if (
                                            "marketplace" in parts
                                            and module_idx > 0
                                            and parts[module_idx - 1] == "marketplace"
                                        ):
                                            if len(parts) > module_idx + 2:
                                                dep_module_path = "/".join(
                                                    ["src"]
                                                    + parts[
                                                        module_idx - 1 : module_idx + 3
                                                    ]
                                                )
                                            else:
                                                dep_module_path = "/".join(
...
def topological_sort(
    module_paths: List[str], dependencies: Dict[str, Set[str]]
) -> List[str]:
    ...
    remaining = [
        path for path, dep_set in deps.items() if dep_set and path not in result
    ]
    if remaining:
    ...
    if not __loaded:
        ...
        try:
            ...
            for modulepath in enabled_modules:
                ...
                if module.name != "__pycache__" and "disabled" not in module.name:
            ...
            for modulepath in sorted_modules:
                try:
                    ...
                    module_relative_path = ".".join(modulepath.split("/"))
                    ...
                    traceback.print_exc()
                    raise SystemExit(
                        f"Application crashed due to module loading failure: {modulepath}"
                    )
