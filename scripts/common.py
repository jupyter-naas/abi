...
def get_storage_credentials(naas_api_key, workspace_id, storage_name):
    response = requests.post(
        f"https://api.naas.ai/workspace/{workspace_id}/storage/credentials/",
        headers={
...
def get_config():
    ...
    if workspace_id is None:
        print(
            "config.workspace_id is not set in config.yaml file. Please set it and try again."
        )
        exit(1)
    ...
    if storage_name is None:
        print(
            "config.storage_name is not set in config.yaml file. Please set it and try again."
        )
        exit(1)
