...
from naas_abi import config, load_modules
from naas_abi.__modules__ import get_modules
...
def publish_remote_agent(
    ...
    if dry_run:
        ...
        existing_plugins = naas_integration.get_plugins(workspace_id).get(
            "workspace_plugins", []
        )
    ...
    if github_access_token is not None and github_repository is not None:
        if dry_run:
            logger.info(
                f'🧪 [DRY RUN] Would update "ABI_API_KEY" secret in Github repository: {github_repository}'
            )
        else:
            try:
                github_integration = GitHubIntegration(
                    GitHubIntegrationConfiguration(access_token=github_access_token)
                )
                logger.info(
                    f'🔑 Updating "ABI_API_KEY" secret in Github repository: {github_repository}'
                )
                ...
                logger.error(
                    f'❌ Error updating "ABI_API_KEY" secret in Github repository: {github_repository}'
                )
    ...
    for module in modules:
        ...
        for agent in module.agents:
            ...
            route_name = (
                f"agents/{route_name}/stream-completion?token={abi_api_key}".replace(
                    "//", "/"
                )
            )
            ...
            plugin_data = {
                ...
                "type": agent_type,
                "remote": {"url": f"{api_base_url}/{route_name}"},
            ...
            if dry_run:
                ...
                logger.info(
                    f"    - Description: {plugin_data['description'][:100]}{'...' if len(plugin_data['description']) > 100 else ''}"
                )
                if plugin_data["suggestions"]:
                    logger.info(
                        f"    - Suggestions: {len(plugin_data['suggestions'])} items"
                    )

                # Simulate checking for existing plugin
                logger.info(
                    f"🧪 [DRY RUN] Would check if plugin '{plugin_data['id']}' already exists"
                )
                logger.info(
                    f"🧪 [DRY RUN] Would create/update plugin in workspace '{config.workspace_id}'"
                )
            else:
...
if __name__ == "__main__":
    ...
    logger.debug(
        f"  GITHUB_ACCESS_TOKEN: {'✅ Set' if github_access_token else '❌ Missing'}"
    )
    logger.debug(f"  Workspace ID: {'✅ Set' if workspace_id else '❌ Missing'}")
    logger.debug(
        f"  GitHub Repository: {'✅ Set' if github_repository else '❌ Missing'}"
    )
    ...
    if auto_publish_enabled:
        logger.info(
            f"🚫 Excluded agents: {exclude_agents if exclude_agents else 'None'}"
        )
    else:
